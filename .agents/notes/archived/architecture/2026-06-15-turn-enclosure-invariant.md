...
2. **Idle context injection.** `agent.inject()` appends a `context/message` directly. Its real production caller is `cch-tool-bash`, which injects a background-task completion notice from `ctx.bash.onTaskDone` — a callback that fires whenever a background bash task finishes, frequently while the agent is **idle** (between turns).
...
- The `cch-session/invariant` companion registers the check with `ctx.invariants`: when selected, a `user/message` / `context/message` / `steering/message` appended while no turn is open throws an `InvariantError` attributed to `@coco-harness/cch-session`.
