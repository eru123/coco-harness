...
Add `FileSystem.listDir(target, signal?)` to `@coco-harness/cch-fs`.
...
**Keep directory enumeration in each consumer.** Rejected. That would bind product packages such as `cch-skill` to Node/local filesystem behavior and bypass policy/remote/sandboxed backends.
