...
The TUI's terminal title is a single static string (`title`, default `Coco Harness`) shared by every session. A user who runs one agent per tmux pane or terminal tab sees the same label on all of them, so panes are indistinguishable at a glance and the tab bar carries no signal about what each session is doing.
...
- The feature defaults off and is enabled only in the interactive product config (`examples/tui-agent/cordis.yml`) and the scripted PTY fixture. Enabling it in the shared `cch-tui-demo` schema default would fire an extra model call in keyless replay and boot scenarios that send no user message.
