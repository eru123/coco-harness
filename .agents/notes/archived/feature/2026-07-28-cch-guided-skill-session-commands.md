# Agent Note: `cch migrate`/`cch upgrade` seed the first turn with a skill
...
Two recurring flows begin with the user manually invoking one skill and answering its questions: migrating from another coding agent, and upgrading this checkout. Both require the user to know the skill exists and to type `/skill:cch-migrate` or `/skill:cch-upgrade` as the session's first turn. A dedicated entry command that drops the user straight into that guided session removes the discovery step.

## Decision

`cch migrate` and `cch upgrade` boot the ordinary TUI as a fresh session whose first turn auto-invokes a bundled skill (`cch-migrate`, `cch-upgrade`), exactly as if the user typed `/skill:<name>` and pressed Enter.
...
**Freshness is gated in the launcher, not the TUI.** `runSkillSession` always mints a fresh session and provides the slot only when `resumeSessionId === undefined`, so a later `cch --resume <id>` of that session is an ordinary TUI session with no re-injection. The TUI stays generic: it invokes whatever skill it is handed, once, at startup.

**`migrate`/`upgrade` take no default-surface options** (`upgrade` additionally carries the [experimental gate](2026-07-31-experimental-subcommand-gate.md)'s `--experimental`). They carry no `--resume`, `--config`, or `-p`; a guided fresh-session entry has nothing to resume or reconfigure. Any leaked default-surface option fails loud, matching the `web`/`meta` rejection pattern in the Commander adapter. The two modes share one `SkillSessionInvocation` discriminant (`mode: 'migrate' | 'upgrade'`); `bin.ts` maps the mode to `cch-${mode}`.

The `cch-migrate` skill is bundled under `skills/` (shipped through `CCH_BUNDLED_SKILL_DIR`, like `cch-upgrade`). It asks which source agent (opencode/pi/Claude Code/Codex) if unstated, then maps each capability — workspace instructions, personal overlay, skills, hooks, MCP, API/env — to its DSH equivalent, grounded in the actual repo surfaces (the `hooks-claude`/`hooks-codex` bridges, `~/.cch/{config.yaml,.env,AGENTS.md,skills/}`, `AGENTS.md`/`CLAUDE.md`, `mcporter`), and states plainly when a capability has no equivalent.
...
No keyless PTY snapshot: per the maintainer's scope call for this change, unit coverage plus interactive verification suffices, and the seed rides the already-snapshotted `/skill:` render path. Both commands were verified interactively in tmux from a scratch cwd: `cch migrate` loaded `cch-migrate` and asked which source agent; `cch upgrade` loaded `cch-upgrade`, which pulled in `cch-customize` and began checkout discovery.
...
**Seed a natural-language instruction ("use the cch-migrate skill…") instead of `/skill:<name>`.** Rejected here: the literal skill-invocation path renders the skill body into the first turn deterministically, identical to the manual command, rather than depending on the model choosing to load the skill.

**Support `--resume` on `migrate`/`upgrade`.** Rejected: these are one-shot guided entries. A resumed session is an ordinary TUI session reachable through the default surface's `cch --resume <id>`; re-injecting the skill on resume would duplicate the first turn.
