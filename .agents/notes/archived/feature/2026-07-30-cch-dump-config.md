# Agent Note: cch --dump-config prints the composed config tree
...
`cch-app-boot` previously duplicated the include's `!!js` YAML type for patch parsing; it now imports `entryListSchema`, so the dialect has one owner.
...
**A `/dump-config` TUI command instead of flags.** Rejected as the only form: the primary use is a piped `cch --dump-config | diff - <(cch --dump-default-config)` style workflow, which needs a boot-free non-TTY surface. A TUI command can be added later over the same `renderConfigDump`.
