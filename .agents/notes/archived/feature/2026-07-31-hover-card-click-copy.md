...
**Change the common Chinese `copied` label from `Copy succeeded` to `Copied`.** That would alter every existing copy control to satisfy one card interaction. The Workspace dictionary owns the card-specific wording instead.
