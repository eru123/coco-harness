...
A single `HarnessError extends Error` base in `cch-llm` (the leaf package every other imports — no new dependency edge): a stable `code` distinct from `message`, `cause` chaining via `ErrorOptions`, and `name` defaulting to the subclass. `isHarnessError` narrows at seams.

- `LlmError` and `ToolArgsError` (cch-tools) extend it, keeping their existing codes.
