...
- `@coco-harness/cch-jobs` registers running work as `ctx.jobs` and owns job ids, authorization, snapshots, reads, cancellation, waiting, completion listeners, and cleanup.
- `@coco-harness/cch-tool-jobs` exposes `job_output`, `job_list`, and `job_kill`, injects completion notices, and supplies the background-job system-prompt guidance.

Long-running tools are producers. `cch-tool-bash` adapts a `ShellProcess` into incremental output and process cancellation; `cch-tool-subagent` adapts a child run into final output and child disposal. The bash and subagent capability seams remain independent of sessions and the job registry.

`JobRegistry` is the Service Definition in `@coco-harness/cch-jobs`; the process-local provider is `LocalJobRegistry` in `@coco-harness/cch-jobs-local` (the [task-registry contract Agent Note](2026-07-26-job-registry-seam.md) records that split).
...
For background bash, `cch-tool-bash` registers the calling agent as owner. Its hooks map `kill()` to cancellation, `done` to a completed or killed `JobOutcome`, and `readOutput()` to the process's bounded incremental output plus spill and sandbox notices. Generic task tools own ids, status lines, listing, waiting, and completion notices.

For background subagents, `cch-tool-subagent` creates a task-owned `AbortController` and begins provider startup inside the task starter. Cancellation aborts the same signal before or after provider publication. `done` awaits both the child result and child disposal, maps completed output to a final result, maps abort to `killed`, and maps other stop reasons or infrastructure failures to `failed`. Intermediate child history remains in the child session and is not exposed through `readOutput()`.
