...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `cch-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `cch-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `cch-timeout` (`idleWatchdog`) |
| Clear the timer | `cch-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `cch-timeout` (`timeoutOf`) |
