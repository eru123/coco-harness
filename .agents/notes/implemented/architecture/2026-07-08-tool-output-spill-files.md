...
| Package | Role |
|---|---|
| `@coco-harness/cch-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@coco-harness/cch-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@coco-harness/cch-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |

There is no dedicated model-facing Consumer package. The Consumer is the existing `ctx.tools` execution pipeline: `cch-spill-policy` consumes final tool results through the `tools/post-execute` waterfall, and the model follows the backend-supplied retrieval hint for the returned locator.
...
With `cch-spill-policy` configured, a large formatted fetch result is automatically retained and spilled. A deployment demonstrates the behavior by setting the provider resource cap higher than the policy cap:
...
- id: spill-local
  name: '@coco-harness/cch-spill-local'

- id: spill-policy
  name: '@coco-harness/cch-spill-policy'
  config:
...
- `@coco-harness/cch-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@coco-harness/cch-spill` owns saving final text and returning a locator plus retrieval hint.
- `@coco-harness/cch-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
...
- `cch-spill` unit tests pin the seam contract: registration as `ctx.spillStore`, one-implementation-per-context, and disposal release.
- `cch-spill-local` unit tests cover `saveText`, `encodeSegment` sanitization (separators/tilde/whole-segment dots/empty), the session-hash directory, owner-only permissions, distinct paths per save, the configured/private root, and a storage-failure rejection.
- `cch-spill-policy` unit tests drive real tools through `ctx.tools.execute`: disabled-mode no-op, oversized-text replacement, small/non-text passthrough, `read` skip, best-effort fallback (save failure / no backend / no owner), and downstream-composition (bounding a replaced result, preserving `additionalContexts`).
- `cch-tool-web` integration drives `web_fetch` through `ctx.tools.execute` with the real `spill-local` backend + policy, proving the model-facing text changes only by the deliberate spill notice while the spill file holds the full formatted result.
