...
The central `cch-invariants` service owns only configuration, registration uniqueness, child-fiber lifecycle, rollback, disposal, and package-attributed failure. It exposes no generic plugin-shape, service-shape, or startup-assertion helpers and imports no product package.
...
|---|---|
| `cch-session` | Strict sequence growth, turn/step enclosure, and same-step tool call/result pairing. |
| `cch-agent` | Non-repeating agent status and terminal disposal transitions. |
| `cch-scope` | Scoped-event carrier presence and routed-subject consistency. |
| `cch-agent-loop` | Explicitly marked, frozen loop request reconstruction from the session event log. |
| `cch-llm` | Stream block grammar, delta type/index matching, single usage, closed blocks, and terminal finish. |
| `cch-llm-retry` | Durable retry records identify the open turn's latest closed step, remain unique per step, increase monotonically, and stay within retry and non-negative timer bounds. |
| `cch-tools` | Monotonic pre/execute/post stages and immutable final execution/result snapshots. |
| `cch-system-prompt` | Authoritative assembly section, tool, and variable data constraints. |
| `cch-compaction` | Compaction start/summary/end pairing, range endpoints, token counts, and successful-summary presence. |
| `cch-hook-protocol` | Hook invocation/result correlation, dialect, identity, and duration constraints. |
| `cch-sandbox-policy` | Durable `sandbox/mode` events use the closed sandbox-mode vocabulary. |
| `cch-fs` | Filesystem decision/observation events carry usable target and version identities. |
| `cch-goal` | Durable goal snapshots preserve source attribution, rendered content, revisions, lifecycle and timestamp relationships, and sequential admitted rounds. |
| `cch-goal-round-driver` | Goal-sourced continuation messages match the prompt reconstructed from the preceding durable goal state. |
| `cch-subagent` | Provider add/remove and child start/end events preserve identity and pairing. |
| `cch-permission-presets` | Durable permission decisions name a preset in the active permission table. |
| `cch-user-approval` | Approval asked/decided records pair by call and use valid outcomes and policies. |
| `cch-workflow` | Workflow and child-agent start/end events preserve run metadata, identity, outcome, count, and error relations. |
| `cch-jobs` | Current and terminal task snapshots preserve id/kind, owner, status, and timestamp relationships. |
| `cch-tool-todo` | Durable whole-list snapshots use unique trimmed items and closed statuses. |
| `cch-time-context` | Plugin-attributed clock readings agree with the session's open turn, next pre-step position, and elapsed baseline; rendered time parses and does not postdate its event. |
