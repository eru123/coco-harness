...
- `@coco-harness/cch-home-paths` shipped a **second** `resolveDshHome` with the same precedence plus tilde expansion — a near-duplicate of `cch-home` that no gate flagged because the two lived in different packages and had already drifted (only one expanded tildes).
...
`@coco-harness/cch-home` is deleted. Its three importers (`cch-tool-bash`, `cch-skill-filesystem`, `cch-agent-spine-demo`) import `resolveDshHome` from `cch-home-paths`.
