...
`@coco-harness/cch-anonymous-user-id` owns `getOrCreateAnonymousUserId()` and the `$CCH_HOME/.anonymous-user-id` storage contract. `session-telemetry-otel` uses the returned id as OpenTelemetry Resource `user.id`; the `/feedback` success acknowledgement reports `Feedback recorded for session {sessionId}` followed by `Anonymous user: {userId}` on a second line; and direct DeepSeek requests carry it as `x-coco-harness-user-id`. Invalid feedback is rejected before resolving the id, and the DeepSeek adapter resolves it only after credentials succeed, so neither an empty command nor a credential failure creates `.anonymous-user-id`.
