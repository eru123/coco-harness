...
The tools registry never had this problem: it is one host singleton layered per scope over `cch-scope`, so deployment-level tools (MCP servers, plugin entries) register globally while a preset's rows register into that preset's layer.
