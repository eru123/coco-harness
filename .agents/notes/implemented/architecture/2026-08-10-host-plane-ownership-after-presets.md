...
Three limits stay open and are recorded where they bite rather than fixed here: projection key presence is not a per-session capability signal ([`cch-session-projection`](../../../../packages/session/session-projection/README.md)); a superseded standing generation is never reclaimed, which the settings-page authoring flow turns into a per-save cost ([`cch-agent-presets`](../../../../packages/preset/agent-presets/README.md)); and a temporary plugin mounted through `cordis_mount` belongs to the composition rather than the session that mounted it ([`cch-tool-cordis`](../../../../packages/extensions/tool-cordis/README.md)).
...
The warning is advisory, so a deployment that adds a roster to the ACP or SDK-server entry points still starts agents with no tools — it just says so once per agent instead of silently. The invariant reaches only compositions that load `cch-invariants`, which fences package tests and development hosts, not a shipped one.
