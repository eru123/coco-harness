...
- **Keeping `enableRuntime` but moving `enableRow` out of `cch-cmdline`**: relocation fixes the package boundary but keeps the vendored state machine whose semantics (survives reapplication, rollback on failure) must be re-derived at every upstream sync.
...
- **Keeping `--dev` with the row created at runtime**: an interim state of this change; it still needed a mode fork in the prompt contract, a `CCH_WEB_MODE` variable, and creation-versus-user-row arbitration, all to avoid an idle poll whose cost is negligible.

## Consequences

- A deployment that supervises `cch --profile headless` with SIGTERM now observes exit 0 instead of 143; the caller sent the signal and sees no answer on stdout.
- The reload chain runs in every `cch web` process; a deployment that must not expose `/plugins/events` disables the `client-hmr` row in its patch layer.
