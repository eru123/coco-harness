...
The repository also used `SDK` in two meanings. The supported Python and TypeScript clients use the JSON-RPC SDK protocol. The project as a whole is Coco Harness, not an SDK project. The removed SDK project toolchain made the broad meaning obsolete, but prose and names preserved parts of it.
...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@coco-harness/cch-sdk-client`, `@coco-harness/cch-sdk-protocol`, and the wire identity `coco-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. Coco Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
Do not remove an intentional vendor qualifier to avoid repetition. `cch-subagent-cch-sdk` names the Coco Harness SDK provider and avoids confusion with another SDK. Its private class becomes `SdkSubagentProvider` because the class also needs to say what it provides.
...
|---|---|---|
| `@coco-harness/cch-jsonrpc` | `@coco-harness/cch-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@coco-harness/cch-sdk-client`, `@coco-harness/cch-sdk-protocol`, and `coco-harness-sdk-runtime`. Exclude `@coco-harness/create-sdk`, `@coco-harness/cch-scripts`, `@coco-harness/cch-helper`, and `@coco-harness/cch-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@coco-harness/cch-bash`, `ctx.bash` | `@coco-harness/cch-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@coco-harness/cch-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@coco-harness/cch-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@coco-harness/cch-pty`, `ctx.pty`, `PtyService` | `@coco-harness/cch-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@coco-harness/cch-pty-local`, `LocalPtyBackend` | `@coco-harness/cch-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@coco-harness/cch-tool-pty` | `@coco-harness/cch-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@coco-harness/cch-lsp-local` | `@coco-harness/cch-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@coco-harness/cch-tasks`, `ctx.tasks`, `TaskService` | `@coco-harness/cch-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@coco-harness/cch-tasks-local`, `LocalTaskService` | `@coco-harness/cch-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@coco-harness/cch-tool-tasks` | `@coco-harness/cch-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@coco-harness/cch-client-ui-task`, `client/ui-task/` | `@coco-harness/cch-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@coco-harness/cch-client-ui-slash`, `ui-slash/` | `@coco-harness/cch-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@coco-harness/cch-agent-tool-mode`, plugin `tool-mode` | `@coco-harness/cch-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@coco-harness/cch-permission`, `ctx.permission`, `PermissionService` | `@coco-harness/cch-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@coco-harness/cch-client-ui-permission` | `@coco-harness/cch-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@coco-harness/cch-user-interaction`, `user-interaction/` | `@coco-harness/cch-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@coco-harness/cch-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@coco-harness/cch-type-meta` | `typert/protocol/`, `@coco-harness/cch-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@coco-harness/cch-tools` and `ctx.tools`. Keep `@coco-harness/cch-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@coco-harness/cch-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@coco-harness/cch-workspace-context`, `context/workspace-context/` | `@coco-harness/cch-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `TelemetryOtel`, `TelemetryMode`, plugin `telemetry-otel` | `OpenTelemetrySessionBackend`, `SessionTelemetryMode`, plugin `session-telemetry-otel` | The provider name states both the OpenTelemetry mechanism and session scope. Keep the package names `cch-session-telemetry` and `cch-session-telemetry-otel`. |
...
| `session/user-id/`, `@coco-harness/cch-user-id` | `identity/anonymous-user-id/`, `@coco-harness/cch-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@coco-harness/cch-environment` | `util/launch-environment/`, `@coco-harness/cch-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
| Public `Environment*`, `createEnvironmentSnapshot`, `environmentOf`, `CCH_ENVIRONMENT_KEY` | `LaunchEnvironment*`, `createLaunchEnvironmentSnapshot`, `launchEnvironmentOf`, `CCH_LAUNCH_ENVIRONMENT_KEY` | The names state the snapshot's lifetime and purpose. |
...
| Former | Current | Reason |
|---|---|---|
| `@coco-harness/cch-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@coco-harness/cch-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@coco-harness/cch-workflow` and workflow events and tools. |
| `@coco-harness/cch-workflow-workerthread`, `WorkerWorkflowEngine` | `@coco-harness/cch-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@coco-harness/cch-goal-session`, `goal/goal-session/` | `@coco-harness/cch-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@coco-harness/cch-compact`, `ctx.compact`, `CompactService` | `@coco-harness/cch-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@coco-harness/cch-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@coco-harness/cch-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@coco-harness/cch-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@coco-harness/cch-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@coco-harness/cch-settings-local`, `SettingsLocal` | `@coco-harness/cch-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@coco-harness/cch-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@coco-harness/cch-session-title-first-message-llm`, cadence `first-message` | `@coco-harness/cch-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@coco-harness/cch-session-title-all-messages-llm`, cadence `all-user-messages` | `@coco-harness/cch-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@coco-harness/cch-code-runtime-worker`, `WorkerCodeRuntime` | `@coco-harness/cch-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@coco-harness/cch-fs-policy` | `@coco-harness/cch-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@coco-harness/cch-skill-local`, `LocalSkillProvider`, provider id `local` | `@coco-harness/cch-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@coco-harness/cch-subagent-spawn`, `SpawnProvider` | `@coco-harness/cch-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@coco-harness/cch-subagent-fork`, `ForkProvider` | `@coco-harness/cch-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@coco-harness/cch-subagent-inprocess`, `subagent-inprocess/` | `@coco-harness/cch-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
| Private `SdkProvider` in `cch-subagent-cch-sdk` | `SdkSubagentProvider` | The repeated package qualifier is intentional, and the class must say that it provides subagents through the SDK. |
...
| `@coco-harness/cch-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@coco-harness/cch-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@coco-harness/cch-subagent-cch-sdk`, its provider id `cch-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@coco-harness/cch-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@coco-harness/cch-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@coco-harness/cch-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@coco-harness/cch-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@coco-harness/cch-timeout-policy` | `@coco-harness/cch-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@coco-harness/cch-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@coco-harness/cch-paths` | `util/home-paths/`, `@coco-harness/cch-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@coco-harness/cch-retention` | `util/output-retention/`, `@coco-harness/cch-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@coco-harness/cch-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@coco-harness/cch-frontend-static` | `@coco-harness/cch-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@coco-harness/cch-jsonrpc-demo` | `@coco-harness/cch-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@coco-harness/cch-frontend` | `@coco-harness/cch-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |

Keep atomic-write, brand, native-command, timeout utility, directory-picker, `cch-base`, `cch-web-app`, app boot, CLI names, and the `headless` package, bundle, and example identity. `headless` is the intended product essence and may later support more than one-shot execution.
...
| `@coco-harness/cch-client-ui-model` | `@coco-harness/cch-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@coco-harness/cch-client-ui-models` | `@coco-harness/cch-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@coco-harness/cch-client-ui-plugin-config`, `client/ui-plugin-config/` | `@coco-harness/cch-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@coco-harness/cch-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@coco-harness/cch-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@coco-harness/cch-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@coco-harness/cch-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@coco-harness/cch-client-ui-question` | `@coco-harness/cch-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@coco-harness/cch-client-ui-command`, `ui-command/` | `@coco-harness/cch-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@coco-harness/cch-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@coco-harness/cch-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@coco-harness/cch-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@coco-harness/cch-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `cch-subagent-cch-sdk` and provider id `cch-sdk`; the repeated qualifier prevents ambiguity.
...
- Keep `@coco-harness/cch-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@coco-harness/cch-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
...
**Rename `cch-compact-basic` to `cch-compaction-llm`.** Rejected. `LLM` adds no distinction in the current backend family. `basic` is less ambitious and does not claim an algorithm that does not exist.
...
- The paired package-creation guide contains the role-word contract, `packages/AGENTS.md` links to it, the terminology table records the chosen words and `Typert` spelling, and root project prose calls the product Coco Harness rather than Coco Harness SDK.
