...
A `cch` launch whose config failed validation printed its diagnostic and returned the user to a broken shell. Typing was invisible, and the next command was mangled by stray text:

```
cch: fatal load failure: ValidationError: invalid config:
...
`cch`'s TUI launcher passes a release that disposes the root context, which runs the TUI's existing `shutdown()` and hands the terminal back.
...
A failed boot now costs one tree disposal (bounded at 2s) before exit, and the exit code stays 1. In exchange, a misconfigured `cch` returns a usable shell instead of one needing `stty sane` or `reset`.
