...
> The full seam is shipped: the `cch-subagent` interface and `cch-tool-subagent` consumer; the two in-process backends (`cch-subagent-spawn-in-process`, `cch-subagent-fork-in-process`); the nested-agent snapshot infrastructure ([per-session snapshot replay](../testing/2026-06-22-subagent-snapshot-replay.md)); and the out-of-process ACP, Codex, and Claude Code backends ([ACP Agent Note](2026-06-22-acp-subagent-backend.md), [product-provider Agent Note](2026-08-04-claude-code-and-codex-subagent-backends.md)).
...
| Package | Role |
|---|---|
| `@coco-harness/cch-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@coco-harness/cch-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@coco-harness/cch-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@coco-harness/cch-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@coco-harness/cch-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@coco-harness/cch-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@coco-harness/cch-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
...
`cch-tool-subagent` passes its execution signal to `start()`, awaits the child result, and disposes the run before reporting. Non-completed outcomes become error results rather than successful partial output, and independent result and disposal rejections retain both diagnostics.

### Provider selection is config, not model-facing

`cch-tool-subagent` binds to exactly one provider name (`Config.provider`); the model sees only `{ description, prompt }`. To expose more than one transport, load the tool plugin more than once, each bound to a different provider and a distinct `toolName` (the tool registry rejects a duplicate name). The *service* holds the multi-provider registry; the *tool* picks one — the schema carries no provider/type parameter.
