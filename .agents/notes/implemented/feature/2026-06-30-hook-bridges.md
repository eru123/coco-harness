# Agent Note: cch-hooks-claude-code + cch-hooks-codex — the Claude Code / Codex hook bridges
...
Matcher semantics, exit-code handling, and merge precedence live in `cch-hook-protocol`; each bridge only parses config, builds dialect payloads, and maps outcomes. Per-file coverage includes config branches plus end-to-end mappings through a real loop, `cch-bash-local`, and shell scripts, while a real-Loader smoke guards the package export shape. Native plugins bypass the wire protocol and return typed decisions directly.
