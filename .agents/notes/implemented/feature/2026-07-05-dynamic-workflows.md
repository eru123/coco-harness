...
### The engine (cch-workflow-worker-thread): one worker thread per run
...
- **Background execution as the default** (CC's shape): deferred; foreground-synchronous matches `cch-tool-subagent`'s cut, and background semantics should be designed ONCE across shell/subagent/workflow rather than per-tool.
