...
Coco Harness uses the same primitive so project-specific review, plugin-authoring, and tool-usage guidance lives next to the workspace or the user's agent configuration instead of being hard-coded into the loop.

## Decision

`@coco-harness/cch-skill` is the pure provider registry (`ctx.skills`), `@coco-harness/cch-skill-filesystem` is the shipped local filesystem provider, and `@coco-harness/cch-tool-skill` owns the durable session catalog and model-facing loader tool. `cch-agent-spine-demo` loads the registry, local provider, and consumer by default so TUI, headless, and ACP apps get the same behavior while embedded or remote providers contribute skills without changing the registry or consumer. Its `skills` config forwards `registry`, `local`, and `tool` branches to those owners.

Dedicated packaged providers can contribute immutable skills without filesystem discovery. The shipped CLI declares `@coco-harness/cch-skill-badge` disabled by default; enabling its composition row contributes the official badge instructions through the same registry and consumer ([decision](2026-08-06-bundled-cch-badge-skill.md)).
...
The local provider scans cwd-sensitive project roots, custom roots, and user roots in first-wins rank order: project `.cch`, project `.agents`, `customSkillDirs`, user `.cch`, then user `.agents`. The user `.cch/skills` scan skips `.system` so a system-owned directory is not treated as normal user content. The local provider does not synthesize built-in system skills; configured bundled roots and dedicated providers supply additional skills.
