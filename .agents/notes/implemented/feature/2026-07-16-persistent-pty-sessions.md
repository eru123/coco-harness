...
|---|---|---|
| `cch-terminal` | `TerminalSessionService`, branded `TerminalSessionId`, backend registry, owner-scoped session contract, and result types | `ctx.terminals` |
| `cch-terminal-bash` | Persistent-shell backend over `ctx.subprocess.spawnTerminal()`: readiness, bounded terminal buffers, sandbox resolution, and owner-aware session lifecycle | registers a backend on `ctx.terminals` |
| `cch-tool-terminal` | Six model-facing tools, task-runtime integration for background sends, guidance, and UI render intents | registers on `ctx.tools` |

Readiness remains PTY-backend behavior, not a second public contract. The terminal-process provider supplies only substrate facts such as the foreground process group and whether it can prove that group is waiting on input; `cch-terminal-bash` combines those facts with prompt and silence evidence into the common send result.
...
A registered `shell` backend constrains how a terminal starts; it does not constrain commands typed after startup. `cch-terminal-bash` therefore applies two protections before spawning:
...
```yaml
plugins:
  '@coco-harness/cch-sandbox-local':
  '@coco-harness/cch-sandbox-policy':
    config:
      mode: workspace-write
      workspaceRoot: .
  '@coco-harness/cch-terminal':
  '@coco-harness/cch-subprocess-local':
  '@coco-harness/cch-terminal-bash':
    config:
...
**Publish `TerminalIdleDetector` as a replaceable registry.** Rejected. Substrate-specific foreground facts come from the mounted terminal-process primitive, while prompt/silence readiness remains one private policy in `cch-terminal-bash`. The filesystem/subprocess execution-world replacement is the necessary extension point.
...
**`node-pty` is a native dependency of `cch-subprocess-local`.** Installation, supported Node versions, prebuild availability, and platform behavior require built-artifact smokes on every supported OS.
