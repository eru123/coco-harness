...
The goal domain can retain an objective and the model-facing tools can mutate its lifecycle, but neither should decide when another model turn begins. A continuation driver must bridge active goal state to the ordinary agent loop without adding goal-specific branches to `cch-agent-loop`, inventing a second conversation, or treating every human turn as an autonomous iteration.
...
The plugin has no configuration. `maxGoalRounds` is resolved and persisted by `cch-goal`, and the same-condition blocking threshold is resolved and prompted by `cch-tool-goal`. Repeating those tunables in the driver would create multiple owners for one policy.
...
- **Add a goal loop inside `cch-agent-loop`** — rejected because the public queue, prompt, session, cancellation, and status contracts are sufficient, and a concrete-loop branch would privilege one policy.
