...
**Personal config (`cch-app-boot`).** The personal overlay lives in the Harness home — `$CCH_HOME`, else `~/.cch` — resolved by the shared [`resolveCchHome`](../architecture/2026-07-24-single-harness-home-resolver.md) (`@coco-harness/cch-home-paths`), the same single root skills and AGENTS.md resolve against. The cch TUI, Web, and headless surfaces consume its two optional files; the demo bins boot their committed trees verbatim:
...
- `cch-app-boot` depends on `js-yaml` and imports the include's `!!js` YAML dialect (`entryListSchema`) directly, and, like `apps/cli`, depends on `@coco-harness/cch-home-paths` for `resolveCchHome`.
