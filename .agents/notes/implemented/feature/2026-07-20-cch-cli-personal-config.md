# Agent Note: The cch CLI and personal config overlays from the Harness home
...
The entry modes and the personal file's name and location below are superseded by the [profile plugin bundles decision](../architecture/2026-08-05-profile-plugin-bundles.md): `cch` boots profiles, and the personal layer became the per-profile and home-level `cordis.patch.yml`. What survives unchanged is this note's substance — the Harness home as the machine-level layer's root, patch semantics over a shipped composition, and fail-loud parsing.

Two coupled pieces, aligned with the `apps/` assembly tier proposed by the `cch web` PR (#443):

**The `cch` CLI (`apps/cli`, npm name `@coco-harness/cch`).** `apps/*` is the product-assembly tier over `packages/*` libraries. One bin dispatches the default interactive TUI, `-p`/`--prompt` headless turns, and the `web` surface. The TUI boots `examples/tui-agent/cordis.yml` (or `--config`) with the invoking directory as the workspace. From a source checkout, the root `pnpm cch` script runs the same entry with tsx's ESM hook without building; the [source-launch decision](../architecture/2026-07-29-cch-source-launch-tsx-esm.md) owns the runtime vector and the [source-launch/build separation decision](../simplification/2026-08-12-separate-source-launch-from-build.md) owns artifact generation.

**Personal config (`cch-app-boot`).** The personal overlay lives in the Harness home — `$CCH_HOME`, else `~/.cch` — resolved by the shared [`resolveDshHome`](../architecture/2026-07-24-single-harness-home-resolver.md) (`@coco-harness/cch-home-paths`), the same single root skills and AGENTS.md resolve against. The cch TUI, Web, and headless surfaces consume its two optional files; the demo bins boot their committed trees verbatim:
...
The PTY smoke's launcher isolates `$CCH_HOME` to a per-test directory, exactly as it already isolates `CCH_AGENTS_HOME`, so a developer's real personal overlay cannot leak into fixtures; only the cch CLI reads personal config, so no other test launcher needed changes.
...
**A separate `bin/cch` wrapper owning the `cch` name.** Rejected because `apps/cli` is the single product CLI for default TUI, headless, and Web dispatch. Two competing entrypoints would collide in `$PATH` and product identity.
...
- `cch-app-boot` depends on `js-yaml` and imports the include's `!!js` YAML dialect (`entryListSchema`) directly, and, like `apps/cli`, depends on `@coco-harness/cch-home-paths` for `resolveDshHome`.
...
`packages/boot/app-boot/tests/user-patches.spec.ts` pins parsing, startup application, exact-path add/failure/recovery/removal, last-good rollback, failure broadcast, and preservation of app-owned patches. `apps/cli/tests/built-bin.e2e.ts` boots the real cch bin over a profile and exercises the live patch layer end to end. Test launchers isolate `$CCH_HOME`, so a developer's real overlay cannot leak into fixtures.
