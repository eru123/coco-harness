...
- A Windows composition using `cch-tool-pwsh` now shows its shell calls exactly as bash calls look in the Web UI: cwd-headed terminal card, raw output, exit-status pill, run-state dot, and the red failure treatment on non-zero exits.
- `parseExitStatus` becomes public contract surface on `@coco-harness/cch-shell`; `cch-tool-bash/src/render.ts` keeps re-exporting it, so no bash-tool consumer changes.
