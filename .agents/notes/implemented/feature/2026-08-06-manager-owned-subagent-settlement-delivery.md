...
- `foldConsumedWork()` replaces `cch-session`'s `findLastMessageTurnEnd()` and moves to `cch-agent`, which owns the inbox marker it reads; the one-shot in-process path folds the same answer and does not classify a cut-short one-shot child as `completed`.
