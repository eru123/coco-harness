...
- `cch-subagent` gains optional peer types on `cch-sandbox-policy` and `cch-user-approval` (the `ctx.get` pattern the one-shot driver used); `cch-subagent-in-process-driver` drops its policy-service peers and type imports entirely and delegates to the shared helpers.
...
- Out-of-process providers (`acp`, `cch-sdk`, `claude-code`, `codex`) support no continuable children (`prepareContinuable` absent), and their one-shot children keep their own deployment policy (`inheritsParentContext = false`); cross-process policy propagation remains out of scope.
