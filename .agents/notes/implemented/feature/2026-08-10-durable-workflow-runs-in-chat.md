...
The workflow package exposes browser-safe run and observation vocabulary through `@coco-harness/cch-workflow/types`; live `Agent` requests and control handles remain Host-only. `@coco-harness/cch-tool-workflow/types` owns the four Session events. Client code imports only these type faces, so the Host and Client TypeScript programs share the durable contract without merging Host Cordis context.
