...
- **One product onboarding path.** The root README owns the recommended package-run path, the source-run alternative, and compact `cch plugin --profile` usage. The published user guide starts with tasks inside the running Web UI, then links to distinct tutorials or reference owners for other interfaces, plugin development, and advanced configuration instead of repeating Web startup.
...
- **A thin workflow skill, contracts in docs.** [.agents/skills/cch-doc-standards](../../../skills/cch-doc-standards/SKILL.md) carries the placement/audit/red-gate workflow and defers to the standard as its source of truth, the same split as cch-translate-docs (`../../../skills/cch-translate-docs/SKILL.md`, since removed) over the i18n contract.
