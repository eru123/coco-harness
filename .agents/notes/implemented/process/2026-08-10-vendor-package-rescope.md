# Agent Note: Rescope vendored Cordis into @coco-harness

Status: implemented

## Problem

The nine packages under `vendor/` kept their upstream npm names (`cordis`, `cosmokit`, `schemastery`, `@cordisjs/plugin-*`). That premise does not survive publication: every harness package declares `cordis` as a peer dependency, so a consumer installing `@coco-harness/cch-*` must resolve it from the registry, which means publishing the harness publishes this framework layer too. Publishing it under the upstream names squats them on the registry, and where that registry proxies npmjs, the same-name entries shadow the real upstream packages and install the wrong framework into unrelated projects.

## Decision

All nine packages move into the `@coco-harness` scope. Directory names, upstream version numbers, and dependency ranges stay untouched, so the `vendor/README.md` manifest still reads as an upstream snapshot. [docs/rescope.md](../../../../docs/rescope.md) restates this mapping for consumers.

| Directory | npm name | Upstream name |
|---|---|---|
| `cordis/` | `@coco-harness/cordis` | `cordis` |
| `cosmokit/` | `@coco-harness/cosmokit` | `cosmokit` |
| `schemastery/` | `@coco-harness/schemastery` | `schemastery` |
| `loader/` | `@coco-harness/cordis-plugin-loader` | `@cordisjs/plugin-loader` |
| `include/` | `@coco-harness/cordis-plugin-include` | `@cordisjs/plugin-include` |
| `group/` | `@coco-harness/cordis-plugin-group` | `@cordisjs/plugin-group` |
| `timer/` | `@coco-harness/cordis-plugin-timer` | `@cordisjs/plugin-timer` |
| `hmr/` | `@coco-harness/cordis-plugin-hmr` | `@cordisjs/plugin-hmr` |
| `logger-console/` | `@coco-harness/cordis-plugin-logger-console` | `@cordisjs/plugin-logger-console` |

The rewrite touches only **delimited, complete package-name tokens**: quoted or backticked specifiers (optionally with a `/subpath`), `package.json` names and dependency keys, `cordis.yml` `name:` values, and `tsconfig.base.json` `paths` keys. Identically spelled strings that are not package names therefore stayed as they were: the `cordis.yml` config-file family, the Loader's literal `cordis:` builtin prefix (`cordis:include`, `cordis:group` — see `vendor/loader/src/config/tree.ts`), kind strings like `cordis-config-entry`, `@coco-harness/cch-tool-cordis`, Schemastery's upstream `Symbol.for('schemastery')` and `vendor:` metadata field, the `packages/<group>/` directory names in `GROUP_ORDER` (`scripts/gen-module-graph.ts`, `scripts/gen-doc-graphs.ts`), and the upstream install instructions in `vendor/*/README.md`.
...
- No upstream name remains in the publication set. `publish-npm-baseline.ts` now requires every published package to be `@coco-harness/*` with no vendored exemption, so regressing the rename fails before packing.
...
- `pnpm-workspace.yaml` drops the `cordis` and `@cordisjs/plugin-loader` `minimumReleaseAgeExclude` entries, which can no longer be fetched from a registry, and `knip.json` drops the `@cordisjs/.+` ignore pattern that `@coco-harness/.+` already covers.
...
**Keep the upstream names and exclude `vendor/` from publication.** Rejected because every harness package declares `cordis` as a peer dependency, so an installed `@coco-harness/cch-*` would have no resolvable framework.
