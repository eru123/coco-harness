...
`SubagentRuntime` is the only public service. It exposes ordinary `start(name, request)`, Task-backed `startContinuable(spec)`, and intent-named `followup(...)`; provider resume dispatch remains private to its continuation manager. The standalone `@coco-harness/cch-subagent-control` package and `ctx.subagentControl` key are absent; the optional `@coco-harness/cch-tool-subagent-control` package injects `ctx.subagents` directly.
...
- The continuation manager is still Task- and persistence-aware inside the `cch-subagent` package, so the package declares optional peer dependencies on those services even though ordinary `start` callers do not need them.
