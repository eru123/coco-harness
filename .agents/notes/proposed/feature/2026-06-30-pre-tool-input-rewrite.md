...
3. **Human-facing presentation reads `tool/call.arguments`**: UI renderers pass them to `presentResult`; `cch-tool-bash` derives the card title, the rawInput, the cwd, and the terminal-vs-background treatment from them.
