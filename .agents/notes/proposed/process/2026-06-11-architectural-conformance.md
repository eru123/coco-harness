...
- `packages/*` (except agent-loop's own tests and examples/) must not import `@coco-harness/cch-agent-loop`.
- No cross-package deep imports (`@coco-harness/cch-*/src/...` paths) — public entry points only.
- No import cycles anywhere in packages/.
- `vendor/*` must not import from `packages/*`.
- Layering: cch-llm imports nothing from other cch packages; cch-session only cch-llm; etc. (the dependency table in packages/README.md, enforced).

**Adapter conformance kit** in cch-llm (`@coco-harness/cch-llm/conformance`): a reusable vitest suite parameterized by an adapter factory, asserting the chunk-protocol contract — index monotonicity per block, no deltas after `block-end` for an index, exactly one `finish`, usage at most once, every `tool-call-delta` carries the call id, abort honored promptly. Run it against the mocks now; the DeepSeek V4 adapter inherits it on day one. Optionally a dev-mode `strictAdapter()` wrapper enforcing the same at runtime behind a debug flag (pairs with [the dev-mode invariants](../../implemented/architecture/2026-06-11-dev-invariants-over-deep-readonly.md)).
...
Dep-cruiser rule maintenance as packages are added — keep rules pattern-based (`cch-*`) rather than enumerated.
