# Agent Note: Fold the persistence interface into cch-session

Status: rejected — the separate persistence Service Definition package is the intended modular role split for the durable-persistence capability seam. Folding it into `cch-session` would reduce package count at the cost of a cleaner backend boundary.

## Problem

`cch-session-persistence` is a Service Definition package whose main concepts are already owned by `cch-session`: `SessionHeader`, `SessionEvent`, `SessionId`, `session/event`, and `session/flush`. The package adds the abstract `SessionPersistence` service, the shared write coordinator, and contract helpers. Provider packages depend on it, and `agent-loop` has to optionally find a sibling service for resume.
...
- `@coco-harness/cch-session-persistence` is removed as a package.
- `cch-session` exports the persistence service type, coordinator, and contract helpers.
- JSONL and SQLite backend packages depend on `cch-session` directly.
