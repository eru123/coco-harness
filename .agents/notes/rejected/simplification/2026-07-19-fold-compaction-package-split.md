...
Compaction is split between `@coco-harness/cch-compaction`, which owns an abstract two-method service and shared types, and `@coco-harness/cch-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@coco-harness/cch-compaction` and remove `@coco-harness/cch-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@coco-harness/cch-compaction-basic` and its workspace/package metadata are removed.
- `@coco-harness/cch-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
