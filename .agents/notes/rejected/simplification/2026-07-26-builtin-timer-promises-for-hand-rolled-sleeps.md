...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`cch-llm-mock-server` `pause()`, `cch-lsp-stdio`, `cch-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
