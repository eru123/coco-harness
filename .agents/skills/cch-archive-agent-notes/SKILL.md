...
2. Make no body edits. Insert only `Archived: YYYY-MM-DD` immediately below `Status: implemented`, using the archival date. Do not reformat, update facts, or repair links inside the note.
...
Run the archive verifier's focused test, `pnpm run verify-archived-agent-notes`, `pnpm run doc-sync`, `pnpm run lint`, and `git diff --check`; select any additional evidence through [cch-pre-push-checks](../cch-pre-push-checks/SKILL.md).
