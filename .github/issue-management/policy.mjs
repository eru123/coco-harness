...
const BODY_LIMIT = 50
const AUDIT_MARKER = '<!-- cch-issue-policy -->'
...
export function validateBody({
  ...
  if (!count.balanced) errors.push('details tags must be closed in balanced pairs')
  if (count.detailsCount === 0) errors.push('body must contain a default-collapsed <details> region')
  if (!count.allCollapsed) errors.push('details must be collapsed by default; open is not allowed')
  if (count.units > BODY_LIMIT) {
    errors.push(`visible body is ${count.units} units, exceeding the 50-unit limit`)
  }
  if (normalized.length >= 2 && !owner) {
    ...
    errors.push('with zero or one assignee there must be no Owner line')
  }
  return errors
}
...
export function validateIssue(issue) {
  ...
  if (!/\p{Script=Latin}/u.test(issue.title)) errors.push('Issue title must contain English')
  ...
  if (
    ...
    errors.push('Issue title must not carry Type, Priority, Status, area, or Owner prefixes')
  }
  if (!TYPES.has(issue.type ?? '')) errors.push('Type must be one of the five native English Types')
  if (!status || !config.statuses.includes(status)) {
    errors.push('Issue must be in the Project with a valid Status')
  }
...
export function validatePullRequest(input) {
  ...
  if (input.references.all.length === 0) {
    errors.push('PR body must reference at least one Issue in this repository')
  }
  if (kinds.length !== 1) {
    errors.push(`PR must have exactly one allowed kind/*, found ${kinds.length}`)
  }
  if (unknownKinds.length > 0) {
    errors.push(`PR has unsupported kind/*: ${unknownKinds.join(', ')}`)
  }
  if (legacyLabels.length > 0) errors.push(`PR has legacy labels: ${legacyLabels.join(', ')}`)
  if (sourceLabels.length > 0) errors.push(`source/* is for Issues only: ${sourceLabels.join(', ')}`)
  if (priorities.length > 1) {
    errors.push(`PR allows at most one p0–p3, found ${priorities.length}`)
  }
  ...
  if (priorities.length === 0 && issuePriorities.length > 0) {
    ...
    errors.push('a resolving PR with Priority requires every resolved Issue to have a Priority')
...
async function projectContext(number, includeStatusActor = false) {
  ...
  if (!project || project.title !== config.projectTitle) {
    throw new Error('target Project not found or title mismatch')
  }
  if (!issue) throw new Error(`#${number} not found`)
  const statusField = project.fields.nodes.find((field) => field?.name === 'Status')
  if (!statusField) throw new Error('Project lacks the Status field')
...
async function updateStatus(context, status) {
  ...
  if (!option) throw new Error(`Status not found: ${status}`)
...
async function runPullRequestCheck(event) {
  ...
  process.stdout.write(
    requiresPullRequestPolicy(pull)
      ? 'Issue policy passed.\n'
      : 'PR is not yet in the mandatory Issue policy scope.\n',
  )
}
