...
const withDetails = (summary) =>
  `${summary}\n\n<details><summary>Acceptance and details</summary>To be added.</details>`

const legalIssue = {
  title: 'Verify the issue management policy',
  body: withDetails('Verify the issue management policy.'),
  assignees: [],
  labels: [],
  type: 'Idea',
  priority: null,
...
test('counts only text outside details', () => {
  assert.deepEqual(countVisibleUnits('Uses GitHub Projects.<details>hidden text</details>'), {
    units: 3,
    balanced: true,
    detailsCount: 1,
    allCollapsed: true,
...
test('requires a balanced default-collapsed details region', () => {
  assert.deepEqual(validateBody({ body: 'Finish the work.', assignees: [] }), [
    'body must contain a default-collapsed <details> region',
  ])
  assert.deepEqual(
    validateBody({
      body: 'Finish the work.\n\n<details open><summary>Details</summary>To be added.</details>',
      assignees: [],
    }),
    ['details must be collapsed by default; open is not allowed'],
  )
  assert.deepEqual(
    validateBody({ body: 'Finish the work.\n\n<details><summary>Details</summary>', assignees: [] }),
    ['details tags must be closed in balanced pairs'],
  )
})
...
test('requires Owner for multiple assignees', () => {
  assert.deepEqual(
    validateBody({
      body: withDetails('Finish the work.'),
...
test('rejects metadata prefixes in an Issue title', () => {
  const errors = validateIssue({ ...legalIssue, title: '[Bug] Fix the resume failure' })
  assert.ok(errors.includes('Issue title must not carry Type, Priority, Status, area, or Owner prefixes'))
})
...
test('separates resolving and informational references', () => {
  assert.deepEqual(
    parseReferences({
      body: 'Fixes #12\nRelated to #4\nRefs cocoharness/cch-test#7',
      repository: 'cocoharness/cch-test',
    }),
...
test('requires repository PR labels in the enforcement scope', () => {
  ...
  assert.ok(errors.includes('PR must have exactly one allowed kind/*, found 0'))
...
test('allows missing Priority only when resolving Issues are also unprioritized', () => {
  ...
  assert.ok(
    validatePullRequest({ ...pull, labels: [...pull.labels, 'p2'] }).includes(
      'a resolving PR with Priority requires every resolved Issue to have a Priority',
    ),
  )
})
