/**
 * Commander adapter for the `cch` command line.
 *
 ...
 * `@coco-harness/cch-cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `cch --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `cch --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @coco-harness/cch/args
 */
...
const HELP_EXAMPLES = `
Examples:
  cch --profile web                          boot the web profile (same as: cch web)
  cch --profile headless "run the tests"     answer one task, print the result, and exit
  cch --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  cch --profile tui --resume <session>       arguments after the launcher flags reach the app
  cch --profile web --help                   the web app's own flags and help
  cch plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): CchInvocation {
...
export function parseDshArgs(argv: readonly string[], version: string): CchInvocation {
  ...
  program
    ...
    .description('cch: boot a Coco Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `cch -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: cch --profile <name> --help)')
    .option('--profile <name>', 'the profile under $CCH_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `cch -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: cch web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
