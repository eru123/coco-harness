/**
 * `cch plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `cch.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `cch.bundle` joins the layer stack; a
 ...
 * gained its `cch.bundle` declaration in a newer version.
 * @module @coco-harness/cch/plugin
 */
 ...
 * @returns true when the package manifest declares `cch.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.cch?.bundle?.patch !== undefined
}

/**
 * Reconcile `cch.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `cch.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.cch?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no cch.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (cch-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.cch = { ...after.cch, profile: { ...after.cch?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `cch` was invoked from.
...
/**
 * Run one `cch plugin` invocation: init if needed, forward to pnpm, reconcile.
