/**
 * Shared profile boot for every `cch` surface: resolve the profile, stack its
 * patch layers (bundle layers in `cch.profile.bundles` order, the profile's
 ...
 * @module @coco-harness/cch/profile-boot
 */

import { writeFileSync } from 'node:fs'
import { join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { FiberState, type Context } from '@coco-harness/cordis'
...
import { CCH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@coco-harness/cch-launch-environment'
...
/**
 * The home-level user patch layer (`$CCH_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$CCH_HOME` may be set by the test or launcher after import.
...
/** The session-telemetry row id the CCH_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# cch profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's cch.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `CCH_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$CCH_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `cch.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$CCH_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  // The writable root the roster appends is `cch-agent-presets`' own, so a
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.CCH_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(CCH_LAUNCH_ENVIRONMENT_KEY, options.environment)
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@coco-harness/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@coco-harness/cordis-plugin-hmr', config: { root: [] } })
      }
