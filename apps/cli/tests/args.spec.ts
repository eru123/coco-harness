...
import { parseCchArgs } from '../src/args.ts'
...
describe('parseCchArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@coco-harness/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@coco-harness/cordis'] })
