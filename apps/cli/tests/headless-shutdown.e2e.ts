...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@coco-harness/cch-loader-smoke'

const cchBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"cch-test: never-dispose ready", b"cch-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'cch-headless-shutdown-'))
  try {
    const home = join(cwd, '.cch')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'cch-profile-headless',
      private: true,
      dependencies: {},
      cch: { profile: { bundles: ['@coco-harness/cch-base', '@coco-harness/cch-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        CCH_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`cch headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`cch headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('cch: observing at ')
    expect(output).toContain('cch-test: never-dispose ready')
    expect(output).toContain('cch-test: never-dispose started')
