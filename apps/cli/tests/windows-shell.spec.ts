 ...
 * the REAL shipped bundle layers (cch-base + cch-web-app resolved from the
...
import { entryListSchema } from '@coco-harness/cordis-plugin-include'
import { evaluate } from '@coco-harness/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@coco-harness/cch-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL cch-base/cch-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'cch-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@coco-harness/cch-base', '@coco-harness/cch-web-app'])
    const profile = loadProfile('cch', 'web', anchor, home)
    ...
    for (const name of ['@coco-harness/cch-pwsh-sandbox', '@coco-harness/cch-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'cch-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@coco-harness/cch-base'])
    const profile = loadProfile('cch', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
