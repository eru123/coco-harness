...
it('keeps the browser responsive while rendering 100,000 reasoning chunks', async () => {
  ...
  try {
    ...
    browser = await chromium.launch({ headless: process.env.CCH_WEB_STRESS_HEADFUL !== '1' })
    page = await newEnglishPage(browser)
    const activePage = page
    await activePage.addInitScript(() => {
      localStorage.setItem('cch.sessions.current', JSON.stringify({ sessionId: 'fx-alpha' }))
    })
