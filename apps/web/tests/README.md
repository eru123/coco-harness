...
the deliberate composition divergences from `cch web` — are documented in
...
## Do not import `@coco-harness/cch-client-*` here
...
`@coco-harness/cch-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@coco-harness/cch-client-web` and the boot-manifest
type from `@coco-harness/cch-client-modules/client`: booting the real shell is what
...
`@coco-harness/cch-client-runtime/client` because `client/runtime` is reachable
