...
describe('web e2e: /goal human transcript presentation', () => {
  ...
  it('shows the bare input and result from a fresh session without a model turn', async () => {
    ...
    await expect.poll(() => page.getByText("Nothing is baked in. That's the point.", { exact: false }).count(), {
      timeout: 15_000,
    }).toBe(1)
    ...
    expect(await page.getByText("Nothing is baked in. That's the point.", { exact: false }).count()).toBe(0)
