...
const PROMPTS = [
  ...
  'Does the CCH file sandbox currently restrict file operations? Answer directly in one sentence. Do not call tools.',
...
describe('web e2e: current sandbox policy reaches the model before tools', () => {
  ...
  it.skipIf(MODE === 'record')('records cache-safe current policy before the corresponding model behavior', async () => {
    ...
    expect(systems[0]).not.toContain('Current CCH file policy:')
    ...
    expect(contexts[0]).toContain('Current CCH file policy: read-only. Any available operation enforced by the CCH file sandbox cannot modify files in the standing mode.')
    ...
    expect(contexts[1]).toContain('Current CCH file policy: danger-full-access. The CCH file sandbox does not restrict file modifications by available operations.')
    ...
    expect(contexts[2]).toContain(`Current CCH file policy: workspace-write. Any available operation enforced by the CCH file sandbox may modify files under the session workspace: ${JSON.stringify(canonicalPath(sessionWorkspace))}. Some platform temporary areas may also be writable.`)
    ...
    expect(contexts[3]).toContain('Current CCH file policy: read-only.')
