...
it('isolates replay skill discovery from every ambient host root', async () => {
  const ambient = await mkdtemp(join(tmpdir(), 'cch-web-ambient-skills-'))
  const cchHome = join(ambient, 'cch-home')
  const agentsHome = join(ambient, 'agents-home')
  const bundled = join(ambient, 'bundled')
  await Promise.all([
    writeSkill(join(cchHome, 'skills'), 'ambient-cch'),
  ...
  process.env.CCH_AGENTS_HOME = agentsHome
  ...
  try {
    ...
    try {
      ...
      expect(names).not.toContain('ambient-cch')
    ...
    try {
      ...
      else process.env.CCH_BUNDLED_SKILL_DIR = originalBundled
