...
// Boots the REAL web composition — the cch-base and cch-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $CCH_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts cch-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `cch web`, all deliberate, all via include
...
import Include, { type PatchOptions } from '@coco-harness/cordis-plugin-include'
import Group from '@coco-harness/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@coco-harness/cch-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
...
import { cchHomePath } from '@coco-harness/cch-home-paths'
...
import type { ReplayHandle } from '@coco-harness/cch-llm-replay'
import { installLlmReplay, parseSessionLog } from '@coco-harness/cch-llm-replay'
import SessionStore, {
  packChunkRuns,
...
import type {} from '@coco-harness/cch-host-webserver'
import type {} from '@coco-harness/cch-agent'
import { provideCmdline } from '@coco-harness/cch-cmdline'
...
// } from '@coco-harness/cch-client-ui-settings-models'
...
/** Snapshot mode for the lane, from $CCH_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.CCH_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`CCH_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the cch-base and cch-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($CCH_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted cch-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'cch-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $CCH_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.cch document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.cch-home')
  ...
  // session is created. Without this a developer's real ~/.cch/skills silently
  // enters replay requests and goldens while CI sees none. `CCH_HOME` follows
  ...
  const skillRootEnvironment = {
    CCH_HOME: harnessHome,
    CCH_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    CCH_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'cch-web-e2e-sessions-'))
  } catch (error) {
  ...
  // (bundle patches in cch.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.cch/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $CCH_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.cch-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.cch, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        cchHome: join(workspaceCwd, '.cch-home'),
    ...
    // to the production OTLP endpoint (or whatever CCH_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @coco-harness/cch-web-frontend); only the URL line is silenced.
    ...
    { id: 'settings', config: { cchHome: harnessHome } },
    { id: 'credentials', config: { cchHome: harnessHome } },
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@coco-harness/cch-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@coco-harness/cch-client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@coco-harness/cch-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    ctx.provide('cchHomePath', cchHomePath)
    ...
    // `@coco-harness/cordis-plugin-group` by name.
...
function normalizeAria(snapshot: string, workspaceCwd: string): string {
  ...
  return snapshot
    ...
    .replace(/\d{4}-\d{1,2}-\d{1,2} \d{2}:\d{2}/g, '{{clock}}')
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run CCH_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
