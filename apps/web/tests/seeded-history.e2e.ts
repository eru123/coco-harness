...
// fixture under the same record discipline as every other: CCH_SNAPSHOT=record drives the turn
...
import type { ContentBlock, Message } from '@coco-harness/cch-llm'
import { deriveEventMessage, SessionId } from '@coco-harness/cch-session'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    process.env.CCH_HOME = scaffold.harnessHome
    try {
      const input = page.locator('textarea').first()
