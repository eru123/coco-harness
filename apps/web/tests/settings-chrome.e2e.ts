...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    const readState = async (target: Page = page): Promise<ThemeState> => await target.evaluate(() => {
      ...
      return {
        ...
        legacy: localStorage.getItem('cch.theme'),
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    expect(await page.evaluate(() => localStorage.getItem('cch.conversation.busyEnter'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
    ...
    try {
      ...
      expect(await secondPage.evaluate(() => localStorage.getItem('cch.conversation.busyEnter'))).toBeNull()
      expect(secondTripwire.pageErrors).toEqual([])
      expect(secondTripwire.warnings).toEqual([])
    } finally {
      await secondPage.close()
  ...
  it('offers English as the only settings language across reload and a distinct port', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-settings-language'))
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: 'Settings' })
    await dialog.waitFor({ timeout: 10_000 })
    // The Language selector pill shows the active locale's own name, and the
    // menu offers English alone: it is the only locale this deployment ships,
    // so no preference is ever stored for it.
    ...
    expect(await page.getByRole('menuitem', { name: 'English', exact: true }).count()).toBe(1)
    await page.keyboard.press('Escape')
    expect(await page.evaluate(() => localStorage.getItem('cch.locale'))).toBeNull()

    // Reload keeps English with no stored preference.
    const warningStart = tripwire.warnings.length
    await page.reload({ waitUntil: 'load' })
    await page.waitForSelector('[class*="frame"]', { timeout: 30_000 })
    acknowledgeReloadConnectionLoss(tripwire, warningStart)
    ...
    // A second live Host on another port sharing the same settings home also
    // stays on the English default.
    const second = await launchWebScaffold({ harnessHome: scaffold.harnessHome })
    const secondPage = await browser.newPage({ viewport: { width: 1680, height: 1000 }, locale: 'en-US' })
    const secondTripwire = watchConsole(secondPage)
    try {
      ...
      expect(await secondPage.evaluate(() => localStorage.getItem('cch.locale'))).toBeNull()
      expect(secondTripwire.pageErrors).toEqual([])
      expect(secondTripwire.warnings).toEqual([])
    } finally {
      await secondPage.close()
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    try {
      ...
      expect(await enPage.evaluate(() => localStorage.getItem('cch.locale'))).toBeNull()
