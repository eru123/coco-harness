...
interface ListMetrics {
  ...
  hoverRules: string[]
  /** `--cch-scrollbar-thumb` resolved on the list, serialized as a colour. */
  token: string
  /** `--cch-scrollbar-thumb-hover` resolved on the list, serialized the same way. */
  hoverToken: string
...
function measureList(page: Page): Promise<ListMetrics> {
  return page.evaluate(() => {
    ...
    return {
      ...
      hoverRules,
      token: resolve('--cch-scrollbar-thumb'),
      hoverToken: resolve('--cch-scrollbar-thumb-hover'),
...
interface PaletteMetrics {
  ...
  hovered: ListMetrics
  /** `--cch-scrollbar-thumb` with the pointer parked outside the column. */
  quietThumb: string
}
...
function renderGeometry(light: PaletteMetrics, dark: PaletteMetrics): string {
  const palette = (name: string, { hovered: metrics, quietThumb }: PaletteMetrics): string[] => [
    `## ${name}`,
    '',
    `- --cch-scrollbar-thumb, pointer outside the sidebar: ${quietThumb}`,
    ...
    `- --cch-scrollbar-thumb, pointer over the list: ${metrics.token}`,
    `- --cch-scrollbar-thumb-hover, pointer over the list: ${metrics.hoverToken}`,
...
/**
 * Resolve `--cch-scrollbar-thumb` as the list sees it, without the rest of the
...
function resolveThumb(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    if (list === null) throw new Error('sidebar session list not in the DOM')
    const probe = document.createElement('span')
    probe.style.color = 'var(--cch-scrollbar-thumb)'
    list.append(probe)
    const value = getComputedStyle(probe).color
    probe.remove()
    return value
...
describe('web e2e: sidebar session list scrollbar (reserved gutter / themed thumb)', () => {
  ...
  it('renders the themed thumb through the WebKit path in both palettes', async () => {
    ...
    expect(light.hoverRules).toEqual(['var(--cch-scrollbar-thumb-hover)'])
