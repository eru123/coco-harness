...
[Cordis](cordis-primer.md) is the framework under cch: plugins contribute services, typed events, and reversible effects to a shared context. Every part of the product is a plugin, including the model adapter, the tool registry, the session log, and the agent loop itself, so every part is replaceable from configuration.

There is no privileged core to patch: you extend cch by mounting a plugin beside the others, and registrations are effects that unwind when their plugin unloads.

## Profiles and bundles

A running `cch` is a plugin tree composed at boot from ordered layers.
...
Each declares itself in its own `package.json` under a `cch` field: `cch.profile` lists a profile's bundles, and `cch.bundle` points at a bundle's patch file.

[`cch-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`cch-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`cch-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
...
| Add persistent terminal execution | register a `ctx.terminals` backend plus `cch-tool-terminal` |
