...
The file-by-file checklist for a new `@coco-harness/cch-<name>` package. This checklist is validated against the bash and adapter packages as templates; if it drifts from them, fix it here.
...
packages/<group>/<pkg>/
  ...
  tsconfig.json    # extends ../../../tsconfig.base.json, rootDir src,
                   ...
                   # you use Config, + ../../<group>/<dep> for each cch dep)
...
| File | Change |
|---|---|
| `tsconfig.base.json` | no edit for an existing group; for a new group, add a `./packages/<group>/*/src` candidate to the `@coco-harness/cch-*` wildcard |
...
A `packages/client/*` package additionally extends `tsconfig.base.client.json` instead of `tsconfig.base.json`, and a client plugin package declares `cch.client` in package.json, exports `./client`, and calls the shared tsdown preset (`packages/client/tsdown.client.ts`) — see [packages/client/AGENTS.md](../../packages/client/AGENTS.md) for the client-side contract.
...
Use `SDK` only for the JSON-RPC client/server protocol used by the supported Python and TypeScript SDKs. Coco Harness itself is an agent harness, not an SDK project. Use the canonical product spelling `Typert`, never `TypeRT` or `typeRT`.
