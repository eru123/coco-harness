...
The neutral vocabulary lives in `cch-tools`; tools never import a UI or transport type. Host/client runtimes map each `card` into their own view. The design and the why are in [the render-intent-union Agent Note](../../.agents/notes/implemented/architecture/2026-07-02-tool-render-intent-union.md); `cch-tool-fs` (generic/diff) and `cch-tool-bash` (terminal) are the reference implementations.
