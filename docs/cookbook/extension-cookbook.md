...
import type { PreToolDecision, ToolExecution } from '@coco-harness/cch-tools'
...
Runnable leaves load their plugin trees from `examples/*/cordis.yml`; the root `demo:*` scripts and those leaf directories are the authoritative inventory. The product `cch` launcher owns Web and one-shot headless execution, ACP leaves use [`@coco-harness/cch-acp-demo`](../../packages/examples/acp-demo), and JSON-RPC leaves use [`@coco-harness/cch-sdk-jsonrpc-demo`](../../packages/examples/jsonrpc-demo). The headless snapshot leaf mounts [`@coco-harness/cch-agent-spine-demo`](../../packages/examples/agent-spine-demo) and JSONL persistence explicitly, then drives them through an example-owned test fixture rather than a shipped app package.
...
|---|---|
| Hook system (user + project level) | listeners on `agent/session-start`, `agent/pre-step`, `agent/request`, `tools/pre-execute`, `tools/post-execute`, and `agent/turn-stopping`; the waterfalls return typed decisions, while `agent/turn-stopping` may steer another step; the `cch-hooks-claude-code` / `cch-hooks-codex` bridges map hook config files onto these extension points |
| `/goal` | `ctx.goals` owns durable state, `cch-goal-round-driver` schedules same-session rounds through the public `Agent`, and separate command/tool producers expose human/model control |
...
| Context compaction (auto + manual) | the `ctx.compaction` seam + `cch-compaction-basic`; automatic pressure runs on serial `agent/pre-step`, canonical overflow recovery runs on `agent/request-error`, and manual callers use the same compact service ([compaction Agent Note](../../.agents/notes/implemented/feature/2026-06-18-compaction-capability-seam.md)) |
...
| Built-in tools | `ctx.tools.register()`; schemas flow into the assembly automatically — the `cch-tool-*` families (bash, fs, web, subagent, todo) are the shipped examples |
...
| Subprocess sandbox (landlock / sandbox-exec) | use a `ctx.sandbox` backend through `cch-bash-sandbox`; use `tools/pre-execute` for capability-level denial |
...
| Plan mode | [`@coco-harness/cch-plan-mode`](../../packages/plan/plan-mode/README.md) — logged `plan/mode` state, the `plan:policy` guidance section, `/plan [message]` entry, `/plan off` direct exit, and the user-reviewed `exit_plan_mode` exit; enforcement stays on the independent sandbox/approval axes |
| Sub-agent delegation | the `ctx.subagents` provider registry (`cch-subagent-spawn-in-process`/`-fork`/`-acp`/`-codex`/`-claude-code`/`-cch-sdk`) + `cch-tool-subagent` exposing one configured provider to the model |
...
| Model adapters | `LlmAdapter` subclass via `registerAdapter` (`cch-llm-deepseek`, `cch-llm-pi-ai`) |
