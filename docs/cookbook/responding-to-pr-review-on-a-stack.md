...
Review comments may target several PRs in a dependent stack (`A ← B ← C …`). Keep that chain linked through GitHub's official stacked-PR feature. This guide owns review-fix placement and propagation; the [cch-merging-stacked-prs](../../.agents/skills/cch-merging-stacked-prs/SKILL.md) skill owns linkage checks and landing.
...
3. Propagate the fixed layer through every affected child in order:
   ...
   - **Native cascading rebase:** use `gh stack rebase`, validate the rewritten layers, then publish with `gh stack push`; or use `gh stack sync`, which may publish first and therefore requires immediate post-sync validation under [cch-pre-push-checks](../../.agents/skills/cch-pre-push-checks/SKILL.md).
