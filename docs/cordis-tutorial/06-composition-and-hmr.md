...
Because unloading releases effects ([chapter 2](02-lifecycle-and-effects.md)) and loading follows dependencies ([chapter 3](03-services.md)), HMR can replace a running plugin by unloading and loading it. The `@coco-harness/cordis-plugin-hmr` plugin watches your files and does exactly that on save.
...
- id: timer
  name: '@coco-harness/cordis-plugin-timer'
- id: hmr
  name: '@coco-harness/cordis-plugin-hmr'
  config:
    root: ['.']
- id: hello
  name: './hello.ts'
...
Two support plugins joined the list: HMR logs through the Cordis logger service, so without a console exporter you would not see its messages, and it `inject`s the `timer` service for debouncing — without `@coco-harness/cordis-plugin-timer` it sits in PENDING forever, silently. That silence is the subject of the next section.
...
`inject: ['timer']` has no provider. Add `- name: '@coco-harness/cordis-plugin-timer'` to the list and the plugin loads. When a plugin does nothing and reports nothing, inspect its fiber state. Iterating without the PENDING filter also shows the loader's own plugins (Loader, Include) as ACTIVE fibers because plugins mount the config file itself.
