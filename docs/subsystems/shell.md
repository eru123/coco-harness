...
The bash execution seam is split across a Service Definition ([cch-shell](../../packages/shell/shell), `ctx.shell`), Service Providers ([cch-bash-local](../../packages/shell/bash-local) and [cch-bash-sandbox](../../packages/shell/bash-sandbox)), and Consumer ([cch-tool-bash](../../packages/shell/tool-bash), the `bash` schema). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Raw process-group mechanics live behind the [subprocess seam](subprocess.md).

Source: [`packages/shell/shell/src/types.ts`](../../packages/shell/shell/src/types.ts)

## Managed shell environment namespace

`CCH_*` variables are Harness-owned child-process facts. The model-facing bash tool collects them through `ctx.shellEnv` and passes them through `ShellExecRequest.cchEnv`; the subprocess service removes inherited `CCH_*` names before merging the current snapshot. The `CchEnvironmentKey`/`CchEnvironment` vocabulary is owned by the [subprocess seam](subprocess.md) and re-exported by `cch-shell`.
...
interface ShellExecRequest {
   ...
   * scrub. Managed facts belong in {@link cchEnv}, which merges after this
  ...
  /**
   * Harness-owned `CCH_*` variables for this execution (typed to managed
   * keys). Executors discard ambient `CCH_*` entries before merging this
   ...
   */
  cchEnv?: CchEnvironment | undefined
...
interface ShellExecSpec {
   ...
   * {@link ShellExecRequest.env}; {@link cchEnv} still merges after them.
   * OPTIONAL on the spec for the same reason as `stdin`: absent means no
   * ordinary extra environment.
   */
  env?: Record<string, string> | undefined
  /** Managed `CCH_*` snapshot (typed to managed keys); merges after {@link env}. */
  cchEnv?: CchEnvironment | undefined
...
`stdin` and `env` are trusted in-process plugin inputs and are not exposed by `cch-tool-bash`. The local executor scrubs ambient credentials before merging explicit caller-supplied env. See [the bash-stdin-env Agent Note](../../.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.md).
...
Each stream is a `CollectedOutput` — the (possibly truncated) text plus recovery info; when truncated, `text` is the **tail** and the complete stream spills to a private file. The fields are owned by the [subprocess seam](subprocess.md) and re-exported by `cch-shell`.

## File sandbox: `ShellSandboxInfo`

A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@coco-harness/cch-sandbox-policy`](../../packages/sandbox/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. The mode/root/enforcement vocabulary is owned by the [`@coco-harness/cch-sandbox` seam](sandbox.md); modes govern file effects only.
...
`start()` returns a handle with no id or owner. `cch-tool-bash` adapts it into `ctx.jobs.start()` hooks; the generic runtime then owns job identity and lifecycle. `done` resolves when the process closes and never rejects, reads remain valid after settlement, and sandbox facts are stamped before `done` resolves.
...
`ShellExecutor` owns `resolve`, foreground `run`, background-process `start`, and the `sandboxMode` capability fact. `cch-bash-local` owns command defaulting, timeout/abort classification, the terminal environment, and the background read merge; process groups, bounded collectors, spill files, credential scrubbing, and disposal quiescence are the [subprocess service](subprocess.md)'s. `cch-tool-bash` owns model-facing rendering and adapts background handles into the [generic job runtime](jobs.md). `cch-shell` owns the shell tools' shared exit-status contract: the exported `parseExitStatus`/`ParsedExitStatus` inverts the `[exit code: N]` / `[killed by signal: X]` markers `cch-tool-bash`'s `renderResult` and `cch-tool-pwsh`'s `renderPwshResult` append, and both tools' `presentResult` use it to split the rendered text into the terminal card's output body and its exit-status pill.
...
Registry (`ctx.shellEnv`) for trusted, per-execution `CCH_*` variables. The namespace is rebuilt for every model shell call: ambient `CCH_*` values are discarded by the executor, then the registry's current snapshot is injected. Built-in shell facts remain owned by the registry itself while plugins can register additional, enumerable facts with effect-scoped disposal.
...
/**
 * Build the trusted `CCH_*` snapshot for one shell tool execution.
 * @param execution - the current tool execution.
 * @returns an immutable environment overlay containing built-ins and current contributions.
 */
collect(execution: ToolExecution): CchEnvironment
...
Types: [CchEnvironment](subprocess.md) · [ToolExecution](tools.md)
