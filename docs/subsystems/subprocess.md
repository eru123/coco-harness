...
The subprocess seam is split across a Service Definition ([cch-subprocess](../../packages/subprocess/subprocess), `ctx.subprocess`) and Service Provider ([cch-subprocess-local](../../packages/subprocess/subprocess-local)); its Consumers are other capability seams and out-of-process backends: the [bash executor family](shell.md) uses collected batch output, LSP uses raw protocol pipes, the PTY backend uses the terminal primitive, and the ACP subagent backend uses piped ndjson plus inherited stderr. This seam owns the managed `CCH_*` environment namespace, the shared credential scrub (`scrubbedParentEnv`), and the `CollectedOutput` shape; [cch-shell](../../packages/shell/shell) re-exports the vocabulary so bash consumers keep one import root.
...
`CCH_*` variables are Harness-owned child-process facts; implementations discard ambient `CCH_*` names before the caller's explicit `env` merges, so a current fact arrives only as a deliberate string entry, while an explicit `undefined` tombstone removes an ordinary ambient value. Each collected stream reports its truncation and spill-recovery state through `CollectedOutput`.

```ts type-equiv
/** One environment key inside the managed {@link CCH_ENV_PREFIX} namespace. */
type CchEnvironmentKey = `${typeof CCH_ENV_PREFIX}${string}`
```

```ts type-equiv
/** Trusted Coco Harness variables for one child-process execution. */
type CchEnvironment = Readonly<Record<CchEnvironmentKey, string>>
```
 ...
 * not a hidden subprocess-service default — decides them (the `cch-shell`
...
interface SubprocessSpawnSpec {
   ...
   * entry or current `CCH_*` fact survives the scrub; `undefined` is a
...
The abstract [`SubprocessRuntime`](../../packages/subprocess/subprocess/src/index.ts) Service Definition specifies execution-world coordinates, executable lookup, ordinary `spawn`, and `spawnTerminal`. [`LocalSubprocessRuntime`](../../packages/subprocess/subprocess-local/src/index.ts) provides them with detached process trees, per-disposition wiring, credential scrubbing, `node-pty`, platform process inspection, and terminate-and-join disposal. See [`cch-subprocess`](../../packages/subprocess/subprocess/README.md) for the Service Definition contract and [`cch-subprocess-local`](../../packages/subprocess/subprocess-local/README.md) for local mechanics.
