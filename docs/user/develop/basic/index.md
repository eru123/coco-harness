...
Run `pwd` from the repository root, then create `scratch-plugin/cordis.yml` as a Web overlay that inserts the local plugin. Replace `/absolute/path/to/coco-harness` below with the printed path:

```yaml
- insert:
    - id: hello
      name: '/absolute/path/to/coco-harness/scratch-plugin/src/my-plugin.ts'
```
...
```sh
pnpm cch web --patch ./scratch-plugin/cordis.yml
```
