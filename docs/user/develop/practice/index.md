...
- **Service Definition** (`cch-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`cch-bash-local`) — executes commands on the local machine
- **Consumer** (`cch-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  cch-shell   │────▶│  cch-bash-local  │     │ cch-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@coco-harness/cch-my-cap'
...
```yaml
- name: '@coco-harness/cch-my-cap-local'
- name: '@coco-harness/cch-tool-my-cap'
```
