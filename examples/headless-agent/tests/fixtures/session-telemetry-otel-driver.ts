...
import { boot, resolveConfigPath } from '@coco-harness/cch-app-boot'
import { recordFeedback } from '@coco-harness/cch-command-feedback'
import { runFixtureTurn } from '@coco-harness/cch-loader-smoke'
...
process.env.CCH_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.CCH_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
