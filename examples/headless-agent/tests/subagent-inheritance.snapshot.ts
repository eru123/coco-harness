...
import { normalizeSessionLog, scrubRequestHeaders, type NormalizeContext } from '@coco-harness/cch-acp-snapshot'
...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@coco-harness/cch-session'
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'cch-subagent-inherit-',
      binScript,
      ...
      env: {
        ...
        CCH_SNAPSHOT_FILE: replayOverride,
        CCH_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@coco-harness/cch-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the CCH file sandbox cannot modify files in the standing mode.')
