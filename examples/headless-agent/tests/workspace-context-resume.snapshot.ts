...
import { renderWorkspaceContext } from '@coco-harness/cch-agent-instructions'
import { resolveConfig, workspaceBaselineIdentity } from '@coco-harness/cch-agent-instructions/src/config.ts'
...
describe('agent-instructions resume snapshot', () => {
  it('appends an offline replacement without duplicating the visible baseline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'cch-workspace-context-resume-',
      binScript,
      ...
      tsconfigPath,
      env: {
        CCH_SNAPSHOT_FILE: replayFixture,
  ...
  it('supersedes an incompatible baseline when precedence changed offline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'cch-workspace-context-precedence-',
      binScript,
      libBinScript: binScript,
      configPath,
      binArgs: [configPath, 'Acknowledge the current workspace instruction.'],
