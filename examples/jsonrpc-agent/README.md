...
| `DEEPSEEK_BASE_URL` | Host endpoint used by `cch-llm-deepseek` |
| `CCH_CWD` | Agent workspace for bash and filesystem tools |
| `CCH_CONTEXT_WINDOW` | Context capacity recorded for the `CCH_MODEL` catalog entry in the minimal variant |
| `CCH_MAX_TOKENS_AS_SUCCESS` | `true` (default) accepts token-limited results; `false` reports them as errors |
| `CCH_MODEL` | Default model used by `minimal.py`; `--model` takes precedence |
| `CCH_SESSION_ROOT` | JSONL session directory |
| `CCH_SYSTEM_PROMPT` | Deployment-provided coding persona |

Pass the config path through the Python SDK's `cordis` option or `CCH_CORDIS_CONFIG`. The bundled executable already carries every plugin named by this file; the target machine does not need Node.js.

## Minimal variant

[`minimal.cordis.yml`](minimal.cordis.yml) is the complete standalone counterpart of the Web `minimal` preset. `CCH_SYSTEM_PROMPT` selects its system prompt, with `You are a helpful software engineer assistant.` as the fallback. It suppresses every system-prompt runtime-context contribution for fresh sessions and mounts no context-compaction plugin. Its model-facing tools are exactly:
