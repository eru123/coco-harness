...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'cch-jsonrpc-agent-smoke-'))
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        CCH_CWD: root,
        CCH_SESSION_ROOT: join(root, '.sessions'),
        ...(envValue === undefined ? {} : { CCH_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
    ...
    try {
      ...
      expect(initialized).toMatchObject({
        jsonrpc: '2.0',
        id: 1,
        result: { serverInfo: { name: 'coco-harness-sdk-runtime' } },
      })
  ...
  it('rejects an invalid max-token success env value', async () => {
    const { exitCode, stdout, stderr } = await execa(process.execPath, [
      ...
      env: {
        DEEPSEEK_API_KEY: 'keyless-smoke-no-call',
        CCH_MAX_TOKENS_AS_SUCCESS: 'sometimes',
      },
      stdin: 'ignore',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@coco-harness/cch-sdk-jsonrpc-server)')
