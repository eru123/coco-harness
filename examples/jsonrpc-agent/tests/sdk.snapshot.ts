 ...
 * the REAL `cch-jsonrpc-agent` runtime (per `CCH_EXAMPLE_MODE`) through the
 * REAL `@coco-harness/cch-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `CCH_SNAPSHOT=record`
 * re-records against the live API; `CCH_SNAPSHOT=refresh` replays committed
...
import { resolveExampleLaunch } from '@coco-harness/cch-loader-smoke'
import { CocoHarness, type HarnessNotification, type RunResult } from '@coco-harness/cch-sdk-client'
...
const mode = process.env.CCH_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  {
    name: 'bash-tool',
    prompt: 'Run this exact command with your bash tool, then reply with its stdout only: echo cch-sdk-proof-7391',
  ...
  },
  {
    ...
    environment: { CCH_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@coco-harness/cch-system-prompt') return []
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    CCH_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    CCH_SESSION_ROOT: sessionsRoot,
    CCH_CWD: cwd,
    CCH_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      CCH_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { CCH_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
    ...scenario.environment,
  }

  const harness = new CocoHarness({
    launch: {
