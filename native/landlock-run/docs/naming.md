...
The public package family belongs to the `@coco-harness` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@coco-harness/node-addon-landlock-run
@coco-harness/node-addon-landlock-run-<platform>
```
