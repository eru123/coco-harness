...
Platform packages (`os`/`cpu`-selected optional dependencies, no JavaScript inside): `@coco-harness/node-addon-landlock-run-linux-x64`, `@coco-harness/node-addon-landlock-run-linux-arm64`. On hosts without one, `launcherPath()` returns a deterministic nonexistent path and `probe()` reports `'unusable'` — there is deliberately no install-time compile fallback.
