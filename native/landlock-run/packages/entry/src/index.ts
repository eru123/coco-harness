 ...
 * npm package `@coco-harness/node-addon-landlock-run-<platform>-<arch>` (npm's
...
export function launcherPath(
  ...
  const platformPackage = `@coco-harness/node-addon-landlock-run-${process.platform}-${process.arch}`
  try {
