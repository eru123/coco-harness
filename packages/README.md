...
npm scope: `@coco-harness/cch-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@coco-harness/cch-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`bundle/`](bundle/README.md) | Installable `cch --profile` patch layers | Product — stable API |
