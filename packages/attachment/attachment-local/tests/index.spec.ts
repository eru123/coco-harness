...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const cchHome = await mkdtemp(join(tmpdir(), 'cch-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { cchHome })
      ...
      await rm(cchHome, { recursive: true, force: true })
    }
  })

  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const cchHome = await mkdtemp(join(tmpdir(), 'cch-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { cchHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { cchHome, maxImageBytes: 1 })
