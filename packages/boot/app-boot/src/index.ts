/**
 * Shared boot glue for the app bins (`cch`, `cch-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.cch`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @coco-harness/cch-app-boot
 */
...
import { Context, type FiberState } from '@coco-harness/cordis'
...
declare module '@coco-harness/cordis' {
  interface Context {
    ...
    cchHomePath?: typeof cchHomePath
  }
}
...
export {
  ...
  type CchBundleManifest,
  type CchManifestSection,
  type CchProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$CCH_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['CCH_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * entries (`@coco-harness/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@coco-harness/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@coco-harness/cordis-plugin-group`
 ...
 * directory. The self-referential `cch-tool-cordis` toolset reads and edits this
