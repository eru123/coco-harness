 ...
 * `cch --profile` launcher family.
 *
 * A profile is a directory under `$CCH_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `cch.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"cch": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `cch.profile.bundles` order over
 ...
 * first from the cch installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$CCH_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @coco-harness/cch-app-boot/profile
 */
...
import type { EntryOptions } from '@coco-harness/cordis-plugin-loader'
import { applyEntryPatches, type PatchOptions } from '@coco-harness/cordis-plugin-include'
...
/** The bundle half of the `cch` manifest section: what a bundle package exports. */
export interface CchBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `cch` manifest section: what a profile directory composes. */
export interface CchProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `cch`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface CchManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: CchBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: CchProfileManifest
}
...
export interface ProfileManifest {
  ...
  cch?: CchManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `cch.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `cch.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`cch --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveCchHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveCchHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`cch: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@coco-harness/cch-base', '@coco-harness/cch-web-app'],
  headless: ['@coco-harness/cch-base', '@coco-harness/cch-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@coco-harness/cch-base', '@coco-harness/cch-web-app', '@coco-harness/cch-headless'],
}

/** The bundle list a `cch plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@coco-harness/cch-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this cch profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `cch.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `cch-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      cch: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`cch: ${link} exists and is not a symlink; remove it so cch can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$CCH_HOME/profiles/node_modules`: one
 * symlink per package in the cch app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`cch-compaction`, `cch-invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the cch app's package.json.
 * @param home - the Harness home; defaults to {@link resolveCchHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveCchHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (cch-subprocess,
    // cch-compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.cch?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    cch: {
      ...manifest.cch,
      profile: { ...manifest.cch?.profile, bundles: [...current] },
    },
  }
 ...
 * `@coco-harness/cch-base` (and every other in-box bundle) always comes from
 * the same installation as the running cch, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `cch.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the cch app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the cch installation or ${profileDir}; `
    + `run 'cch plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `cch.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `cch.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the cch app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveCchHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveCchHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'cch plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the cch section entirely.
  const bundles = manifest.cch?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.cch?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no cch.bundle in its package.json`)
    }
