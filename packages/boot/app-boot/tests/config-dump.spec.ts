...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'cch-config-dump-'))
...
function writeBase(dir: string): string {
  ...
  writeFileSync(base, [
    ...
    '  config:',
    '    value: base',
    '    key: !!js process.env.CCH_DUMP_SPEC',
    '- id: untouched',
    '  name: ./noop.mjs',
    '',
  ].join('\n'))
...
describe('renderConfigDump', () => {
  it('composes overlay layers in order, prints !!js verbatim, and labels each section with its source and patches', () => {
    ...
    expect(parsed).toEqual([
      {
        id: 'shared',
        name: './noop.mjs',
        config: { value: 'surface', key: { __jsExpr: 'process.env.CCH_DUMP_SPEC' } },
      },
    ...
    expect(dump).toContain('!!js process.env.CCH_DUMP_SPEC')
