/**
 * Profile machinery of `cch-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'cch-profile-'))
...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { cch: { bundle: { patch: './cordis.patch.yml' } } },
    }))
    if (spec.patch !== undefined) writeFileSync(join(dir, 'cordis.patch.yml'), spec.patch)
  }
  writeFileSync(join(appDir, 'package.json'), JSON.stringify({ name: 'cch-app', dependencies: appDeps }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@coco-harness/cch-base'])
    ...
    expect(readProfileManifest('t', dir).cch?.profile?.bundles).toEqual(['@coco-harness/cch-base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', cch: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).cch?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each cch.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or cch section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @coco-harness/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@coco-harness/cch-base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).cch?.profile?.bundles)
      .toEqual([...PROFILE_TEMPLATES.web ?? []])
  })

  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@coco-harness/cch-base': { patch: '[]\n' },
      '@coco-harness/cch-web-app': { patch: '[]\n' },
      '@coco-harness/cch-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@coco-harness/cch-base', '@coco-harness/cch-web-app', '@coco-harness/cch-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).cch?.profile?.bundles)
      .toEqual(['@coco-harness/cch-base', '@coco-harness/cch-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@coco-harness/cch-base', '@coco-harness/cch-web-app', '@coco-harness/cch-headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).cch?.profile?.bundles).toEqual([
      '@coco-harness/cch-base', '@coco-harness/cch-web-app', '@coco-harness/cch-headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no cch.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no cch.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', () => {
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'cch-app']) {
  ...
  it('throws when a fallback entry is a real directory', () => {
    const anchor = stageInstallation({})
    const home = tmp()
    mkdirSync(join(home, 'profiles', 'node_modules', 'cch-app'), { recursive: true })
  ...
  it('replaces a wrong symlink', () => {
    ...
    symlinkSync(tmp(), join(fallback, 'cch-app'), 'junction')
    healProfilesModuleFallback(anchor, home)
    expect(readlinkSync(join(fallback, 'cch-app'))).toContain('app')
  })
  ...
  it('tolerates losing the concurrent-heal race to an identical link and rejects a different one', () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    expect(lstatSync(join(fallback, 'cch-app')).isSymbolicLink()).toBe(true)
  })
})
