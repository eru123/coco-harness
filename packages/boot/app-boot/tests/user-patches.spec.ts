/**
 * User patch-layer behavior of `cch-app-boot`: the optional patch-list loader
...
import Hmr from '@coco-harness/cordis-plugin-hmr'
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'cch-user-patches-'))
...
describe('loadOptionalPatches', () => {
  ...
  it('parses a patch list and preserves !!js expressions as loader expression nodes', () => {
    ...
    writeFileSync(join(dir, PROFILE_PATCH_FILENAME), [
      '- id: agent-loop',
      "  name: '@coco-harness/cch-agent-loop'",
      '  config:',
      '    model: !!js process.env.CCH_SPEC_MODEL',
      '- insert:',
      '    - id: llm',
      "      name: '@coco-harness/cch-llm-pi-ai'",
      '',
    ].join('\n'))
    ...
    expect(patches?.[0]).toMatchObject({
      id: 'agent-loop',
      config: { model: { __jsExpr: 'process.env.CCH_SPEC_MODEL' } },
    })
...
describe('boot with user patches', () => {
  it('applies id-targeted overrides, inserts, and interpolates !!js from the environment', async () => {
    ...
    writeFileSync(join(userDir, PROFILE_PATCH_FILENAME), [
      ...
      '  name: ./noop.mjs',
      '  config:',
      '    value: !!js process.env.CCH_APP_BOOT_USER_SPEC',
      '- insert:',
      '    - id: user-extra',
      '      name: ./noop.mjs',
      '',
    ].join('\n'))
    process.env['CCH_APP_BOOT_USER_SPEC'] = 'user-value'
    ...
    try {
      ...
      await ctx.fiber.dispose()
      delete process.env['CCH_APP_BOOT_USER_SPEC']
    }
  })
