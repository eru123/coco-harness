/**
 * @coco-harness/cch-headless — one-shot direct Agent driver. The bundle patch
 * rides over cch-base without Host, HTTP, or browser plugins; this runner
 ...
 *
 * @module @coco-harness/cch-headless
 */
...
import type { ModelSelectionRef } from '@coco-harness/cch-agent'
import type {} from '@coco-harness/cch-agent-default-model'
...
import type {} from '@coco-harness/cch-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`cch: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`cch: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
