 ...
 * @module @coco-harness/cch-headless/startup
 */
...
function headlessCommand(): Command {
  return new Command()
    ...
    .addHelpText('after', `
Examples:
  cch --profile headless "run the tests"     answer one task and exit
`)
}
...
export function apply(ctx: Context): void {
  ...
  program.action(() => {
    ...
    if (task.trim() === '') program.error('error: a task is required, for example: cch --profile headless "run the tests"')
