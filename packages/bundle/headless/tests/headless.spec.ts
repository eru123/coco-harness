...
import AgentDefaultModelConfig from '@coco-harness/cch-agent-default-model'
import { createAssistantMessage } from '@coco-harness/cch-llm'
...
describe('headless runner', () => {
  ...
  it('prints the durable model failure when the final turn ends in error', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '\n',
      err: 'cch: SERVER: provider unavailable\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('reports a direct Agent creation failure', async () => {
    ...
    expect(err).toBe('cch: factory exploded\n')
