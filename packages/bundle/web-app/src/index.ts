/**
 * @coco-harness/cch-web-app — the browser-surface bundle's runtime glue plugin
 * plus the bundle patch (`cordis.patch.yml`, declared by the `cch.bundle.patch`
 ...
 * @module @coco-harness/cch-web-app
 */
...
import { addHarnessSourceSection } from '@coco-harness/cch-app-boot'
import * as FrontendStatic from '@coco-harness/cch-host-frontend-static'
import type {} from '@coco-harness/cordis-plugin-loader'
...
/** This cch installation's root, from either this package's source or built entry. */
...
export interface Config {
   ...
   * section and the `CCH_WEB_URL` bash variable). A one-shot non-interactive
...
const CCH_WEB_URL = 'CCH_WEB_URL' as const
...
/** Model-visible orientation and acceptance boundary for sessions created through `cch web`. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but client-plugin changes reload without a refresh only while '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic updates. '
    + 'Every other change — the apps/web shell and plain packages — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the Coco Harness Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only cch web injects window.__CCH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@coco-harness/cch-web-frontend/dist/index.html')
  } catch {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [CCH_WEB_URL]: { description: 'Canonical local URL of the Coco Harness Web GUI serving this session.' },
        },
        resolve: () => ({ [CCH_WEB_URL]: localWebUrl(runtimeCtx) }),
      })
    })
  }
  if (config.printUrl) {
    ...
    const printUrl = (): void => {
      ...
      console.log(`cch web: ${localWebUrl(ctx)}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
    }
