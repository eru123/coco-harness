...
import { stubSettingsScope, type StubSettingsScope } from '@coco-harness/cch-client-test-runtime'
import type { LocaleSettings, LocaleSnapshot } from '@coco-harness/cch-client-locale/client'
 ...
 * `usePinnedBrowserLanguages` (cch-client-test-runtime): they need the shapes
...
describe('LocaleRuntime', () => {
  beforeEach(() => {
    // An English browser is the baseline these specs assert their en state on.
    stubLanguages('en-US')
  })
  ...
  it('translates through the active locale and echoes missing keys', () => {
    const { svc } = make()
  ...
  it('setLocale to the already-active locale no-ops: no scope write, no event', () => {
  ...
  it('adopts a standing Host preference without writing it back', () => {
    const host = stubSettingsScope<LocaleSettings>()
    const { svc, events } = make(host)
