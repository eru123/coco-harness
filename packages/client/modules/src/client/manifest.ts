 ...
 * `__CCH_BOOT__` wire types, the boot-manifest parser, and the boundaries around
 * {@link ClientModuleSystem}. The package root is the host-side service that
 * composes the wire.
 */

import type {} from '@coco-harness/cordis'
...
export function parseBootManifest(wire: unknown): BootManifest {
  if (typeof wire !== 'object' || wire === null) {
    throw new Error('client-modules: window.__CCH_BOOT__ is missing or not an object')
  }
...
export interface CchWindow {
   ...
   */
  __CCH_MODULES__?: ClientModuleSystem
}
