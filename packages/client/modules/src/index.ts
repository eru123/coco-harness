/**
 * Node half of the client module system (`cch.client` dual-face package): scans
 * the host Loader's entries for packages declaring `cch.client`, composes the
 * `window.__CCH_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @coco-harness/cch-client-modules
 */
...
/** package.json `cch.client` declaration fields, validated one by one after reading the file. */
interface CchClientDeclaration {
  inject?: string[]
  platform: string
...
/** Resolved package metadata for one `cch.client` package (cached per name, never expires). */
interface PkgMeta {
  clientPath: string
  inject?: string[]
  immediately: boolean
...
function parseDshClient(pkgName: string, value: unknown): CchClientDeclaration | undefined {
  ...
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} cch.client.platform must be a string`)
  }
  if (decl.inject !== undefined && (!Array.isArray(decl.inject) || decl.inject.some(i => typeof i !== 'string'))) {
    throw new Error(`client-modules: ${pkgName} cch.client.inject must be a string array`)
  }
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} cch.client.immediately must be a boolean`)
  }
  return {
...
/**
 * Inject the boot entry graph into index.html: `window.__CCH_BOOT__` as the
...
export function injectBootManifest(html: string, graph: WebBootGraph): string {
  ...
  const script = `<script>window.__CCH_BOOT__ = ${json}</script>`
...
/**
 * The web plugin table service: incremental `cch.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `cch.client` declaration) are
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares cch.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
