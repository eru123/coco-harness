/**
 * Package-owned invariant companion for `@coco-harness/cch-client-modules`.
 * @module @coco-harness/cch-client-modules/invariant
 */
...
const PACKAGE_NAME = '@coco-harness/cch-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __CCH_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
