 ...
 */
import type { Context, Fiber } from '@coco-harness/cordis'
import type {
...
import { SESSION_SEARCH_RESULT_LIMIT } from '@coco-harness/cch-host-apiproxy/api'
import type {
  HostObservable, SessionMaybeProvideInfo, SessionProvideInfo,
} from '@coco-harness/cch-client-ui-slots'
import type { SessionProjectionMap } from '@coco-harness/cch-session-projection/types'
...
// cch-scope, keyed by Agent identity); re-exported here so existing
...
export class SessionRuntime implements ISessions {
  ...
  constructor(
    ...
    this.selection = createSnapshotStore<SessionSelection>(
      {},
      { persist: { name: 'cch.sessions.current' } })
   ...
   */
  async create(opts: { workspaceId?: WorkspaceId; cwd?: string; sessionId?: SessionId; mode?: 'buddy' } = {}): Promise<SessionId> {
