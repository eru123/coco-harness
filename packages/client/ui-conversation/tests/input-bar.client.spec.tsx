...
describe('Enter semantics', () => {
  ...
  it('keeps the owning placeholder or ordinary guidance when whole-queue steering is unavailable', () => {
    ...
    expect(bench({ running: true, queue: [row('q-1')], draft: 'a message' }).textarea.placeholder).toBe('Message the agent')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      subagent: {
  ...
  it('draft content outranks the queue: accelerated Enter steers the draft only', () => {
    const steerQueue = vi.fn()
    const { textarea, sink } = bench({ running: true, queue: [row('q-1')], draft: 'interject', steerQueue })
    fireEvent.keyDown(textarea, { key: 'Enter', ctrlKey: true })
    expect(sink).toHaveBeenCalledWith('interject', [], 'steer')
    expect(steerQueue).not.toHaveBeenCalled()
  })
...
describe('running and lock semantics', () => {
  it('running keeps the input free (typing + Enter queue) while the primary turns stop', () => {
    const { textarea, button, stop, sink } = bench({ running: true, draft: 'queued note' })
    expect(textarea.disabled).toBe(false)
    fireEvent.change(textarea, { target: { value: 'queued note 2' } })
    fireEvent.keyDown(textarea, { key: 'Enter' })
    expect(sink).toHaveBeenCalledWith('queued note 2', [], 'queue')
  ...
  it('running plain Enter follows the busy-state Steer preference', () => {
    const { textarea, sink } = bench({ running: true, busyEnter: 'steer', draft: 'direct steer' })
    fireEvent.keyDown(textarea, { key: 'Enter' })
    expect(sink).toHaveBeenCalledWith('direct steer', [], 'steer')
  })
  ...
  it('running continuable subagent keeps Send beside an independent Stop', () => {
    ...
    expect(textarea.disabled).toBe(false)
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('a follow-up message', [], 'queue')
...
describe('decorations', () => {
  ...
  it('an inserted reference renders as a chip at its placeholder offset', () => {
    ...
    expect(shell.snapshot.draft).toBe('refer to \uFFFC content')
  })
...
describe('strips and variants', () => {
  ...
  it('renders the notice strip from the machine notice store', () => {
    const { view, shell } = bench()
    act(() => { shell.notify('error', 'the command failed') })
    expect(view.getByText('the command failed')).toBeTruthy()
  })
