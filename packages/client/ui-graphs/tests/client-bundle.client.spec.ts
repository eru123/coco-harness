 ...
 * not built (`pnpm --filter @coco-harness/cch-client-ui-graphs bundle`).
 */
...
const PLUGIN_ID = '@coco-harness/cch-client-ui-graphs'
...
function readBundle(): string | undefined {
  try {
    ...
    return readFileSync(resolve('packages/client/ui-graphs/lib/client.js'), 'utf8')
  } catch {
    return undefined
  }
}
...
describe('tsdown client artifact', () => {
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@coco-harness/cch-client-runtime/client', await import('@coco-harness/cch-client-runtime/client')],
    ])
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@coco-harness/cch-client-locale/client')
    ctx.plugin({ inject: [...locale.inject], apply: locale.apply })
    const fiber = ctx.plugin(exports as { apply: (ctx: Context) => void })
    await fiber.await()
    expect(slots.entries('conversation.view').map(e => e.options.id)).toEqual(['graphs'])
    await fiber.dispose()
