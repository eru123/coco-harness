...
describe('SidebarRoot.module.css', () => {
  it('shares and cancels the wide shell trailing padding structurally', () => {
    ...
    expect(root?.get('--cch-sidebar-inline-padding')).toBe('12px')
    expect(root?.get('padding')).toBe('6px var(--cch-sidebar-inline-padding)')
    ...
    expect(declarations('.regionArea')?.get('margin-right')).toBe(
      'calc(-1 * var(--cch-sidebar-inline-padding))',
    )
