...
import type { RunningToolCall, ToolResultNode } from '@coco-harness/cch-client-runtime/client'
...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'skill', argsRaw: '{"name":"cch-manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"cch-manage-issues"}'): RunningToolCall {
  return {
...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    const row = screen.getByRole('button', { name: 'Skillcch-manage-issues' })
    ...
    expect(view.container.textContent).not.toContain('{"name":"cch-manage-issues"}')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('cch-manage-issues')
