...
import type { ConvViewProps, ViewTab } from '@coco-harness/cch-client-ui-conversation/client'
import {
  ConversationSession, ConversationSessionHeader,
  type ConversationSessionHeaderProps, type ConversationSessionProps,
} from '@coco-harness/cch-client-ui-conversation/src/client/skeleton/ConversationSession.tsx'
import { createChatStore } from '@coco-harness/cch-client-ui-conversation/src/client/stores.ts'
import { en as conversationEn } from '@coco-harness/cch-client-ui-conversation/src/client/locales.ts'
...
import { apply, inject } from '@coco-harness/cch-client-ui-trajectory/client'
import { apply as nodeApply } from '@coco-harness/cch-client-ui-trajectory'
...
describe('plugin registration', () => {
  ...
  it('shares one browser-wide duration preference across session injections', async () => {
    ...
    expect(localStorage.getItem('cch.trajectory.duration')).toBe('true')
    expect(localStorage.getItem(`cch.trajectory.duration.${SID}`)).toBeNull()
  })
...
describe('tab switching in ConversationRoot', () => {
  ...
  it('labels the trajectory tab in the active locale', async () => {
    ...
    // en is the only shipped locale: a redundant setLocale must not disturb the label.
    locale.setLocale('en')
    expect(labelOf()).toBe('Trajectory')
  })
