...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('counts the themed scrollbar inside the shell trailing inset', () => {
    expect(root?.get('--cch-session-list-edge-inset')).toBe('var(--cch-sidebar-inline-padding)')
    expect(root?.get('--cch-session-list-scrollbar-width')).toBe('8px')
    expect(root?.get('--cch-session-list-scrollbar-offset')).toBe('2px')
    expect(root?.get('padding-right')).toBe('var(--cch-session-list-edge-inset)')
    expect(listArea?.get('margin-left')).toBe('-4px')
    expect(listArea?.get('padding-left')).toBe('4px')
    expect(listArea?.get('margin-right')).toBe('calc(-1 * var(--cch-session-list-edge-inset))')
    expect(declarations('.fade')?.get('right')).toBe('var(--cch-session-list-edge-inset)')
    expect(list?.get('margin-right')).toBe('var(--cch-session-list-scrollbar-offset)')
    expect(list?.get('margin-left')).toBe('-4px')
    expect(list?.get('padding-left')).toBe('4px')
    expect(list?.get('padding-right')).toBe([
      'calc(',
      'var(--cch-session-list-edge-inset)',
      '- var(--cch-session-list-scrollbar-width)',
      '- var(--cch-session-list-scrollbar-offset)',
      ')',
    ].join(' '))
