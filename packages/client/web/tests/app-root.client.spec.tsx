...
afterEach(cleanup)
import { AppRoot } from '@coco-harness/cch-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@coco-harness/cch-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@coco-harness/cch-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@coco-harness/cch-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
