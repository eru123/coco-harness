...
describe('compaction-tool-result-pruner real Loader composition', () => {
  it('loads and resolves the flat YAML plugin shape', async () => {
    root = await mkdtemp(join(tmpdir(), 'cch-compact-tool-result-prune-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@coco-harness/cch-token-meter') return TokenMeter
        if (specifier === '@coco-harness/cch-compaction-tool-result-pruner') return ToolResultPruner
