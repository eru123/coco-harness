# @coco-harness/cch-agent-instructions
...
Instructions from: ~/.cch/AGENTS.md
...
The user-global file is always `$CCH_HOME/AGENTS.md` with no local overlay; both candidate lists only control project scopes. `$CCH_HOME` defaults to `~/.cch`, and configured `~`, `~/...`, and Windows-style `~\...` prefixes are expanded against the operating-system home directory. A non-positive or non-finite render budget disables both baseline and dynamic loading; configured `maxSourceBytes` must be a positive integer.
...
- **Candidate semantics stay intentionally small** — lowercase names, `.claude/rules/`, and `@path` imports are not interpreted; project scopes load `AGENTS.local.md`/`CLAUDE.local.md` overlays by default, but the user-global `$CCH_HOME` scope has no local overlay and other custom names require explicit candidate configuration.
