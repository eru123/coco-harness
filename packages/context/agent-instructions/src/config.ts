 ...
 *
 * @module @coco-harness/cch-agent-instructions/config
 */
...
export interface Config {
  /** Harness home containing the fixed user-global `AGENTS.md`; defaults to `$CCH_HOME` or `~/.cch`. */
  cchHome?: string
...
export function resolveDiscoveryConfig(
  config: Pick<Config, 'cchHome' | 'projectRootMarkers' | 'instructionFileCandidates' | 'localInstructionFileCandidates'>,
): ResolvedDiscoveryConfig {
  return {
    cchHome: resolveCchHome(config.cchHome),
