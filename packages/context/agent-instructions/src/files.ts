 ...
 *
 * @module @coco-harness/cch-agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@coco-harness/cch-fs'
import { assertNever } from '@coco-harness/cch-llm'
import { cchHomeDisplay } from '@coco-harness/cch-home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.cchHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.cchHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.cchHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(cchHome: string): string {
  return `${cchHomeDisplay(cchHome)}/AGENTS.md`
}
