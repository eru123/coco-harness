...
import * as workspaceContext from '@coco-harness/cch-agent-instructions'
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@coco-harness/cch-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@coco-harness/cch-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@coco-harness/cch-agent'
import AgentLoop from '@coco-harness/cch-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@coco-harness/cch-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
async function tempRepo(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'cch-workspace-context-'))
}
...
describe('workspace context instruction discovery', () => {
  it('treats ENOTDIR while probing a host candidate as confirmed absence', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, cchHome: homeFile })
  ...
  it('loads user-global first, then every root-to-cwd candidate in precedence order', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd, cchHome: home })
  ...
  it('re-reads content after a same-version, same-size rewrite', async () => {
    ...
    try {
      ...
      expect(await loadBaselineInstructions({ cwd, cchHome: home, maxBytes: 65536 })).toBeUndefined()

      const leaf = join(cwd, 'AGENTS.md')
      await write(leaf, 'first')
      const first = await loadBaselineInstructions({ cwd, cchHome: home, maxBytes: 65536 })
      expect(first?.text).toContain('first')
      const again = await loadBaselineInstructions({ cwd, cchHome: home, maxBytes: 65536 })
      ...
      const second = await loadBaselineInstructions({ cwd, cchHome: home, maxBytes: 65536 })
  ...
  it.skipIf(process.platform === 'win32')('skips a file that becomes unreadable after discovery without failing the request', async () => {
    ...
    try {
      ...
      const loaded = await loadBaselineInstructions({ cwd, cchHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file to its target content', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, cchHome: home })
      const loaded = await loadBaselineInstructions({ cwd: root, cchHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file through ctx.fs to its target content', async () => {
    ...
    try {
      ...
      await mountWorkspaceContext(ctx, { cchHome: home, maxBytes: 65536 })
  ...
  it('disables baseline loading when the byte budget is zero', async () => {
    ...
    try {
      ...
      await expect(loadBaselineInstructions({ cwd: root, cchHome: home, maxBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({ cwd: root, cchHome: home, maxBytes: 65536, maxSourceBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({
        cwd: root, cchHome: home, maxBytes: 65536, maxSourceBytes: Infinity,
  ...
  it('defaults cchHome and uses cwd itself as root when no project marker exists', async () => {
    const root = await tempRepo()
    const emptyHome = await tempRepo()
    // Isolate the default-home fallback: blank CCH_HOME is treated as unset, and
    // the home dirs point at an empty dir so the default ~/.cch holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.cch/AGENTS.md would otherwise leak in.
    vi.stubEnv('CCH_HOME', '')
  ...
  it('honors CCH_HOME when cchHome is not configured explicitly', async () => {
    const root = await tempRepo()
    const envHome = await tempRepo()
    try {
      await write(join(envHome, 'AGENTS.md'), 'env global rule')
      vi.stubEnv('CCH_HOME', envHome)

      const files = await discoverBaselineInstructionFiles({ cwd: root })

      expect(files).toEqual([{ absolutePath: join(envHome, 'AGENTS.md'), displayPath: '$CCH_HOME/AGENTS.md' }])
    } finally {
      vi.unstubAllEnvs()
      await rm(root, { recursive: true, force: true })
  ...
  it('labels the default DSH home as ~/.cch when HOME points at the configured default', async () => {
    ...
    const home = await tempRepo()
    try {
      await write(join(home, '.cch/AGENTS.md'), 'global default rule')

      // A set CCH_HOME would override the homedir default and relabel the home.
      vi.stubEnv('CCH_HOME', '')
      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@coco-harness/cch-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root })

      expect(files.map(file => file.displayPath)).toEqual(['~/.cch/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.cch home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.cch/AGENTS.md'), 'global tilde rule')

      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@coco-harness/cch-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, cchHome: '~/.cch' })

      expect(files).toEqual([{ absolutePath: join(home, '.cch/AGENTS.md'), displayPath: '~/.cch/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
  ...
  it('deduplicates user-global instructions when cchHome points at the project root', async () => {
    const root = await tempRepo()
    try {
      await mkdir(join(root, '.git'), { recursive: true })
      await write(join(root, 'AGENTS.md'), 'same file')

      const files = await discoverBaselineInstructionFiles({ cwd: root, cchHome: root })

      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$CCH_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('preserves a visible baseline when its source is unavailable during resume', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { cchHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
      await composeBaselinePrefix(ctx, original)
  ...
  it('does not promote an unchanged budget-omitted baseline file during resume', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cchHome: home, maxBytes: 700 })
  ...
  it('recomposes the baseline when candidate precedence changes between resumes', async () => {
    ...
    try {
      ...
      await write(join(root, 'CLAUDE.md'), 'claude rule')
      await mountWorkspaceContext(originalCtx, { cchHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it('reuses an inserted but unadmitted baseline after session recovery and plugin reload', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      const fiber = await mountWorkspaceContext(ctx, { cchHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it.each([
    ...
    try {
      ...
      await resumedCtx.plugin(workspaceContext, { cchHome: home, maxBytes })
      const resumed = stubAgent(root, [...original.session.events])
  ...
  it('reports a changed user-global instruction through the visible-scope reconcile path', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { cchHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('retains a visible baseline after a plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      const fiber = await mountFileToolsAndWorkspaceContext(ctx, { cchHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('restores a compacted baseline on a hot plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await ctx.plugin(LocalFileSystem, { cwd: '/' })
      const fiber = await ctx.plugin(workspaceContext, { cchHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
      const baseline = baselineEvents(agent)[0]
  ...
  it('deduplicates one AGENTS.md that is both user-global and the project-root candidate', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cchHome: root, maxBytes: 65536 })
      const agent = stubAgent(root)
  ...
  it.each([10, 120])('does not expose state markers when baseline content is omitted at %i bytes', async (maxBytes) => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cchHome: home, maxBytes })
      const agent = stubAgent(root)
  ...
  it('keeps the direct provider API usable without an operation signal', async () => {
    ...
    try {
      ...
      const rendered = await loadBaselineInstructions({ cwd: root, cchHome: home, maxBytes: 65536 }, fs)
  ...
  it('rejects a provider-sized instruction file before reading content', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { cchHome: home, maxBytes: 65536, maxSourceBytes: 4 })
  ...
  it('does not inject anything when maxBytes is zero', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cchHome: home, maxBytes: 0 })
      const agent = stubAgent(root)
  ...
  it('does not inject an empty agent-instructions message when maxBytes is negative', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cchHome: home, maxBytes: -1 })
      const agent = stubAgent(root)
  ...
  it('labels a custom cchHome as CCH_HOME instead of pretending it is ~/.cch', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['$CCH_HOME/AGENTS.md'])
    } finally {
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
  ...
  it('does not repeat a candidate metadata probe during one discovery and read pass', async () => {
    ...
    try {
      ...
      const isolated = await import('@coco-harness/cch-agent-instructions')
      await isolated.loadBaselineInstructions({ cwd: root, cchHome: home, maxBytes: 65536 })
      observedStats.clear()
      await isolated.loadBaselineInstructions({ cwd: root, cchHome: home, maxBytes: 65536 })
  ...
  it('skips an unavailable host candidate but still loads its available sibling', async () => {
    ...
    try {
      ...
      const rendered = await isolated.loadBaselineInstructions({ cwd: root, cchHome: home, maxBytes: 65536 })
...
describe('dynamic nested workspace context injection', () => {
  ...
  it('skips visible baseline scopes when baseline scopes are excluded from reconciliation', async () => {
    ...
    try {
      ...
      agent.session.append('user/message', authoritative, { surfaceOp: 'append' })
      const resolved = resolveConfig({ cchHome: home, maxBytes: 65536, localInstructionFileCandidates: [] })
      const cache: InstructionVersionCache = new WeakMap()
  ...
  it('does not mark omitted nested files as pending-loaded', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/sub/file.txt'), 'subtree file')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { cchHome: home, maxBytes: 700 })
      const agent = stubAgent(root)
      await ctx.tools.execute({
        signal: testToolSignal,
  ...
  it('does not attach nested instructions when the byte budget is disabled', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/deep/file.txt'), 'hello')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { cchHome: home, maxBytes: 0 })
  ...
  it('retries a nested instruction touch when only a truncated budget notice was rendered', async () => {
    ...
    try {
      ...
      fs.entries.set(join(root, 'pkg/file.txt'), { type: 'file', content: 'hello' })
      await ctx.plugin(ToolFs)
      await ctx.plugin(workspaceContext, { cchHome: home, maxBytes: 20 })
      const agent = stubAgent(root)
...
describe('workspace context inbox synchronization', () => {
  ...
  it('holds back a dynamic change a one-byte positive render budget cannot represent', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { cchHome: home, maxBytes: 1 })
      const agent = stubAgent(root)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
