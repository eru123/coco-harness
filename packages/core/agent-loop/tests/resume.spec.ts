...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId, SessionPreparation } from '@coco-harness/cch-session'
...
async function persistentHarness(adapter: MockAdapter): Promise<{ ctx: Context; root: string }> {
  const root = await mkdtemp(join(tmpdir(), 'cch-resume-'))
  dirs.push(root)
