 ...
 *
 * @module @coco-harness/cch-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@coco-harness/cordis'
import type { Fiber } from '@coco-harness/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@coco-harness/cch-scope'
import type { Scoped } from '@coco-harness/cch-scope'
import type { SessionEvent, SessionId } from '@coco-harness/cch-session'
import type { TypertContext, TypertLookup } from '@coco-harness/cch-typert-protocol'
...
declare module '@coco-harness/cordis' {
  interface Context {
     ...
     * `cch-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in cch-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `cch-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `cch-agent-loop` package.
 */
 ...
 * (`@coco-harness/cch-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@coco-harness/cch-agent#Agent',
