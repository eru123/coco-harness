 ...
 *
 * @module @coco-harness/cch-scope
 */

import type { Context, Fiber } from '@coco-harness/cordis'
import { Context as CordisContext } from '@coco-harness/cordis'
...
function linkScopeParent(key: ScopeKey, parent: ScopeKey): void {
  for (let cursor: ScopeKey | undefined = parent; cursor !== undefined; cursor = scopeParents.get(cursor)) {
    if (cursor === key) throw new Error('cch-scope: scope parent link would form a cycle')
  }
...
export function bindScopeParent(key: ScopeKey, parent: ScopeKey): ScopeParentBinding {
  if (scopeParents.has(key)) {
    throw new Error('cch-scope: scope key is already bound to a parent; re-linking requires the binding returned by the original bind')
  }
