 ...
 *
 * @module @coco-harness/cch-system-prompt
 */

import { Context, Service } from '@coco-harness/cordis'
import z from '@coco-harness/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@coco-harness/cch-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@coco-harness/cch-scope'
import type { ContextSnapshotSection, ToolSchema } from '@coco-harness/cch-llm'
...
declare module '@coco-harness/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@coco-harness/cch-scope`): scoped listeners
...
export interface Config {
  /** Include the fixed Coco Harness identity before the deployment persona (default true). */
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by Coco Harness.',
      })
    }
    this.section({
