...
import SystemPrompt, { AssembleContext, PromptAssembly, renderContextSnapshot, renderPrompt } from '@coco-harness/cch-system-prompt'
...
const IDENTITY = 'You are an AI agent powered by Coco Harness.'
...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are Coco Harness.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are Coco Harness.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
