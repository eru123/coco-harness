 ...
 * execution pipeline.
 * @module @coco-harness/cch-tools
 */

import { Context, Service } from '@coco-harness/cordis'
import z from '@coco-harness/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@coco-harness/cch-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@coco-harness/cch-scope'
import type { CallId, ContentBlock, ToolSchema } from '@coco-harness/cch-llm'
import { assertNever, deepFreeze, HarnessError } from '@coco-harness/cch-llm'
import type { Agent } from '@coco-harness/cch-agent'
import { snapshotJsonValue } from '@coco-harness/cch-session'
import type { JsonValue, UserMessage } from '@coco-harness/cch-session'
import type { ToolProviderResult } from '@coco-harness/cch-system-prompt'
import type { CodeRuntime } from '@coco-harness/cch-code-runtime'
 ...
 * instead of deriving them: the seam's `cch-code-runtime` README pair, its
...
export type { JsonValue } from '@coco-harness/cch-session'
...
// lives in its own UI-facing module; re-export it so `@coco-harness/cch-tools`
...
declare module '@coco-harness/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@coco-harness/cch-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@coco-harness/cch-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@coco-harness/cch-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `cch-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`cch-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`cch-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @coco-harness/cch-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`cch-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `cch-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('cch.tool.execution') as ToolExecutionToken
}
