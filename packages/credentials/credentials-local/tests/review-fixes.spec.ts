...
const ALPHA = credentialRef('CCH_REVIEW_ALPHA')
const BETA = credentialRef('CCH_REVIEW_BETA')
const INNER = credentialRef('CCH_REVIEW_INNER')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cch-cred-review-'))
...
describe('document editor', () => {
  it('leaves a sibling multi-line value untouched while patching one entry', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    const wrapped = `CCH_REVIEW_WRAPPED: |-\n  line1\n  line2\n${ALPHA}: a\n`
    await writeCredentials(path, wrapped)
    const ctx = await boot({ path, watch: false })
    await ctx.credentials.set(ALPHA, 'b')
    expect(await readFile(path, 'utf8')).toBe(`CCH_REVIEW_WRAPPED: |-\n  line1\n  line2\n${ALPHA}: b\n`)
    expect(await ctx.credentials.resolve(credentialRef('CCH_REVIEW_WRAPPED')))
      .toEqual({ value: 'line1\nline2', source: 'file' })
  })
