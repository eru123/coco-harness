...
describe('credentials invariant companion', () => {
  ...
  it('fails an update event emitted without a live service', async () => {
    ...
    expect(() => { ctx.emit('credentials/updated', REF) }).toThrow(/invariant violated by "@coco-harness\/cch-credentials"/)
  })
  ...
  it('reserves the package name against duplicate registration', async () => {
    ...
    expect(() => {
      ctx.invariants.register('@coco-harness/cch-credentials', () => {})
