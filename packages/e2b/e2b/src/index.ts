...
export function e2bControlEnvs(
  ...
  return { ...overrides, HOME: `/.cch-e2b-control-${randomUUID()}` }
}
...
export class E2BRuntime extends Service {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.runtimeRoot = posix.join(this.cwd, '.cch-e2b')
  ...
  private validate(): void {
    if (this.config.apiKey.length === 0) {
      throw new Error('cch-e2b: configure apiKey or set E2B_API_KEY')
    }
    if (!posix.isAbsolute(this.config.cwd)) {
      throw new Error(`cch-e2b: cwd must be an absolute Linux path: ${this.config.cwd}`)
    }
    if (!Number.isFinite(this.config.timeoutMs) || this.config.timeoutMs <= 0) {
      throw new Error('cch-e2b: timeoutMs must be a positive finite number')
    }
  }
  ...
  private async open(): Promise<Sandbox> {
    ...
    try {
      ...
      if (runtimeRoot.type !== FileType.DIR || runtimeRoot.symlinkTarget !== undefined) {
        throw new Error(`cch-e2b: runtime root must be a real directory: ${this.runtimeRoot}`)
      }
