...
describe('E2BRuntime', () => {
  it('gives each SDK login shell a fresh non-overridable control home', () => {
    ...
    expect(first.HOME).toMatch(/^\/\.cch-e2b-control-/)
  ...
  it('creates one protected shared sandbox and kills it on default disposal', async () => {
    ...
    expect(service.runtimeRoot).toBe('/home/user/workspace/.cch-e2b')
    ...
    expect(fixture.makeDir).toHaveBeenNthCalledWith(2, '/home/user/workspace/.cch-e2b')
    expect(fixture.getInfo).toHaveBeenCalledWith('/home/user/workspace/.cch-e2b')
    const runOptions = fixture.run.mock.calls[0]?.[1]
    expect(runOptions?.envs?.HOME).toMatch(/^\/\.cch-e2b-control-/)
    expect(fixture.run).toHaveBeenCalledWith(
      "chmod 700 -- '/home/user/workspace/.cch-e2b'",
