 ...
 * @module @coco-harness/cch-fs-e2b
 */

import { createHash, randomUUID } from 'node:crypto'
import { Buffer } from 'node:buffer'
import { posix } from 'node:path'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@coco-harness/cch-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import type { EntryInfo, Sandbox } from '@coco-harness/cch-e2b'

const VERSION_METADATA_KEY = 'cch-version'
...
export class E2BFileSystem extends FileSystem {
  ...
  private async writeAtomic(
    ...
    const stagingDirectory = posix.join(posix.dirname(targetPath), `.cch-${randomUUID()}.tmp`)
