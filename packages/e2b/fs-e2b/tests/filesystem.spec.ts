...
import { FsTargetKey, FsVersion } from '@coco-harness/cch-fs'
import E2BFileSystem from '@coco-harness/cch-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.cch-e2b-control-/)
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['cch-version']).toBeDefined()
