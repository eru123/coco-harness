...
import { e2bControlEnvs } from '@coco-harness/cch-e2b'
import type { Sandbox } from '@coco-harness/cch-e2b'
import { SENSITIVE_ENV_PATTERN } from '@coco-harness/cch-subprocess'
...
export async function readRemoteEnvironment(sandbox: Sandbox, signal?: AbortSignal): Promise<string> {
  ...
  const result = await sandbox.commands.run(
    'set -o pipefail; cch_e2b_passwd="$(getent passwd "$(id -u)")"; IFS=: read -r _ _ _ _ _ cch_e2b_home _ <<<"$cch_e2b_passwd"; test -n "$cch_e2b_home" -a -d "$cch_e2b_home"; printf \'%s\' "$cch_e2b_home" | base64 -w 0; printf \'\\n\'; env -0 | base64 -w 0',
...
export function scrubRemoteEnvironment(raw: string): Map<string, string> {
  ...
  for (const [name, value] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('CCH_') || SENSITIVE_ENV_PATTERN.test(name)) continue
...
export function bootstrapEnvironment(raw: string): Record<string, string> {
  ...
  for (const [name] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('CCH_') || SENSITIVE_ENV_PATTERN.test(name)) environment[name] = ''
  }
  return environment
}
