...
import type { CommandHandle, CommandResult, Sandbox } from '@coco-harness/cch-e2b'
import type {
  SubprocessOutcome,
...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'cch_state=$1',
  'mapfile -d \'\' -t cch_env < "$cch_state/environment"',
  'mapfile -d \'\' -t cch_argv < "$cch_state/argv"',
  'cch_output_marker=$(<"$cch_state/output-marker")',
  'rm -f -- "$cch_state/environment" "$cch_state/argv" "$cch_state/output-marker" "$cch_state/runner.bash"',
  'if (( ${#cch_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$cch_output_marker"',
  'exec env -i -- "${cch_env[@]}" "${cch_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`cch-e2b-bootstrap:${randomUUID()}`)
