# @coco-harness/cch-acp-demo

ACP automation server app: the default agent spine, client-created agents through [`@coco-harness/cch-acp`](../../acp/acp/README.md), JSONL persistence, and semantic checkpointing behind one JSON-RPC stdio bin. Programmatic clients create fresh sessions; this package mounts no human UI.

## Composition

| Plugin | Role |
|---|---|
| `@coco-harness/cch-agent-spine-demo` | Providerless agent spine with no pre-created agents; `session/new` creates each agent. |
| `@coco-harness/cch-session-persistence-jsonl` | Durable session logs used by checkpointing, observability, and snapshot replay. |
| `@coco-harness/cch-session-checkpoint-policy` | Durability barriers before model calls and top-level tool effects, plus completed-step checkpoints. |
| `@coco-harness/cch-session-query-sqlite` | Derived exact/FTS session-query service, opened before the ACP transport so leaf consumers are ready for the first model request. |
| `@coco-harness/cch-acp` | Automation-only ACP transport over stdin/stdout. |
...
| `persona` | — | Deployment persona template for `cch-system-prompt`. |
| `toolOrder` | lexicographic | Explicit model-facing tool order for `cch-system-prompt`. |
| `tools` | `{ mode: 'native' }` | Native, Code Mode, or combined model tool transport. |
| `cchHome` | `$CCH_HOME` or `~/.cch` | Harness home shared by bash and local skill discovery. |
...
`cch-acp-demo [--config path-to-cordis.yml]` (short form `-c`; default `./cordis.yml`) loads the gitignored `.env`, except in replay mode; `CCH_SNAPSHOT=replay` selects the sibling `cordis.snapshot.yml`; stdin EOF disposes the context and flushes sessions before exit. Loader's installed optional `node-addon-require-builtin` peer resolves bare plugin specifiers for the built bin under plain Node. Diagnostics use stderr because stdout is the ACP wire.

## Model Experience

Indirectly, through `cch-agent-spine-demo` and the leaf's model-facing plugins. ACP prompt text becomes the ordinary logged user message; protocol metadata and permission choices do not enter the model request.
