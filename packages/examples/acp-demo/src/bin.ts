 ...
 * `cch-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in cch-app-boot. Replay skips `.env` and selects sibling
 ...
 * @module @coco-harness/cch-acp-demo/bin
 */

import { parseArgs } from 'node:util'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@coco-harness/cch-app-boot'

const NAME = 'cch-acp-demo'

/* v8 ignore start -- thin self-executing composition over the unit-tested
   cch-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
installFailLoud(NAME)
const snapshotMode = process.env['CCH_SNAPSHOT']
