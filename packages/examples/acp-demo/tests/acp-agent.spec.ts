...
import { agentEvents } from '@coco-harness/cch-agent'
import { TOOL_ORDER_REST } from '@coco-harness/cch-system-prompt'
import type { Message } from '@coco-harness/cch-llm'
import { SessionId } from '@coco-harness/cch-session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @coco-harness/cch-acp-demo composition:
...
async function mount(config: acpAgent.Config, withBash = false): Promise<Context> {
  ...
  config.persistenceRoot ??= await mkdtemp(join(tmpdir(), 'cch-acp-demo-persistence-'))
...
async function isolatedSkillsConfig(catalogDescriptionMaxLength?: number): Promise<NonNullable<acpAgent.Config['skills']>> {
  const home = await mkdtemp(join(tmpdir(), 'cch-acp-demo-skills-'))
  return {
    filesystem: { cchHome: join(home, '.cch'), agentsHome: join(home, '.agents') },
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  const home = await mkdtemp(join(tmpdir(), 'cch-acp-demo-default-skills-'))
  process.env.CCH_HOME = join(home, '.cch')
  process.env.CCH_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
...
describe('cch-acp-demo composition', () => {
  it('brings up the spine + persistence + the ACP bridge', async () => {
    const ctx = await mount({
      provider: 'mock',
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/cch-acp-demo-test',
  ...
  it('forwards explicit project-instruction controls to the bundled spine', async () => {
    const ctx = await mount({
      ...
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/cch-acp-demo-workspace-context',
      workspaceContext: false,
    })
    expect(ctx.get('agents')).toBeDefined()
    expect(ctx.get('agentLoop')).toBeDefined()
  ...
  it('forwards skill config and cchHome into agent-spine-demo', async () => {
    const skills = await isolatedSkillsConfig(6)
    const ctx = await mount({ provider: 'mock', model: 'mock', persona: 'hi', cchHome: skills.filesystem!.cchHome!, skills, workspaceContext: false })
  ...
  it('forwards maxParallelToolCalls to the bundled agent loop', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/cch-acp-demo-test-parallel',
      skills: await isolatedSkillsConfig(),
      workspaceContext: false,
    })
  ...
  it('forwards toolOrder through agent-spine-demo to the system-prompt assembly', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/cch-acp-demo-test-tool-order',
      workspaceContext: false,
    })
