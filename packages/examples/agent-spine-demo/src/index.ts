 ...
 * @module @coco-harness/cch-agent-spine-demo
 */

import type { Context } from '@coco-harness/cordis'
import Timer from '@coco-harness/cordis-plugin-timer'
...
import SessionTitleService, { type Config as SessionTitleConfig } from '@coco-harness/cch-session-title'
import SystemPrompt, { type Config as SystemPromptConfig } from '@coco-harness/cch-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@coco-harness/cch-tools'
import SkillRegistry, { type Config as SkillRegistryConfig } from '@coco-harness/cch-skill'
import * as SkillFileSystem from '@coco-harness/cch-skill-filesystem'
import AgentRegistry from '@coco-harness/cch-agent'
import GoalService, { type Config as GoalDomainConfig } from '@coco-harness/cch-goal'
import * as goalSession from '@coco-harness/cch-goal-round-driver'
import * as toolGoal from '@coco-harness/cch-tool-goal'
import LocalJobRegistry, { type Config as JobsConfig } from '@coco-harness/cch-jobs-local'
import InvariantRegistry, { type Config as InvariantConfig } from '@coco-harness/cch-invariants'
...
import * as bashEnv from '@coco-harness/cch-shell-env'
 ...
 * `cchHome` to bash environment and local skill discovery, `sessionTitle` to
...
export interface Config {
  /** The agent-loop `agents` list (see cch-agent-loop's `Config`). */
  ...
  /** The deployment persona (see cch-system-prompt's `Config`). */
  persona?: SystemPromptConfig['persona']
  /** The explicit model-facing tool order (see cch-system-prompt's `Config`). */
  toolOrder?: SystemPromptConfig['toolOrder']
  /** The tool registry's config — its presentation `mode` (see cch-tools' `Config`). */
  tools?: ToolsConfig
  /** Coco Harness home directory shared by shell context and local skill discovery. */
  cchHome?: string
...
export const Config = z.intersect([
  ...
  }) as unknown as z<Pick<Config, 'tools' | 'cchHome' | 'sessionTitle' | 'skills' | 'workspaceContext' | 'toolBash' | 'jobs' | 'toolJobs' | 'invariants' | 'goals'>>,
...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.cchHome !== undefined ? { cchHome: config.cchHome } : {},
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (skillsEnabled) {
    ...
    ctx.plugin(SkillFileSystem, Object.assign({}, config.skills?.filesystem, { cchHome }))
  }
