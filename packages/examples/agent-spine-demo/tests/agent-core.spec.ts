...
import { renderPrompt, TOOL_ORDER_REST } from '@coco-harness/cch-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@coco-harness/cch-agent'
...
import * as scopeInvariant from '@coco-harness/cch-scope/invariant'
import * as agentLoopInvariant from '@coco-harness/cch-agent-loop/invariant'

const testToolSignal = new AbortController().signal

declare module '@coco-harness/cch-jobs' {
...
/**
 * Unit coverage for the @coco-harness/cch-agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  ...
  try {
    ...
    if (oldAgentsHome === undefined) {
      ...
      process.env.CCH_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('cch-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@coco-harness/cch-agent$'] },
      { package_blocklist: ['^@coco-harness/cch-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by Coco Harness.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-skill-custom-'))
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-skill-refresh-'))
    const home = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-skill-refresh-home-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('shares top-level cchHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'cch-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'cch-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-cch-home-test') as ToolExecution['token'],
      callId: CallId('agent-core-cch-home'),
      rootCallId: CallId('agent-core-cch-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ CCH_HOME: home, CCH_SHELL: '1' })
    await ctx.fiber.dispose()
  })
  ...
  it('rejects conflicting global and nested CCH home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        ...
        workspaceContext: false,
        skills: { filesystem: { cchHome: '/nested-cch-home' } },
      })
    }).toThrow('agent-spine-demo: cchHome and skills.filesystem.cchHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cch-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
