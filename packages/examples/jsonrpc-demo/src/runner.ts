 ...
 *
 * @module @coco-harness/cch-sdk-jsonrpc-demo/runner
 */
...
export async function runJsonrpcAgent(bareModuleBaseUrl?: string): Promise<void> {
  ...
  const fromEnv = process.env['CCH_CORDIS_CONFIG']
  ...
  if (configPath === undefined || !existsSync(configPath)) {
    process.stderr.write(
      `usage: ${NAME} <path/to/cordis.yml> (or set CCH_CORDIS_CONFIG=<path>, which wins); the config is required — there is no built-in fallback\n`,
    )
    process.exit(1)
  }
