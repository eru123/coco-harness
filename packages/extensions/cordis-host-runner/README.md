# @coco-harness/cch-cordis-host-runner

The host half of model-mounted dynamic packages: the definition registry, the `node:vm` sandbox and fiber lifecycle for host halves, the invoke handler table, and the run round trip a browser page carries out. Provided as `ctx.dynamicCordisRunner`. The model-facing tools live in [`@coco-harness/cch-tool-cordis`](../tool-cordis/README.md); the browser half is loaded by [`@coco-harness/cch-cordis-client-runner`](../cordis-client-runner/README.md).
