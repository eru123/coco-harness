...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    description: 'Agent service (`ctx.agents`): tracks live agents and carries the initiating Agent through one process-local asynchronous driver chain. Agent *creation* is provided by whichever plugin implements the AgentFactory (`@coco-harness/cch-agent-loop`), registered via setFactory.\n\nInitiator methods provide same-process causal attribution only. Ambient presence is neither liveness proof nor authorization; subjects and owners remain explicit, as does identity at worker, process, persistence, and wire boundaries. Returned Promise boundaries drain during teardown, except a nested lineage that starts an owning-fiber unload is excluded from its own drain.',
    methods: [
      {
  ...
  {
    key: 'clientModules',
    summary: 'The web plugin table service: incremental `cch.client` scan + wire composition + bundle route + index tap.',
  ...
  {
    key: 'shellEnv',
    summary: 'Registry (`ctx.shellEnv`) for trusted, per-execution `CCH_*` variables.',
    ...
    methods: [
      ...
      {
        signature: 'collect(execution: ToolExecution): CchEnvironment',
        description: 'Build the trusted `CCH_*` snapshot for one shell tool execution.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.agent - the newly registered agent with its live session and completed setup. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/disposed',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.agent - the exact agent removed from the registry. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/error',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.error - the failure, verbatim. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.turn - the owning turn. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the discarded message. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the inserted message. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/pre-step',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s cancellation signal. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/request',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s explicit abort signal. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.signal - the turn abort signal. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.source - why the session started (fresh startup, resume, …). Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/status',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.status - the status just entered (the transition\'s destination). Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Ask composed answerers for one decision. Return an outcome to claim the request or call `next()`; failure yields the fail-closed default. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.',
    parameters: [{ name: 'req', description: 'the pending decision (agent, tool identity, reason, signal).' }],
  },
  {
  ...
  },
  {
    name: 'goal/changed',
    mode: 'emit',
    signature: '\'goal/changed\'(this: import(\'@coco-harness/cch-scope\').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void',
    summary: 'Goal mutation accepted by one live agent.',
    description: 'Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@coco-harness/cch-scope`): agent-scoped listeners receive only that agent.',
  ...
  },
  {
    ...
    description: 'Emitted once when an announced session leaves the store, including publication rollback, but never for an entry whose creation announcement did not begin. Listener failures are logged and contained. Scope-filtered dispatch (`@coco-harness/cch-scope`) reuses the owner scope.',
  ...
  },
  {
    ...
    description: 'Awaited parallel durability checkpoint: every listener runs and the caller awaits all of them, with no waterfall veto. Scope-filtered dispatch (`@coco-harness/cch-scope`) reuses the session\'s owner scope.',
    parameters: [{ name: 'session', description: 'the session whose buffered events must reach durable storage.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Observe the frozen, lossless-JSON final outcome. Listener failures are contained. Scope-filtered dispatch (`@coco-harness/cch-scope`): keyed by `exec.agent`.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'SkillSource',
    declaration: 'export type SkillSource = \'project-cch\' | \'project-agents\' | \'runtime\' | \'user-cch\' | \'user-agents\' | \'custom\' | \'bundled\' | (string & {});',
  },
  {
    name: 'SkillSummary',
