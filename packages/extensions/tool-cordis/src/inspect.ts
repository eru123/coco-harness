 ...
 * `name` adds.
 * @module @coco-harness/cch-tool-cordis/inspect
 */
...
import type {} from '@coco-harness/cch-cordis-host-runner'
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  if (rows.length === 0) {
    return ['No dynamic Plugins are defined in this session. Definitions live only in this process\'s memory, so a CCH restart clears them.']
  }
