# @coco-harness/cch-message-feedback
...
Public request, value, version, and failure types are exported from the package root and `@coco-harness/cch-message-feedback/types`; [`src/types.ts`](src/types.ts) is their source.
