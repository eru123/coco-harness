...
// Load it alongside a ctx.fs provider (e.g. @coco-harness/cch-fs-local) and the
// @coco-harness/cch-tool-fs tools; the tools dispatch the fs/* events this plugin
...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@coco-harness/cch-tool-fs` | model-facing schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@coco-harness/cch-fs-observation-policy` (this) | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@coco-harness/cch-fs` | `ctx.fs`: text IO + atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@coco-harness/cch-fs-local` | local implementation of `ctx.fs` |

## How the gate participates

Three `fs/*` events (declared by `@coco-harness/cch-fs`, dispatched by `@coco-harness/cch-tool-fs`):
...
Because the plugin influences the world only through events, removing it does not break `@coco-harness/cch-tool-fs` at a service-injection boundary: the tool falls through to the bare `ctx.fs` provider (unconditional write/edit, no observed-state). Loading it back layers the policy on. That graceful add/remove is the whole point of the event gate over a mandatory method service.
