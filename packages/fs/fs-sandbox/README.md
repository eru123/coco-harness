# cch-fs-sandbox — the sandbox-enforcing filesystem backend
...
Loading it INSTEAD OF `cch-fs-local`, together with a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/README.md), is the whole swap; the model-facing tools (`cch-tool-fs`) are untouched. The tool layer resolves the calling session's mode and cwd into the SAME per-call policy bash receives, so the two families never confine to different roots.
...
The policy owner contributes capability-neutral `sandbox:policy` context. Indirectly, `cch-tool-fs` renders this backend's `FS_SANDBOX_DENIED` refusals as the `[sandbox: file access denied under <mode> mode]` marker plus the same-turn escalation hint.
