 ...
 * `@coco-harness/cch-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@coco-harness/cch-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@coco-harness/cch-tool-fs`),
 * exactly as bash's does.
 *
 * @module @coco-harness/cch-fs-sandbox
 */

import { Context } from '@coco-harness/cordis'
import { LocalFileSystem } from '@coco-harness/cch-fs-local'
import type { Config as LocalConfig } from '@coco-harness/cch-fs-local'
import { FsError } from '@coco-harness/cch-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@coco-harness/cch-fs'
import { writableRoots } from '@coco-harness/cch-sandbox'
 ...
 * INSTEAD OF `cch-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `cch-tool-fs` resolves
