...
import { FsError, FsTargetKey } from '@coco-harness/cch-fs'
import type { FsTarget } from '@coco-harness/cch-fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.cch-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'cch-fssbx-tmp-')), 'temp.txt')
