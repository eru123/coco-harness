...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@coco-harness/cch-tool-fs` | model-facing `read`/`write`/`edit` schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@coco-harness/cch-fs-observation-policy` | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@coco-harness/cch-fs` (this) | `ctx.fs`: execution-world paths, text IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@coco-harness/cch-fs-local` | the host-filesystem implementation |
...
Indirectly, through `cch-tool-fs`, which renders provider text and errors as bounded, retained filesystem tool results.
