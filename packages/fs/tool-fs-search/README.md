# @coco-harness/cch-tool-fs-search
...
await ctx.plugin(LocalSubprocessRuntime)                     // @coco-harness/cch-subprocess-local
...
await ctx.plugin(LocalSpillStore)                           // @coco-harness/cch-spill-local
```
...
| `timeoutMs` | `30000` | Cooperative tool-call budget attached to both tool definitions, enforced by `@coco-harness/cch-tool-call-timeout-policy` through `exec.signal`; the subprocess seam's terminate escalation is the hard kill. |
...
The glob description states the configured over-cap ordering. The generated [`glob` and `grep` schemas](../../../docs/tool-catalog.md#coco-harnesscch-tool-fs-search) use `sampleOverCapGlobResults: true`; the tools are registered unconditionally.
