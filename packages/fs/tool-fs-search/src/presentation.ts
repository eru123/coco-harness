 ...
 * model-facing render consumes ({@link module:@coco-harness/cch-tool-fs-search/search-core}
 ...
 * final output budget (`cch-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @coco-harness/cch-tool-fs-search/presentation
 */
 ...
 * ({@link module:@coco-harness/cch-tool-fs-search/grep} `formatGrepMatches`), so
