...
import { ItemRetainer, TextRetainer } from '@coco-harness/cch-output-retention'
import type { RetainedItems } from '@coco-harness/cch-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@coco-harness/cch-subprocess'
import type { SaveTextSpill, SpillRef } from '@coco-harness/cch-spill'
 ...
 * `@coco-harness/cch-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`cch-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * retention pass both the model-facing render ({@link module:@coco-harness/cch-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@coco-harness/cch-tool-fs-search/presentation} `grepSearchMeta`)
