...
// caller-selected session workspace wins, matching cch-tool-bash.
describe('per-session cwd', () => {
  let sessionDir: string
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'cch-tool-fs-cfg-'))
    sessionDir = await mkdtemp(join(tmpdir(), 'cch-tool-fs-session-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
