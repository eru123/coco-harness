# @coco-harness/cch-hook-protocol

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@coco-harness/cch-hooks-claude-code`, `@coco-harness/cch-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`cch-hook-protocol`) | The bridge (`cch-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `cch-hooks-claude-code` and `cch-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
