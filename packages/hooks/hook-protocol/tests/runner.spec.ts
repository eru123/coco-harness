...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@coco-harness/cch-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@coco-harness/cch-hook-protocol'
import type { RunHookOptions } from '@coco-harness/cch-hook-protocol'
 ...
 * executor (cch-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what cch-bash-local's resolve does for the fields runHook sets.
      return {
