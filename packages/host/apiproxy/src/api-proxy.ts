...
import { installModelSelection } from '@coco-harness/cch-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@coco-harness/cch-agent'
import type {} from '@coco-harness/cch-agent-presets/types'
import { AttachmentError } from '@coco-harness/cch-attachment'
import type { ImageAttachmentRef } from '@coco-harness/cch-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@coco-harness/cch-llm'
import { errorChain } from '@coco-harness/cch-llm'
import type { ContentBlock, MessageSource } from '@coco-harness/cch-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@coco-harness/cch-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@coco-harness/cch-session'
import type { SessionPersistence } from '@coco-harness/cch-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@coco-harness/cch-session-query'
import { SubagentError } from '@coco-harness/cch-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@coco-harness/cch-subagent'
import { isUserInvocable } from '@coco-harness/cch-skill'
import type { Workspace, WorkspaceRecord } from '@coco-harness/cch-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@coco-harness/cch-workspace'
...
import type { PresetBearingSession } from '@coco-harness/cch-agent-presets'
...
import type { SessionRawArtifact } from '@coco-harness/cch-session-persistence'
import {
...
import type {} from '@coco-harness/cch-jobs'
import type { JobSnapshot } from '@coco-harness/cch-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@coco-harness/cch-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@coco-harness/cch-goal'
import type { GoalRef as CoreGoalRef } from '@coco-harness/cch-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@coco-harness/cch-commands'
...
import type {} from '@coco-harness/cch-cordis-host-runner/types'
import type {} from '@coco-harness/cch-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@coco-harness/cch-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@coco-harness/cch-settings'
import { credentialRef } from '@coco-harness/cch-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@coco-harness/cch-session-title'
import type { CallId } from '@coco-harness/cch-llm/brand'
import type { ScopeKey } from '@coco-harness/cch-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@coco-harness/cch-user-approval'
...
} from '@coco-harness/cch-user-questions'
import { UserQuestionError } from '@coco-harness/cch-user-questions'
import { DirectoryPickerError } from '@coco-harness/cch-host-directory-picker'
import {
...
} from '@coco-harness/cch-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @coco-harness/cch-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: cch-llm depends on cch-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @coco-harness/cch-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @coco-harness/cch-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @coco-harness/cch-credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @coco-harness/cch-session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in cch-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts cch-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @coco-harness/cch-skill', details: {} })
        }
