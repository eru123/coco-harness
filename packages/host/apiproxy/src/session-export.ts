...
import type { AttachmentStore, ImageAttachmentRef } from '@coco-harness/cch-attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@coco-harness/cch-session-query'
import type { SessionId, SessionStore } from '@coco-harness/cch-session'
import type { SessionPersistence, SessionRawArtifact } from '@coco-harness/cch-session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `cch-session-${safeSessionIdSegment(sessionId)}.zip`
}
