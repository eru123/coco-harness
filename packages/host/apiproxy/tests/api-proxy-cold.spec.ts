...
import { TypertLookupFailure } from '@coco-harness/cch-typert-protocol'
import TypertRegistry from '@coco-harness/cch-typert-registry'
import { createUserMessage, MessageId } from '@coco-harness/cch-llm'
import type { Agent } from '@coco-harness/cch-agent'
import UserQuestionService from '@coco-harness/cch-user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@coco-harness/cch-session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@coco-harness/cch-session-persistence'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'cch-cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'cch-cold-race-'))
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@coco-harness/cch-session').Session
