...
import type { ApiProxy, GoalRef, HostFrame, MuxFrame, RpcMessage, RpcRequest, RpcResponse } from '@coco-harness/cch-host-apiproxy'
import { InProcessApiClient, RpcId, toFetchHandler } from '@coco-harness/cch-host-apiproxy'
...
describe('unary round trip', () => {
  ...
  it('rejects a method/path mismatch as bad-request', async () => {
    ...
    const response = await handler.fetch('http://cch.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) })
  ...
  it('rejects a malformed envelope as bad-request, salvaging the rpcId or falling back to the sentinel', async () => {
    ...
    const noId = await handler.fetch('http://cch.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ nonsense: true }) })
    ...
    const withId = await handler.fetch('http://cch.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ rpcId: 'salvage-me', nonsense: true }) })
  ...
  it('maps carrier failures to HTTP statuses and the client throws transport failure', async () => {
    const handler = toFetchHandler(scriptedApi())
    // Unknown method → 404.
    const notFound = await handler.fetch('http://cch.internal/api/no.such', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })
    expect(notFound.status).toBe(404)
    // Non-JSON body → 400.
    const badBody = await handler.fetch('http://cch.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{oops' })
  ...
  it('rejects non-JSON media types before executing anything (cross-site simple-request fence)', async () => {
    ...
    const plain = await handler.fetch('http://cch.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'text/plain' }, body })
    expect(plain.status).toBe(415)
    // A string body with no explicit header defaults to text/plain — same fence.
    const unlabelled = await handler.fetch('http://cch.internal/api/session.list', { method: 'POST', body })
    expect(unlabelled.status).toBe(415)
    expect(list).not.toHaveBeenCalled()
    // Media-type parameters pass: the fence checks the type, not the exact string.
    const charset = await handler.fetch('http://cch.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json; charset=utf-8' }, body })
  ...
  it('passes a signal-less doFetch straight through to the handler', async () => {
    ...
    const response = await probe.direct(new URL('http://cch.internal/probe'))
...
describe('respond path', () => {
  ...
  it('returns bad-response for a malformed client-response without reaching the impl', async () => {
    ...
    const response = await handler.fetch('http://cch.internal/api/respond', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'client-response' }) })
