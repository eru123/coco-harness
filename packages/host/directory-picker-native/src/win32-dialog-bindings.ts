...
export async function loadWin32DialogBindings(): Promise<Win32DialogBindings> {
  ...
  const protoShow = koffi.proto('int32 __stdcall CchDialogShow(void *self, void *owner)')
  const protoSetOptions = koffi.proto('int32 __stdcall CchDialogSetOptions(void *self, uint32 options)')
  const protoSetTitle = koffi.proto('int32 __stdcall CchDialogSetTitle(void *self, str16 title)')
  const protoGetResult = koffi.proto('int32 __stdcall CchDialogGetResult(void *self, _Out_ void **item)')
  const protoGetDisplayName = koffi.proto('int32 __stdcall CchItemGetDisplayName(void *self, int32 form, _Out_ void **name)')
  const protoRelease = koffi.proto('uint32 __stdcall CchComRelease(void *self)')
...
export async function closeThreadWindows(threadId: number): Promise<void> {
  ...
  const protoEnumProc = koffi.proto('int __stdcall CchEnumThreadWndProc(void *hwnd, intptr lparam)')
