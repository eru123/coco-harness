# @coco-harness/cch-anonymous-user-id

Shared anonymous identity for session telemetry, direct feedback acknowledgement, and DeepSeek provider requests. `getOrCreateAnonymousUserId()` returns a random UUID v4 scoped to one harness home, persisted as the bare line `$CCH_HOME/.anonymous-user-id` (`~/.cch/.anonymous-user-id` when `CCH_HOME` is unset). The OpenTelemetry backend reports it as Resource `user.id`; `/feedback` includes the same value in its acknowledgement; and `cch-llm-deepseek` sends it as `x-coco-harness-user-id`, allowing the receiving systems to correlate records without independently generated identities.
...
- **No cross-home identity** — different `$CCH_HOME` values cannot be correlated.
- **Configured DeepSeek gateways receive the id** — `cch-llm-deepseek` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
