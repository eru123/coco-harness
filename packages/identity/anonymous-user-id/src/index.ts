 ...
 * harness home resolved by {@link resolveCchHome} (`$CCH_HOME` > `~/.cch`),
 ...
 * machine: every process sharing one `$CCH_HOME` reports the same id, and
 ...
 *
 * @module @coco-harness/cch-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `CCH_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveCchHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
