...
import { bindScopeParent, createScope, scopeOf } from '@coco-harness/cch-scope'
import type { ScopeKey } from '@coco-harness/cch-scope'
import { JobId } from '@coco-harness/cch-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@coco-harness/cch-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @coco-harness/cch-tool-jobs in its composition)')
  })
