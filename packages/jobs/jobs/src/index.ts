 ...
 * process-local registry lives in `@coco-harness/cch-jobs-local`.
 * @module @coco-harness/cch-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@coco-harness/cch-jobs is the abstract job registry seam; load an implementation such as @coco-harness/cch-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
