...
import { JobId, JobRegistry } from '@coco-harness/cch-jobs'
import type {
 ...
 * behavior suite lives with `@coco-harness/cch-jobs-local`.
 */
...
describe('JobRegistry seam', () => {
  ...
  it('mounting the abstract seam directly fails loudly at load (stale-composition fence)', async () => {
    ...
    await expect(ctx.plugin(JobRegistry as unknown as typeof StubJobRegistry))
      .rejects.toThrow(/abstract job registry seam; load an implementation such as @coco-harness\/cch-jobs-local/)
  })
})
