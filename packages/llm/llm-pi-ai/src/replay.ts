 ...
 *
 * @module cch-llm-pi-ai/replay
 */

import { LlmError } from '@coco-harness/cch-llm'
import type { Message, ModelMessageSource } from '@coco-harness/cch-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'cch-foreign',
    provider: source?.provider ?? 'cch-foreign',
    model: source?.model ?? 'cch-foreign',
    usage: emptyPiUsage(),
