...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@coco-harness/cch-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@coco-harness/cch-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'cch-llm-retry-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@coco-harness/cch-llm-retry', retry],
    ['@coco-harness/cch-agent-loop', AgentLoop],
  ])
