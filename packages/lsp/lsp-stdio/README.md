...
| `env` | `{}` | Extra env merged on top of the credential-scrubbed ambient env (vars matching `KEY`/`PASSWORD`/`SECRET`/`TOKEN` are not forwarded); an explicit `CCH_*` entry merges after the seam's scrub of ambient ones. |
...
Indirectly, through `cch-tool-lsp`, which surfaces this provider's normalized results; this host contributes no prompt or schema itself.
