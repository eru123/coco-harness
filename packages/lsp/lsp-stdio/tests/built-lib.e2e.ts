...
/**
 * Keyless built-artifact smoke: plain Node imports `@coco-harness/cch-lsp` and
 * `@coco-harness/cch-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@coco-harness/cch-lsp')
      const LspLocal = await import('@coco-harness/cch-lsp-stdio')
      const { default: LocalFileSystem } = await import('@coco-harness/cch-fs-local')
