...
import { LspConnection } from '@coco-harness/cch-lsp-stdio'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit CCH_* env entries to the child', async () => {
    // A configured CCH_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'CCH_LSP_TEST_FACT', CCH_LSP_TEST_FACT: 'managed' })
