 ...
 *
 * @module @coco-harness/cch-plan-mode
 */
...
import type { Session, SessionEvent, UserMessage } from '@coco-harness/cch-session'
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('cch-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'cch-plan-mode: close service lifetime')
