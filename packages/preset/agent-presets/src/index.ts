 ...
 * @module @coco-harness/cch-agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context, Service } from '@coco-harness/cordis'
import z from '@coco-harness/schemastery'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@coco-harness/cch-scope'
// Type-only: resolves the `agent/created` lifecycle event this service watches.
import type {} from '@coco-harness/cch-agent'
import { settingsNamespace, type SettingsScope, type default as SettingsService } from '@coco-harness/cch-settings'
...
export class AgentPresets extends Service {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = config.includeUserRoot
      ? [...config.roots, { path: cchHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...config.roots]
   ...
   * scope key. The binding is cch-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `cch-skill-filesystem` watches its
