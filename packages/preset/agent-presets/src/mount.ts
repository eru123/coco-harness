 ...
 * @module @coco-harness/cch-agent-presets/mount
 */
...
import type { EntryTree } from '@coco-harness/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@coco-harness/cch-scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@coco-harness/cch-*` row would fail to import. The
 ...
 * invariant companion's service listener, and `cch-invariants` is a
