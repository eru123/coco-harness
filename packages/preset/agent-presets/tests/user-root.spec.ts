 ...
 * authors into `<cchHome>/.agent-presets`, the way `cch-skill-filesystem` owns
 * `<cchHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$CCH_HOME` is repointed per test because the derived root is resolved in the
...
import AgentPresets, { COMPOSITION_FILE, type Config } from '@coco-harness/cch-agent-presets'
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'cch-preset-home-'))
...
describe('the harness-home preset root', () => {
  ...
  it('yields to a configured user root for authoring, which writableRoot takes first', async () => {
    const explicit = await mkdtemp(join(tmpdir(), 'cch-preset-explicit-'))
    const ctx = await roster({
      roots: [
