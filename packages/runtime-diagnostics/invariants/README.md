...
Session itself owns immutable, surface-valid log storage in every composition: it takes one lossless JSON snapshot of each candidate, validates complete cited source-event coverage and positional replacement, restricts `tool/result` replacement to one current result's `content`, deep-freezes the accepted record, and exposes the log through immutable array snapshots. The `cch-session` invariant companion checks the remaining cross-record rules that Session does not own.
...
| Companion | Checks |
|---|---|
| `cch-session`, `cch-agent`, `cch-scope`, `cch-agent-loop` | Session enclosure and call/result trace, agent-status transitions, inbox FIFO conservation, scoped subjects, and model-request reconstruction. |
| `cch-llm`, `cch-llm-retry`, `cch-tools`, `cch-system-prompt` | Stream grammar, durable retry position and bounds, tool-pipeline stages and frozen results, and authoritative prompt-assembly data. |
| `cch-compaction`, `cch-hook-protocol`, `cch-sandbox-policy` | Durable compaction and hook pairing, compaction metadata, and sandbox-mode vocabulary. |
| `cch-fs`, `cch-subagent`, `cch-workflow` | Filesystem event identity, provider/child pairing, and workflow/agent lifecycle identity. |
| `cch-goal`, `cch-goal-round-driver` | Durable goal source/content agreement, revision and lifecycle transitions, timestamps, sequential admitted rounds, and reconstructed continuation prompts. |
| `cch-permission-presets`, `cch-user-approval` | Active-preset references and approval asked/decided audit pairing. |
| `cch-jobs`, `cch-tool-todo` | Task snapshot lifecycle/ownership fields and durable whole-list todo structure. |
| `cch-time-context` | Durable clock readings agree with the session's open turn and next pre-step position and elapsed baseline; rendered time parses and does not postdate its event. |
...
ctx.plugin(InvariantRegistry, {
  ...
  package_blocklist: ['^@coco-harness/cch-agent-loop$'],
})
