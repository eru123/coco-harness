...
import InvariantRegistry, {
  InvariantError,
  type Config,
} from '@coco-harness/cch-invariants'
...
describe('InvariantRegistry selection', () => {
  it('applies defaults when constructed directly without schema normalization', async () => {
    ...
    const probe = vi.fn()
    const registration = runtimeRegistration(service.register('@coco-harness/cch-session', (child) => {
      child.on('invariants-test/ping', probe, { global: true })
    }))
    await registration
  ...
  it('enables registrations by default and treats empty lists as admit-all and exclude-none', async () => {
    for (const config of [{}, { package_allowlist: [], package_blocklist: [] }]) {
      ...
      const probe = vi.fn()
      await registerProbe(ctx, '@coco-harness/cch-session', probe)
      ctx.emit('invariants-test/ping')
      expect(probe).toHaveBeenCalledOnce()
    }
  })
  ...
  it('disables every installer while still reserving package ownership', async () => {
    ...
    const probe = vi.fn()
    const registration = await registerProbe(ctx, '@coco-harness/cch-session', probe)
    expect(() => ctx.invariants.register('@coco-harness/cch-session', () => {}))
  ...
  it('uses unanchored, case-sensitive JavaScript regex sources', async () => {
    ...
    await registerProbe(unanchored.ctx, '@coco-harness/cch-session-extra', unanchoredProbe)
    unanchored.ctx.emit('invariants-test/ping')
    expect(unanchoredProbe).toHaveBeenCalledOnce()

    const anchored = await setup({ package_allowlist: ['^@coco-harness/cch-session$'] })
    const anchoredProbe = vi.fn()
    await registerProbe(anchored.ctx, '@coco-harness/cch-session-extra', anchoredProbe)
    ...
    await registerProbe(caseSensitive.ctx, '@coco-harness/cch-session', caseProbe)
  ...
  it('lets the blocklist override an allowlist match', async () => {
    const { ctx } = await setup({
      package_allowlist: ['^@coco-harness/cch-'],
      package_blocklist: ['session'],
    })
    const sessionProbe = vi.fn()
    const agentProbe = vi.fn()
    await registerProbe(ctx, '@coco-harness/cch-session', sessionProbe)
    await registerProbe(ctx, '@coco-harness/cch-agent', agentProbe)
    ctx.emit('invariants-test/ping')
  ...
  it('accepts zero-match patterns for packages registered later', async () => {
    ...
    const now = vi.fn()
    const later = vi.fn()
    await registerProbe(ctx, '@coco-harness/cch-session', now)
  ...
  it('allows the same source in both lists and applies blocklist precedence', async () => {
    ...
    const probe = vi.fn()
    await registerProbe(ctx, '@coco-harness/cch-agent', probe)
    ctx.emit('invariants-test/ping')
    expect(probe).not.toHaveBeenCalled()
  })
})
...
describe('InvariantRegistry lifecycle', () => {
  it('honors the installer dependency API in its child fiber', async () => {
    ...
    await ctx.plugin({
      ...
      apply(child: Context) {
        ...
        registration = runtimeRegistration(child.invariants.register('@coco-harness/cch-probe', installer))
  ...
  it('attributes failures to the registering package with the stable code', async () => {
    const { ctx } = await setup()
    const registration = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-session', (child, fail) => {
    ...
    expect(caught).toMatchObject({
      name: 'InvariantError',
      code: 'INVARIANT',
      packageName: '@coco-harness/cch-session',
      message: 'invariant violated by "@coco-harness/cch-session": seq must strictly increase',
    })
  })
  ...
  it('disposes the child fiber completely and permits HMR re-registration', async () => {
    const { ctx } = await setup()
    const first = vi.fn()
    const firstRegistration = await registerProbe(ctx, '@coco-harness/cch-session', first)
    ctx.emit('invariants-test/ping')
    ...
    const second = vi.fn()
    await registerProbe(ctx, '@coco-harness/cch-session', second)
    ctx.emit('invariants-test/ping')
    expect(first).toHaveBeenCalledOnce()
  ...
  it('reserves ownership until asynchronous child disposal completes', async () => {
    ...
    const registration = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-session', (child) => {
    ...
    const replacement = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-session', () => {}))
    await replacement
    await replacement()
  })
  ...
  it('rolls back listeners and ownership atomically when an installer fails', async () => {
    const { ctx } = await setup()
    const leaked = vi.fn()
    const failed = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-session', (child) => {
    ...
    const retry = vi.fn()
    await registerProbe(ctx, '@coco-harness/cch-session', retry)
    ctx.emit('invariants-test/ping')
  ...
  it('rolls back publication effects and ownership when child-fiber publication fails', async () => {
    ...
    const failed = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-publication-probe', () => {}))
    await expect(Promise.resolve(failed)).rejects.toThrow('publication failed')
    ctx.emit('invariants-test/ping')
    expect(leaked).not.toHaveBeenCalled()
    stopRejecting()

    const retry = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-publication-probe', () => {}))
    await retry
    await retry()
  })
  ...
  it('joins asynchronous checks and rolls back their effects on failure', async () => {
    const { ctx } = await setup()
    const leaked = vi.fn()
    const failed = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-async-probe', async (child, fail) => {
      child.on('invariants-test/ping', leaked, { global: true })
    ...
    const retry = runtimeRegistration(ctx.invariants.register('@coco-harness/cch-async-probe', async () => {
      await Promise.resolve()
    }))
    await retry
    await retry()
  ...
  it('releases a synchronous reservation if the service fiber is already inactive', async () => {
    ...
    await fiber.dispose()
    expect(() => service.register('@coco-harness/cch-session', () => {})).toThrow(/inactive/i)
  })
})
