 ...
 * @module @coco-harness/cch-sandbox-local
 */
...
} from '@coco-harness/node-addon-landlock-run'
import { Context } from '@coco-harness/cordis'
import z from '@coco-harness/schemastery'
import { assertNever } from '@coco-harness/cch-llm'
import { SandboxProvider, SandboxUnavailableError } from '@coco-harness/cch-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@coco-harness/cch-sandbox'
import type { SessionId } from '@coco-harness/cch-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@coco-harness/cch-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@coco-harness/cch-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@coco-harness/cch-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@coco-harness/cch-sandbox-windows-acl/src/runner.ts'))
