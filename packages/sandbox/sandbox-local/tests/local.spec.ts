...
import { LAUNCHER_FAILURE_EXIT } from '@coco-harness/node-addon-landlock-run'
import { SANDBOX_UNAVAILABLE, SandboxUnavailableError } from '@coco-harness/cch-sandbox'
import type { SandboxPolicy } from '@coco-harness/cch-sandbox'
import {
  LocalSandboxProvider,
} from '@coco-harness/cch-sandbox-local'
import type { Config } from '@coco-harness/cch-sandbox-local'
...
function absentRunnerEntry(): string {
  return join(mkdtempSync(join(tmpdir(), 'cch-absent-acl-entry-')), 'runner.js')
}
...
function fakeLauncher(report = 'landlock: fully enforced'): string {
  const dir = mkdtempSync(join(tmpdir(), 'cch-fake-landlock-'))
...
function fakeSeatbeltExec(status: number): string {
  const dir = mkdtempSync(join(tmpdir(), 'cch-fake-seatbelt-'))
...
describe('the platform chains', () => {
  ...
  // live in @coco-harness/cch-sandbox-windows-acl/tests/provider-chain.spec.ts
...
describe('probeTimeoutMs config', () => {
  ...
  it('bounds the default probes: a launcher slower than the configured timeout reads as unusable', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'cch-slow-landlock-'))
    const launcher = join(dir, 'landlock-run')
...
describe('the windows-acl probe (runner invocation contract)', () => {
  ...
  it('prefers the built lib/runner.js entry when the resolved file exists', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'cch-fake-acl-entry-'))
