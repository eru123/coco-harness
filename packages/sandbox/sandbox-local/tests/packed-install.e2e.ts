...
const platformPackageName = `@coco-harness/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @coco-harness peer (koffi arrives
  // from the registry).
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'cch-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'cch-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'cch-packed-work-'))
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@coco-harness/node-addon-landlock-run'
