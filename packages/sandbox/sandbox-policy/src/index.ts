 ...
 * remain session-free.
 *
 * @module @coco-harness/cch-sandbox-policy
 */
...
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from '@coco-harness/cch-sandbox'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    case 'read-only':
      return 'Current CCH file policy: read-only. Any available operation enforced by the CCH file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.'
    case 'workspace-write':
      return `Current CCH file policy: workspace-write. Any available operation enforced by the CCH file sandbox may modify files under the session workspace: ${JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
    case 'danger-full-access':
      return 'Current CCH file policy: danger-full-access. The CCH file sandbox does not restrict file modifications by available operations.'
