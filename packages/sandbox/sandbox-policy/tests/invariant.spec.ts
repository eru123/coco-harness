...
import SessionStore, { type Session, type SessionEvent } from '@coco-harness/cch-session'
import InvariantRegistry, { InvariantError } from '@coco-harness/cch-invariants'
import * as SandboxPolicyInvariant from '@coco-harness/cch-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@coco-harness/cch-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@coco-harness/cch-sandbox-policy',
    })
  })
})
