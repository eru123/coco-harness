...
import SandboxPolicyService, { SANDBOX_MODES, effectiveSandboxMode, setSandboxMode } from '@coco-harness/cch-sandbox-policy'
import SystemPrompt, { renderContextSnapshot, renderPrompt } from '@coco-harness/cch-system-prompt'
...
describe('SandboxPolicyService', () => {
  ...
  it.skipIf(process.platform === 'win32')('resolves a symlink-sensitive session cwd with POSIX component semantics', async () => {
    const root = mkdtempSync(join(tmpdir(), 'cch-policy-cwd-'))
    try {
...
describe('sandbox:policy request context', () => {
  ...
  it.each(['read-only', 'workspace-write', 'danger-full-access'] as const)('renders the exact %s policy without a capability inventory', async (mode) => {
    ...
    const expected = {
      'read-only': 'Current CCH file policy: read-only. Any available operation enforced by the CCH file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.',
      'workspace-write': `Current CCH file policy: workspace-write. Any available operation enforced by the CCH file sandbox may modify files under the session workspace: ${JSON.stringify(workspaceRoot)}. Some platform temporary areas may also be writable.`,
      'danger-full-access': 'Current CCH file policy: danger-full-access. The CCH file sandbox does not restrict file modifications by available operations.',
    } as const
  ...
  it('reflects the latest durable switch on the next assembly and stays byte-stable otherwise', async () => {
    ...
    expect(danger).toBe('Current CCH file policy: danger-full-access. The CCH file sandbox does not restrict file modifications by available operations.')
    expect(await policyContext(ctx, active)).toBe(danger)

    setSandboxMode(active, 'workspace-write')
    expect(await policyContext(ctx, active)).toBe(`Current CCH file policy: workspace-write. Any available operation enforced by the CCH file sandbox may modify files under the session workspace: ${JSON.stringify(resolve('/projects/current'))}. Some platform temporary areas may also be writable.`)
  })
