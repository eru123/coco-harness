 ...
 * @module @coco-harness/cch-sandbox-windows-acl/acl
 */
...
/**
 * One lock file per protected path: `<GetTempPathW()>\cch-acl-locks\<first 16
 * hex of sha256(lowercased path)>.lock`. The lock root derives from
 * GetTempPathW (never from runner argv or CCH_HOME), and the lowercasing
...
export function lockFilePath(api: Win32Bindings, path: string): string {
  ...
  return join(getTempPath(api), 'cch-acl-locks', `${digest}.lock`)
}
