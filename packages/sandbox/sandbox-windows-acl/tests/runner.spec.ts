 ...
 * the argv shape cch-sandbox-local's confine() builds), with piped stdio
...
describe.skipIf(!isWin32 || !pwshAvailable())('windows-acl runner', () => {
  ...
  beforeAll(() => {
    scratchRoot = mkdtempSync(join(tmpdir(), 'cch-acl-runner-'))
    writableDir = join(scratchRoot, 'writable')
    mkdirSync(writableDir)
    isolatedTemp = mkdtempSync(join(tmpdir(), 'cch-acl-runner-temp-'))
    ...
    try {
      publicProbeDir = mkdtempSync(join(process.env.PUBLIC ?? 'C:\\Users\\Public', 'cch-acl-public-'))
    } catch {
