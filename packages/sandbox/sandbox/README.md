...
Implementations: [`@coco-harness/cch-sandbox-local`](../sandbox-local/) (Linux: `bwrap`, else the per-platform Landlock launcher; macOS: `sandbox-exec`/Seatbelt). Consumers: [`@coco-harness/cch-bash-sandbox`](../../shell/bash-sandbox/) (wraps `['bash', '-c', command]`).
...
Through [`cch-bash-sandbox`](../../shell/bash-sandbox/README.md) and [`cch-tool-bash`](../../shell/tool-bash/README.md), failure to enforce a requested mode produces code `SANDBOX_UNAVAILABLE` and the exact error below. An execution-time runner failure adds ` Runner failure: <detail>`.
