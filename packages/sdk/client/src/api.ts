/**
 * High-level run API over {@link HarnessClient}: `CocoHarness` owns one
 ...
 * Mirrors the Python SDK's `CocoHarness`/`Session` pair.
 *
 * @module @coco-harness/cch-sdk-client/api
 */
...
import type { ContentBlock, CocoHarnessOptions, HarnessClientOptions, HarnessNotification, RunResult } from './types.ts'

/**
 * Reusable SDK for running Coco Harness agent turns in a runtime
 ...
 * child is reaped.
 */
export class CocoHarness implements AsyncDisposable {
  ...
  constructor(options: CocoHarnessOptions) {
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: CocoHarness, readonly id: string) {}
