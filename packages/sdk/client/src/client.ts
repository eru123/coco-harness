/**
 * Low-level JSON-RPC client for a Coco Harness SDK runtime subprocess.
 * {@link HarnessClient} owns the child process: it spawns the runtime, speaks
 * the `@coco-harness/cch-sdk-protocol` wire over the child's stdio, fans
 ...
 * spawns directly rather than through the `cch-subprocess` service — the
 * seam's documented exception for SDK-managed transports.
 *
 * @module @coco-harness/cch-sdk-client/client
 */
...
} from '@coco-harness/cch-sdk-protocol'
...
/**
 * JSON-RPC client for the Coco Harness SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('Coco Harness runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('Coco Harness runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('Coco Harness runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('Coco Harness runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('Coco Harness runtime is not running')
    ...
    try {
      ...
      const timer = setTimeout(() => {
        abandon.abort(new RequestTimeoutError(`${method} timed out after ${timeout}ms waiting for the Coco Harness runtime`))
      }, timeout)
      try {
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('Coco Harness runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('Coco Harness runtime closed'))
  }
