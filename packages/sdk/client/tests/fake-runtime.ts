#!/usr/bin/env node
/**
 * Scripted stand-in for the Coco Harness SDK runtime, driven entirely by
    ...
    case 'initialize':
      ...
      if (env.FAKE_INIT_READY !== undefined && env.FAKE_INIT_GO !== undefined) {
        ...
        const poll = setInterval(() => {
          ...
          clearInterval(poll)
          write({ jsonrpc: '2.0', id, result: { serverInfo: { name: 'coco-harness-sdk-runtime', version: '0.0.1' } } })
        }, 5)
        return
      }
      ...
      if (env.FAKE_ECHO_CWD_IN_INIT !== undefined) {
        respond({ serverInfo: { name: 'coco-harness-sdk-runtime', version: process.cwd() } })
        return
      }
      respond({ serverInfo: { name: 'coco-harness-sdk-runtime', version: '0.0.1' } })
      return
    case 'session/prompt': {
