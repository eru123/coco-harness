...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): CocoHarness {
  const harness = new CocoHarness({ launch: fakeLaunch(env, extra) })
...
describe('CocoHarness', () => {
  ...
  it('resolves a relative launch cwd to an absolute workspace before the handshake', async () => {
    ...
    const dir = await mkdtemp(join(process.cwd(), '.cch-sdk-client-relcwd-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const recordFile = join(dir, 'init.jsonl')
  ...
  it('supports await using disposal', async () => {
    let captured: CocoHarness
    {
      await using harness = new CocoHarness({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('fails fast when the command does not exist', async () => {
    const client = new HarnessClient({ command: join(tmpdir(), 'cch-no-such-runtime-bin') })
    cleanups.push(() => client.close())
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('coco-harness-sdk-runtime')
