# @coco-harness/cch-sdk-protocol

The shared wire protocol for the Coco Harness SDK runtime: one newline-delimited JSON-RPC 2.0 transport class plus the named request, result, and notification types both wire ends speak. The package root enumerates the protocol consumer interface; source modules are not exported as deep imports. The server side is the [`cch-sdk-jsonrpc-server`](../server/README.md) plugin; clients are [`cch-sdk-client`](../client/README.md) (TypeScript) and the [Python SDK](../../../python/README.md) (which mirrors these shapes but does not import them). A pure library — no plugin, no Config, no registration.
...
- **No cancel or session-close methods** — a client abandons a turn by closing the runtime process; see the [`cch-sdk-jsonrpc-server` README](../server/README.md).
