...
import { createSnapshotStore, type SessionId, type SnapshotStore } from '@coco-harness/cch-client-runtime/client'
...
export function sessionLogZipFilename(sessionId: SessionId): string {
  return `cch-session-${String(sessionId).replace(/[^A-Za-z0-9_-]/g, '_')}.zip`
}
...
function hostBase(): string {
  ...
  return origin !== undefined && origin !== 'null' ? origin : 'http://cch.internal'
}
