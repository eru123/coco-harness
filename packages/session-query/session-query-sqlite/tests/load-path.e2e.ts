 ...
 *
 * @module @coco-harness/cch-session-query-sqlite/tests/load-path
 */

import { afterEach, describe, expect, it } from 'vitest'
import { Context } from '@coco-harness/cordis'
import Loader from '@coco-harness/cordis-plugin-loader'
import { SESSION_FORMAT_VERSION, SessionId } from '@coco-harness/cch-session'
...
import SqliteSessionQueryEngine, * as queryModule from '@coco-harness/cch-session-query-sqlite'
...
async function temporaryPath(name: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'cch-session-search-loader-'))
  temporaryDirectories.push(directory)
  return join(directory, name)
}

describe('cch-session-query-sqlite real Loader path', () => {
