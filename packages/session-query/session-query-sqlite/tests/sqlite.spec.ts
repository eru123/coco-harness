import { createAssistantMessage, createUserMessage } from '@coco-harness/cch-llm'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { Context, type Fiber } from '@coco-harness/cordis'
import { DatabaseSync } from 'node:sqlite'
import { chmod, mkdtemp, rm, stat, writeFile } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join } from 'node:path'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@coco-harness/cch-session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@coco-harness/cch-session'
import SessionPersistence, { SessionPersistenceRevision } from '@coco-harness/cch-session-persistence'
import type { SessionPersistenceSnapshot } from '@coco-harness/cch-session-persistence'
import SqliteSessionPersistence from '@coco-harness/cch-session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@coco-harness/cch-session-query-sqlite'
import {
...
} from '@coco-harness/cch-session-query'
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'cch-session-search-'))
