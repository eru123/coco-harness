import { MessageId, createUserMessage, createMessage } from '@coco-harness/cch-llm'
...
import type { Session, SessionEvent, SessionHeader } from '@coco-harness/cch-session'
...
async function freshRoot(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cch-jsonl-'))
  dirs.push(dir)
  return dir
}
...
runCoordinatorContract('jsonl-none', async (): Promise<CoordinatorFixture> => {
  const dir = await mkdtemp(join(tmpdir(), 'cch-jsonl-coord-'))
  return {
    mount: async (ctx) => {
      const fiber = await ctx.plugin(JsonlSessionPersistence, { root: dir, compression: 'none' })
      return fiber
...
describe('JsonlSessionPersistence: format helpers', () => {
  ...
  it('projectKey normalizes project paths into bounded readable names', () => {
    expect(projectKey('/Users/qyj/work/coco-harness')).toBe('--Users-qyj-work-coco-harness--')
