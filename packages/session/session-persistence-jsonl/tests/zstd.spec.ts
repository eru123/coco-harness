...
async function freshRoot(prefix = 'cch-jsonl-zstd-'): Promise<string> {
...
runPersistenceContract('jsonl-zstd', async () => {
  const root = await mkdtemp(join(tmpdir(), 'cch-jsonl-zstd-contract-'))
  const ctx = new Context()
  await ctx.plugin(SessionStore)
...
runCoordinatorContract('jsonl-zstd', async (): Promise<CoordinatorFixture> => {
  const root = await mkdtemp(join(tmpdir(), 'cch-jsonl-zstd-coordinator-'))
  return {
...
describe('JsonlSessionPersistence: encoding selection', () => {
  it('rejects roots owned by the opposite encoding in both directions', async () => {
    const rawRoot = await freshRoot('cch-jsonl-raw-mismatch-')
    ...
    const zstdRoot = await freshRoot('cch-jsonl-zstd-mismatch-')
