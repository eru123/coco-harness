...
import * as SessionStatsPlugin from '@coco-harness/cch-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'cch-session-stats-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@coco-harness/cch-session-projection', SessionProjectionRegistry],
    ['@coco-harness/cch-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@coco-harness/cch-session-projection'",
