...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@coco-harness/cch-llm'
import type { GenerateOptions, StreamChunk } from '@coco-harness/cch-llm'
import SessionStore, { SessionId } from '@coco-harness/cch-session'
import SessionTitleService from '@coco-harness/cch-session-title'
import * as providerPlugin from '@coco-harness/cch-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'cch-title-loader-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@coco-harness/cch-llm'",
    "- name: '@coco-harness/cch-session'",
    "- name: '@coco-harness/cch-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@coco-harness/cch-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@coco-harness/cch-session-title', SessionTitleService],
    ['@coco-harness/cch-session-title-first-prompt-llm', providerPlugin],
  ])
