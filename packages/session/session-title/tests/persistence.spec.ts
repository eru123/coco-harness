...
import SessionTitleService, { foldSessionTitle } from '@coco-harness/cch-session-title'
...
describe('session title persistence round trips', () => {
  it('round-trips through a remounted JSONL backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cch-title-jsonl-'))
    roots.push(root)
  ...
  it('round-trips through a remounted SQLite backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cch-title-sqlite-'))
    roots.push(root)
