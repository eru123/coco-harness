...
const spillDir = mkdtempSync(join(tmpdir(), 'cch-bash-exec-spec-'))
...
describe('LocalBashExecutor.run', () => {
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'true', workdir: '/nonexistent-cch' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/cchEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: 'cat; echo "[$SEAM_VAR][$CCH_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
...
describe('LocalBashExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: 'cat; echo "[$BG_VAR][$CCH_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      cchEnv: { CCH_BG_VAR: 'bg-cch-env' },
    }))
    const output = await readUntil(proc, '[bg-env][bg-cch-env]')
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'true', workdir: '/nonexistent-cch' }))
