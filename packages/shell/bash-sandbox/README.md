# @coco-harness/cch-bash-sandbox

Sandbox-consuming Service Provider for the [`@coco-harness/cch-shell`](../shell/) executor seam. Load it **instead of** `@coco-harness/cch-bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@coco-harness/cch-sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `cch-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
...
- Process mechanics (spawn, process-group kills, output collection/spill, background handles, credential scrub) are inherited from [`cch-bash-local`](../bash-local/); runner selection lives in [`cch-sandbox-local`](../../sandbox/sandbox-local/).

Deny-only at the seam: a denial is a reported fact, and this executor never negotiates permissions itself — the approval question lives in the tool layer (`cch-tool-bash`), which drives the override this package honors.

```yaml
- id: sandbox
  name: '@coco-harness/cch-sandbox-local'
- id: sandbox-policy
...
- id: bash
  name: '@coco-harness/cch-bash-sandbox'
```
...
Small fixed schema increment on requests where `bash` is visible, plus the current-policy clause owned by `cch-sandbox-policy`.
