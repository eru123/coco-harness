...
import { bwrapProfileArgs } from '@coco-harness/cch-sandbox-local/src/profiles.ts'
 ...
 * backend-only confinement is covered by `@coco-harness/cch-sandbox-local`.
 *
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'cch-bwrap-e2e-'))
  tempDirs.push(dir)
  return dir
}
