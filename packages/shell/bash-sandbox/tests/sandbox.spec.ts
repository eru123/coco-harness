...
import type { ShellRunResult, CollectedOutput } from '@coco-harness/cch-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@coco-harness/cch-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@coco-harness/cch-sandbox'
import { SandboxPolicyService } from '@coco-harness/cch-sandbox-policy'
import { SandboxBashExecutor } from '@coco-harness/cch-bash-sandbox'
...
import type { Config } from '@coco-harness/cch-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'cch-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'CCH_WRAP=1', 'bash', '-c', 'printf "%s" "$CCH_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$CCH_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'cch-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.CCH_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'cch-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'cch-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
