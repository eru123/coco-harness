...
import { seatbeltProfileArgs } from '@coco-harness/cch-sandbox-local/src/profiles.ts'
 ...
 * confinement is covered by `@coco-harness/cch-sandbox-local`. Skips off macOS or when
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'cch-seatbelt-e2e-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(!seatbeltUsable)('bash-sandbox: real Seatbelt confinement through ctx.shell', () => {
  ...
  it('evaluates BASH_ENV only after Seatbelt confines the inner Bash', async () => {
    ...
    await writeFile(hook, [
      'printf hook > "$CCH_BASH_ENV_INSIDE"',
      'printf escaped > "$CCH_BASH_ENV_OUTSIDE"',
      '',
    ].join('\n'))
    const bash = await sandboxedBash(workdir, 'workspace-write')
    ...
    await bash.run(bash.resolve({
      ...
      env: { BASH_ENV: hook },
      cchEnv: {
        CCH_BASH_ENV_INSIDE: insideProbe,
        CCH_BASH_ENV_OUTSIDE: outsideProbe,
      },
    }))
