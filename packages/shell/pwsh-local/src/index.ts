 ...
 *
 * @module @coco-harness/cch-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors cch-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@coco-harness/cordis'
import z from '@coco-harness/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@coco-harness/cch-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@coco-harness/cch-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@coco-harness/cch-subprocess'
import { installSettingsSection } from '@coco-harness/cch-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@coco-harness/cch-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of cch-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `cch-bash-local`'s `runArgv`/`startArgv` hooks; see
   * `@coco-harness/cch-pwsh-sandbox`).
   */
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.cchEnv },
    }
  }
   ...
   * `cch-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@coco-harness/cch-pwsh-sandbox`.
