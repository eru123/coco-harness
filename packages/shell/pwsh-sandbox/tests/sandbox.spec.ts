...
import type { ConfinedArgv, RunnerFailureRule, SandboxExecutionPolicy, SandboxPolicy } from '@coco-harness/cch-sandbox'
...
const spillDir = mkdtempSync(join(tmpdir(), 'cch-pwsh-sandbox-spec-'))
...
describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'cch-pwsh-sandbox-helpers-'))
  afterAll(() => {
...
describe.skipIf(!pwshAvailable())('SandboxPwshExecutor', () => {
  ...
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'cch-pwsh-sandbox-ro-'))
