 ...
 * trusted, per-execution `CCH_*` variables consumed by the model-facing shell
 * tools (`cch-tool-bash`, `cch-tool-pwsh`). Built-in shell facts are owned by
 * the registry itself while plugins can register additional, enumerable facts
 * with effect-scoped disposal.
 *
 * @module @coco-harness/cch-shell-env
 */

import { Service, type Context } from '@coco-harness/cordis'
...
export interface Config {
  /** Coco Harness home directory exposed as `CCH_HOME`; defaults to `$CCH_HOME` or `~/.cch`. */
  cchHome?: string
}
...
/** Model-visible metadata for one managed `CCH_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `CCH_*` keys this contributor may return. */
  variables: Readonly<Record<CchEnvironmentKey, BashEnvVariable>>
  /**
   ...
   */
  resolve(execution: ToolExecution): Readonly<Partial<Record<CchEnvironmentKey, string>>>
}
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `CCH_*` environment variable name. */
  key: CchEnvironmentKey
}

const CCH_SHELL_KEY = `${CCH_ENV_PREFIX}SHELL` as const
const CCH_SESSION_ID_KEY = `${CCH_ENV_PREFIX}SESSION_ID` as const
const CCH_SESSION_JSONL_KEY = `${CCH_ENV_PREFIX}SESSION_JSONL` as const
const RESERVED_BASH_ENV_KEYS = new Set<CchEnvironmentKey>([
  CCH_HOME_ENV,
  CCH_SHELL_KEY,
  CCH_SESSION_ID_KEY,
])
...
/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `CCH_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `CCH_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<CchEnvironmentKey, string>()
  private readonly cchHome: string
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [CchEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(CCH_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(CCH_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): CchEnvironment {
    const values: Record<CchEnvironmentKey, string> = {
      [CCH_HOME_ENV]: this.cchHome,
      [CCH_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[CCH_SESSION_ID_KEY] = execution.agent.session.header.id
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as CchEnvironmentKey
 ...
 * shell-agnostic persistence contributor (`CCH_SESSION_JSONL`).
 * @param ctx - Cordis context that owns the service and registrations.
 * @param config - home-directory configuration for the built-in variables.
 */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  registry.register({
    ...
    resolve(execution) {
      ...
      return location?.kind === 'jsonl' ? { [CCH_SESSION_JSONL_KEY]: location.path } : {}
    },
  })
}
