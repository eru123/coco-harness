 ...
 * @module @coco-harness/cch-tool-bash
 */
...
import { ESCALATION_TARGETS, approveEscalation, canonicalPath, validateEscalationArgs } from '@coco-harness/cch-sandbox'
...
function bashDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a bash command (`bash -c`) and return its stdout/stderr. '
    ...
    + `Current harness environment facts are exposed through managed \`$${CCH_ENV_PREFIX}*\` variables; inspect them when needed. `
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: BashToolArgs, exec) {
      ...
      const cchEnv = ctx.shellEnv.collect(exec)
      const request = {
