...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@coco-harness/cch-shell'
...
const spillDir = mkdtempSync(join(tmpdir(), 'cch-tool-bash-spec-'))
...
describe('bash tool', () => {
  ...
  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'bash', { command: 'true', description: 'test command', workdir: '/nonexistent-cch' })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
   ...
   * (the credential scrub in cch-bash-local is the security control; see the
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$CCH_*')
    expect(description).not.toContain('CCH_SESSION_JSONL')
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { CCH_SESSION_ID: 'spoofed', CCH_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.CCH_SESSION_ID
    ...
    expect(process.env.CCH_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests[0]?.cchEnv?.CCH_SESSION_JSONL).not.toBe(bash.requests[1]?.cchEnv?.CCH_SESSION_JSONL)
  })
