# @coco-harness/cch-tool-pwsh

The model-facing `pwsh` tool registered over the `ctx.shell` executor seam. Intended for Windows compositions where a PowerShell executor (e.g. `@coco-harness/cch-pwsh-local`) backs `ctx.shell`; the tool contract is PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables. Behavior mirrors `cch-tool-bash` call-for-call — foreground and `run_in_background` execution through the generic job runtime, the managed `CCH_*` environment through the shared `shell-env` registry, the sandbox denial rendering with the same-turn `sandbox_permissions` escalation surface, and the bash marker/truncation rendering story (a clean exit produces no marker).
...
Every foreground and background model pwsh call receives a freshly collected trusted `CCH_*` environment through the shared [`cch-shell-env`](../shell-env/) registry: `CCH_HOME` (the absolute Harness home), `CCH_SHELL=1`, the agent's `CCH_SESSION_ID`, and `CCH_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `CCH_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.cchEnv` channel; `process.env` is never modified. The description teaches the generic `$env:CCH_*` convention rather than naming persistence-specific variables.
...
The model sees the generated [`pwsh` schema](../../../docs/tool-catalog.md#coco-harnesscch-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
