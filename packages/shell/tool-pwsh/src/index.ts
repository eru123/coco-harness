 ...
 * `@coco-harness/cch-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `cch-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `CCH_*` environment through the
 ...
 * exit-status parse from `@coco-harness/cch-shell`.
 *
 * @module @coco-harness/cch-tool-pwsh
 */
...
import type {} from '@coco-harness/cch-system-prompt'
...
import type { SandboxExecutionPolicy, SandboxMode } from '@coco-harness/cch-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@coco-harness/cch-sandbox'
import type { SandboxPolicyService } from '@coco-harness/cch-sandbox-policy'
import type { ShellRunResult } from '@coco-harness/cch-shell'
import { parseExitStatus } from '@coco-harness/cch-shell'
...
/* jscpd:ignore-start -- minimal mirror of cch-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:CCH_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror cch-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of cch-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of cch-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of cch-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors cch-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with cch-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors cch-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        cchEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @coco-harness/cch-jobs and @coco-harness/cch-tool-jobs')
        }
