/**
 * Integration tests: the REAL `@coco-harness/cch-pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@coco-harness/cch-tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@coco-harness/cch-pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'cch-tool-pwsh-'))
