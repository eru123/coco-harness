 ...
 * managed `CCH_*` collection, abort translation, canonical result projection,
...
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@coco-harness/cch-tools'
...
import type { ApprovalOutcome } from '@coco-harness/cch-user-approval'
import { ShellExecutor } from '@coco-harness/cch-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@coco-harness/cch-shell'
...
import type { ShellProcessRead } from '@coco-harness/cch-shell'
...
class FakeBash extends ShellExecutor {
  ...
  override resolve(request: ShellExecRequest): ShellExecSpec {
    ...
    return {
      ...
      ...request.cchEnv !== undefined ? { cchEnv: request.cchEnv } : {},
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, cchHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, cchHome === undefined ? {} : { cchHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, cchHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed CCH_* environment', async () => {
    const cchHome = mkdtempSync(join(tmpdir(), 'cch-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, cchHome)
    ...
    expect(request?.cchEnv).toEqual({
      CCH_HOME: cchHome,
      CCH_SHELL: '1',
      CCH_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const cchEnv = bash.requests[0]?.cchEnv
    expect(cchEnv).toBeDefined()
    expect(cchEnv?.['CCH_SHELL']).toBe('1')
    expect(cchEnv?.['CCH_HOME']).toEqual(expect.any(String))
    expect(cchEnv).not.toHaveProperty('CCH_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'cch-tool-pwsh-policy-'))
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @coco-harness/cch-jobs and @coco-harness/cch-tool-jobs')
  })
