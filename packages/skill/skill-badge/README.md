# @coco-harness/cch-skill-badge

Optional bundled skill provider that contributes `cch-badge` to `ctx.skills`. The skill supplies the official “powered by cch” Markdown snippets and the packaged PNG for systems that cannot import a remote image reliably.
...
The provider exposes its packaged `assets/` directory as the skill resource base. `cch-badge.png` is the 726×120 source asset, and consumers render it at 121×20.

## Model Experience

Indirectly, through `@coco-harness/cch-tool-skill`, which renders the catalog entry and selected skill body.
