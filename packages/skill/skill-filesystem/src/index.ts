 ...
 *
 * @module @coco-harness/cch-skill-filesystem
 */
...
import {
  ...
  type SkillSource,
} from '@coco-harness/cch-skill'

const PROJECT_CCH_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_CCH_RANK = 400
...
export interface Config {
  ...
  /** Coco Harness config root. Defaults to `$CCH_HOME` or `~/.cch`. */
  cchHome?: string
  /** Shared agent config root. Defaults to `$CCH_AGENTS_HOME` or `~/.agents`. */
  agentsHome?: string
  ...
  /** Bundled skill root; defaults to `$CCH_BUNDLED_SKILL_DIR` when default roots are included, otherwise mounts none. */
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.CCH_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.cch/skills'), source: 'project-cch', rank: PROJECT_CCH_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.cchHome, 'skills'), source: 'user-cch', rank: USER_CCH_RANK, skipSystem: true },
