...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@coco-harness/cch-fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `cch-${name}-`)))
}
...
describe('cch-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.cch/skills'), 'same', 'user cch skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.cch/skills'), 'same', 'project cch skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.cch/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project cch skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-cch')
    ...
    await writeSkill(join(noGit, '.cch/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.cch/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.cch/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.cch/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.cch/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.cch/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.cch/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.cch/skills/broken-link'))
    await symlink('/dev/null', join(home, '.cch/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { cchHome: join(home, '.cch'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.cch/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.cch/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    ...
    try {
      process.env.CCH_HOME = join(envHome, '.cch')
      process.env.CCH_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.CCH_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.cch/skills'), 'env-skill', 'Env skill')
      ...
      process.env.CCH_HOME = join(envHome, 'empty-cch')
      delete process.env.CCH_BUNDLED_SKILL_DIR
      process.env.CCH_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      if (previousAgentsHome === undefined) {
        ...
        process.env.CCH_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.CCH_BUNDLED_SKILL_DIR
      } else {
        process.env.CCH_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
