...
import { createUserMessage, CallId, type Message } from '@coco-harness/cch-llm'
import { createScope, type Scope } from '@coco-harness/cch-scope'
import { Session, SessionId, type SessionEvent, type UserMessage } from '@coco-harness/cch-session'
...
import AgentRegistry, { agentEvents, Inbox, type Agent, type PreStepDecision } from '@coco-harness/cch-agent'
...
describe('cch-tool-skill', () => {
  ...
  it('resumes from the durable entries of the latest visible catalog', async () => {
    ...
    session.append('user/message', createUserMessage({
      ...
      source: { kind: 'plugin', plugin: 'cch-tool-skill' },
    }), { surfaceOp: 'append' })
  ...
  it('loads a skill for the calling agent cwd', async () => {
    ...
    await writeSkill(join(project, '.cch/skills'), 'project-skill', 'Project skill', 'Project instructions.')
    const ctx = await setup(home)
    ...
    expect(result.value).toEqual({
      name: 'project-skill',
      provider: 'filesystem',
      resourceBase: { kind: 'directory', path: join(project, '.cch/skills/project-skill') },
    ...
    expect(block.text).toBe([
      ...
      '<skill_resources>',
      `Base directory for this skill: ${join(project, '.cch/skills/project-skill')}`,
  ...
  it('returns isError for unknown, invalid, and model-disabled skills', async () => {
    ...
    await writeSkill(join(home, '.cch/skills'), 'hidden-skill', 'Hidden skill', 'Hidden instructions.')
    await writeFile(join(home, '.cch/skills/hidden-skill/SKILL.md'), '---\nname: hidden-skill\ndescription: Hidden skill\ndisable-model-invocation: true\n---\n\nHidden instructions.\n')
    const ctx = await setup(home)
    ctx.skills.register({
      name: 'model-only-skill',
      description: 'Model-only skill',
