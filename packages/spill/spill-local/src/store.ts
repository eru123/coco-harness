 ...
 * (like `cch-bash-local`'s `run.ts`) so the filesystem behavior is unit-testable
 * without a `ctx` and without the OS temp dir.
 *
 * @module @coco-harness/cch-spill-local/store
 */
...
export function privateRoot(): string {
  defaultRoot ??= mkdtempSync(join(tmpdir(), 'cch-spill-'))
  return defaultRoot
}
