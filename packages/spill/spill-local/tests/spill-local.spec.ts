...
import type { SaveTextSpill } from '@coco-harness/cch-spill'
import LocalSpillStore, { encodeSegment, privateRoot, saveTextFile, sessionDir } from '@coco-harness/cch-spill-local'

let root: string

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'cch-spill-test-'))
})
afterEach(() => {
