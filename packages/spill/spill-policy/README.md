...
This plugin registers **no service** and owns no storage or preview mechanics: preview is [`@coco-harness/cch-output-retention`](../../util/output-retention) (`TextRetainer`), storage is `ctx.spillStore`. It only decides WHEN to spill and composes the notice.
