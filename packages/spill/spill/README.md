...
| Package | Role |
|---|---|
| `@coco-harness/cch-spill` (this) | Service Definition: abstract service + vocabulary types |
| `@coco-harness/cch-spill-local` | Service Provider: private session-scoped files on the host filesystem |
| `@coco-harness/cch-spill-policy` | Consumer: the tool-result policy that spills oversized final results |
