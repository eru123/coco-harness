 ...
 * `@coco-harness/cch-spill-local` (host filesystem) is the first.
 *
 * The Service Definition is deliberately minimal: `saveText` and nothing else. It owns NO
 * retention policy (that is `@coco-harness/cch-output-retention`), NO tool-result
 * replacement (that is `@coco-harness/cch-spill-policy`), and NO retrieval or
 * search API. The backend supplies the locator and retrieval hint appropriate
 * for its storage substrate.
 *
 * @module @coco-harness/cch-spill
 */
