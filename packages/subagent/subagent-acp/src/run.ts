 ...
 * @module @coco-harness/cch-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@coco-harness/cch-subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `CCH_*` entries are deployment-owned facts for the child harness
   * (e.g. `CCH_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and CCH_* names
