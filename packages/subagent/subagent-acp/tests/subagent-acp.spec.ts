...
import type { SubprocessOutcome } from '@coco-harness/cch-subprocess'
...
describe('child env layering (through the subprocess seam)', () => {
  ...
  it('forwards explicit CCH_* config entries to the child', async () => {
    // A deployment sets child-harness facts like CCH_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'CCH_ACP_TEST_FACT', CCH_ACP_TEST_FACT: 'managed' })
...
describe('disposeAcpChild (the backend-owned teardown ladder over seam verbs)', () => {
  ...
  it('observes a spawn-level rejection and returns without a process to reap', async () => {
    const child = spawnSubprocess({
      ...
      cwd: '/nonexistent-dir-cch-acp-ladder-test',
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1000 }, stderr: { maxBytes: 1000 } },
      graceMs: 200,
    })
