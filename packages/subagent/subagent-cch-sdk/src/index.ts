 ...
 * @module @coco-harness/cch-subagent-cch-sdk
 */

import type { Context } from '@coco-harness/cordis'
import z from '@coco-harness/schemastery'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@coco-harness/cch-subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@coco-harness/cch-subagent'
import {
...
export const name = 'subagent-cch-sdk'
export const inject = ['subagents']

/** Config: how to spawn and drive the child SDK runtime process. */
export interface Config {
  /** Provider name on `ctx.subagents` (default `cch-sdk`). */
  providerName: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `CCH_CORDIS_CONFIG` naming its
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('cch-sdk'),
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    const spec: SdkRunSpec = {
      ...
      cwd: resolveChildCwd('subagent-cch-sdk', this.config.cwd, request.parent.session.header.cwd),
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-cch-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-cch-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-cch-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-cch-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-cch-sdk maxTokens must be a positive safe integer')
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-cch-sdk', resolved.cwd)
