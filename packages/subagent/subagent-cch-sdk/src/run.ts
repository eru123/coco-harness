/**
 * Fresh-process SDK subagent client. Drives one child Coco Harness
 * runtime over stdio JSON-RPC through `@coco-harness/cch-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@coco-harness/cch-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @coco-harness/cch-subagent-cch-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { CocoHarness, type HarnessNotification } from '@coco-harness/cch-sdk-client'
...
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@coco-harness/cch-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@coco-harness/cch-subagent'
import { scrubbedParentEnv } from '@coco-harness/cch-subprocess'

/** Resolved spawn spec for an SDK runtime child process (no defaults — see Config). */
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `cch-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `CCH_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `CCH_*` fact survives while ambient namesakes never leak.
   */
