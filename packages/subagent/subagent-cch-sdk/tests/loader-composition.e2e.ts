...
import { resolveExampleLaunch, runLoaderSmoke } from '@coco-harness/cch-loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-cch-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'cch-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'cch-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        CCH_TEST_CHILD_COMMAND: childLaunch.command,
        CCH_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        CCH_TEST_CHILD_ENV: JSON.stringify({
