...
import { snapshotSubagentDescriptor } from '@coco-harness/cch-subagent'
...
beforeEach(async () => {
  workspace = await realpath(await mkdtemp(join(tmpdir(), 'cch-inherit-')))
})
  ...
  it('records the parent sandbox override and the approval pin before publishing a spawn child', async () => {
    ...
    try {
      ...
      const contextText = runtimeContext.data.content
        ...
        .map(block => block.text)
        .join('\n')
      expect(contextText).toContain('Current CCH file policy: read-only')
