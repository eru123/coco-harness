...
import * as SubagentFork from '@coco-harness/cch-subagent-fork-in-process'
import type { GenerateOptions, MessageId, StreamChunk } from '@coco-harness/cch-llm'
import { CallId, createUserMessage, LlmAdapter } from '@coco-harness/cch-llm'
...
async function setupWith(adapter: LlmAdapter, options: { persistence?: boolean } = {}) {
  ...
  if (options.persistence !== false) {
    root = mkdtempSync(join(tmpdir(), 'cch-subagent-continuation-'))
    roots.push(root)
...
describe('continuable settlement delivery', () => {
  ...
  it('reports a turn that failed before reaching its first step', async () => {
    ...
    const { ctx, parent } = await setupWith(adapter)
    // The shipped durability checkpoint (`cch-session-checkpoint-policy`) is
...
describe('continuable errors', () => {
  ...
  it('unloading the manager drains its live activations', async () => {
    ...
    const ctx = new Context()
    await mountAgentLoopTestDependencies(ctx)
    const root = mkdtempSync(join(tmpdir(), 'cch-subagent-continuation-'))
    roots.push(root)
