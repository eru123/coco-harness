...
import { assembleContextFor } from '@coco-harness/cch-agent'
...
import * as control from '@coco-harness/cch-tool-subagent-control'
...
async function setup(options: { load?: boolean; config?: tool.Config } = {}) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'cch-tool-subagent-report-'))
...
describe('cch-tool-subagent-report', () => {
...
describe('cch-tool-subagent-report result independence', () => {
