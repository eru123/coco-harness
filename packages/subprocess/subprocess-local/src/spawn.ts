 ...
 * @module cch-subprocess-local/spawn
 */
...
function privateSpillDir(): string {
  defaultSpillDir ??= mkdtempSync(join(tmpdir(), 'cch-subprocess-'))
...
export class OutputCollector {
  ...
  private spillAll(chunk: Buffer): void {
    ...
    if (this.spillFd === undefined) {
      ...
      this.spillFile = join(
        this.spillDir,
        `cch-subprocess-${process.pid}-${++spillCounter}-${randomBytes(6).toString('hex')}-${this.label}.log`,
      )
