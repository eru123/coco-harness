 ...
 * @module cch-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for Coco Harness-managed child environment facts. */
export const CCH_ENV_PREFIX = 'CCH_' as const

/** One environment key inside the managed {@link CCH_ENV_PREFIX} namespace. */
export type CchEnvironmentKey = `${typeof CCH_ENV_PREFIX}${string}`

/** Trusted Coco Harness variables for one child-process execution. */
export type CchEnvironment = Readonly<Record<CchEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@coco-harness/cch-terminal` without a cross-seam dependency;
