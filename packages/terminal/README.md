...
|---|---|---|
| [`pty`](terminal/README.md) (`@coco-harness/cch-terminal`) | Backend registry, branded ids, exact-Agent ownership, session operations, and awaited cleanup | `ctx.terminals` |
| `terminal-bash` (`@coco-harness/cch-terminal-bash`) | Shell backend over `ctx.subprocess.spawnTerminal`: readiness detection, bounded terminal state, sandbox policy, and session operations | registers on `ctx.terminals` |
| `tool-terminal` (`@coco-harness/cch-tool-terminal`) | Six model-facing tools and generic task integration for background sends | registers on `ctx.tools` |
