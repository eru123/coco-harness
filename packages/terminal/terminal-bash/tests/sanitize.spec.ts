...
import { normalizeTerminalText, TerminalSanitizer } from '@coco-harness/cch-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('cch> ')).toEqual({ text: 'cch> ', prompt: false, promptTail: 'cch> ' })
  })
