...
function snapshotMode(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    default: throw new Error(`unknown CCH_SNAPSHOT mode: ${value}`)
  }
}
...
defineAcpSnapshotSuite({
  ...
  mode: snapshotMode(process.env.CCH_SNAPSHOT),
})
```
...
- **Built mode requires current artifacts** — run `pnpm run build` before selecting `CCH_EXAMPLE_MODE=lib`; source mode remains the zero-build path.
