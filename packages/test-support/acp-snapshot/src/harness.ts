 ...
 *
 * @module @coco-harness/cch-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `cch-llm-replay` via `$CCH_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      CCH_SNAPSHOT: opts.mode,
      CCH_SNAPSHOT_FILE: opts.fixtureFile,
      CCH_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      CCH_SNAPSHOT_SPILL_ROOT: spillRoot,
      CCH_HOME: join(cwd, '.cch'),
      CCH_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { CCH_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { CCH_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in cch-llm-replay's loadSessionScripts
