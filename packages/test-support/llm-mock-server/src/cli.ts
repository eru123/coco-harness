 ...
 * @module @coco-harness/cch-llm-mock-server/cli
 */
...
/** Result of parsing `cch-llm-mock-server` arguments. */
...
export const MOCK_LLM_CLI_USAGE = `Usage: cch-llm-mock-server [options]
...
function numberValue(option: string, value: string): number {
  ...
  if (!Number.isFinite(parsed)) throw new Error(`cch-llm-mock-server: ${option} must be a finite number`)
  return parsed
}
...
function boundedIntegerValue(option: string, value: string, min: number, max: number): number {
  ...
  if (!Number.isInteger(parsed) || parsed < min || parsed > max) {
    throw new Error(`cch-llm-mock-server: ${option} must be an integer between ${min} and ${max}`)
  }
  return parsed
}
...
function parseSequence(raw: string): { startsUnavailable: boolean; sequence: MockLlmBehavior[] } {
  ...
  if (entries.some(entry => entry.length === 0)) {
    throw new Error('cch-llm-mock-server: --sequence must contain non-empty comma-separated behaviors')
  }
  const startsUnavailable = entries[0] === CONNECTION_REFUSED_BEHAVIOR
  if (entries.slice(1).includes(CONNECTION_REFUSED_BEHAVIOR)) {
    throw new Error('cch-llm-mock-server: connection_refused is allowed only as the first behavior')
  }
  const requestEntries = startsUnavailable ? entries.slice(1) : entries
  if (requestEntries.length === 0) {
    throw new Error('cch-llm-mock-server: connection_refused must be followed by a request behavior')
  }
  for (const entry of requestEntries) {
    if (!BEHAVIORS.has(entry)) throw new Error(`cch-llm-mock-server: unknown behavior ${JSON.stringify(entry)}`)
  }
...
function parseRandomWeights(raw: string): MockLlmRandomWeights {
  ...
  for (const entry of raw.split(',')) {
    ...
    if (behavior === undefined || behavior === '' || rawWeight === undefined || rawWeight === '' || extra.length > 0) {
      throw new Error('cch-llm-mock-server: --random-weights expects behavior=weight comma-separated entries')
    }
    if (!BEHAVIORS.has(behavior) || behavior === 'random') {
      throw new Error(`cch-llm-mock-server: random weight requires a concrete behavior, got ${JSON.stringify(behavior)}`)
    }
    if (Object.hasOwn(weights, behavior)) {
      throw new Error(`cch-llm-mock-server: duplicate random weight for ${JSON.stringify(behavior)}`)
    }
...
export function parseMockLlmCliArgs(argv: readonly string[]): MockLlmCliParseResult {
  ...
  if (values.sequence === undefined) throw new Error('cch-llm-mock-server: --sequence is required')
  ...
  if (parsedSequence.startsUnavailable && port === 0) {
    throw new Error('cch-llm-mock-server: connection_refused requires an explicit nonzero --port')
  }
  if (!parsedSequence.startsUnavailable && listenDelayMs !== undefined) {
    throw new Error('cch-llm-mock-server: --listen-delay-ms requires connection_refused first in --sequence')
  }
  if (!parsedSequence.sequence.includes('random') && (randomSeed !== undefined || randomWeights !== undefined)) {
    throw new Error('cch-llm-mock-server: --seed and --random-weights require random in --sequence')
  }
