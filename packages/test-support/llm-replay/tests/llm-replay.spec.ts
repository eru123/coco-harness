...
import { CompactionId } from '@coco-harness/cch-compaction'
import LlmRuntime, { CallId, createUserMessage, GenerateOptions, LlmAdapter, StreamChunk } from '@coco-harness/cch-llm'
import {
  type Config,
  type ReplayEntry,
  type SessionScript,
...
describe('apply (the plugin entry)', () => {
  const ORIG = {
    file: process.env.CCH_SNAPSHOT_FILE,
    override: process.env.CCH_SNAPSHOT_OVERRIDE,
    children: process.env.CCH_SNAPSHOT_CHILD_FILES,
  }
  afterEach(() => {
    if (ORIG.file === undefined) delete process.env.CCH_SNAPSHOT_FILE
    else process.env.CCH_SNAPSHOT_FILE = ORIG.file
    if (ORIG.override === undefined) delete process.env.CCH_SNAPSHOT_OVERRIDE
    else process.env.CCH_SNAPSHOT_OVERRIDE = ORIG.override
    if (ORIG.children === undefined) delete process.env.CCH_SNAPSHOT_CHILD_FILES
    else process.env.CCH_SNAPSHOT_CHILD_FILES = ORIG.children
  })
  ...
  it('falls back to $CCH_SNAPSHOT_FILE / $CCH_SNAPSHOT_OVERRIDE when config is empty', async () => {
    writeFileSync(file, sessionJsonl([]), 'utf8')
    const overrideFile = join(dir, 'replay.override.json')
    writeFileSync(overrideFile, JSON.stringify([{ kind: 'chunks', chunks: TEXT_CHUNKS }]), 'utf8')
    process.env.CCH_SNAPSHOT_FILE = file
    process.env.CCH_SNAPSHOT_OVERRIDE = overrideFile
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    expect(await drain(ctx.llm.stream({ provider: 'm', model: 'm', messages: [] }))).toEqual(TEXT_CHUNKS)
  ...
  it('uses only the file when no override path is configured or in the env', async () => {
    writeFileSync(file, sessionJsonl(TEXT_CHUNKS.map((c, i) => chunkEvent(i + 1, 1, 1, c))), 'utf8')
    process.env.CCH_SNAPSHOT_FILE = file
    delete process.env.CCH_SNAPSHOT_OVERRIDE
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    expect(await drain(ctx.llm.stream({ provider: 'm', model: 'm', messages: [] }))).toEqual(TEXT_CHUNKS)
  ...
  it('throws when no fixture path is given by config or env', async () => {
    delete process.env.CCH_SNAPSHOT_FILE
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
  ...
  it('falls back to $CCH_SNAPSHOT_CHILD_FILES (path-delimited) when config omits childFiles', async () => {
    ...
    process.env.CCH_SNAPSHOT_FILE = file
    process.env.CCH_SNAPSHOT_CHILD_FILES = childFile // single entry, no delimiter needed
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    const live = (id: string): GenerateOptions =>
  ...
  it('ignores an empty $CCH_SNAPSHOT_CHILD_FILES (single-session)', async () => {
    writeFileSync(file, sessionJsonl(TEXT_CHUNKS.map((c, i) => chunkEvent(i + 1, 1, 1, c)), { id: 'p', createdAt: 1 }), 'utf8')
    process.env.CCH_SNAPSHOT_FILE = file
    process.env.CCH_SNAPSHOT_CHILD_FILES = ''
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    expect(await drain(ctx.llm.stream({ provider: 'm', model: 'm', messages: [] }))).toEqual(TEXT_CHUNKS)
