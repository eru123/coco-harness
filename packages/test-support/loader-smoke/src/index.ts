 ...
 * zero-build dev path, resolving `@coco-harness/cch-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @coco-harness/cch-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'CCH_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.CCH_EXAMPLE_MODE`.
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { CCH_HOME: join(cwd, '.cch'), CCH_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
