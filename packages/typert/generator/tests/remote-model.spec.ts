...
function copyFixture(sourceRoot = fixtureRoot): string {
  const root = mkdtempSync(join(tmpdir(), 'cch-typert-remote-model-'))
...
`
  ...
  writeFileSync(configPath, JSON.stringify({
    ...
    compilerOptions: {
      ...
      skipLibCheck: false,
      paths: {
        '@coco-harness/cch-typert-protocol': ['./typert-protocol.d.ts'],
...
import type { TypertRemoteNamespaceMap } from '@coco-harness/cch-typert-protocol'
declare const ctx: { remote: TypertRemoteNamespaceMap }
