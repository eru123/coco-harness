...
import type { TypertContribution } from '@coco-harness/cch-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@coco-harness/cch-tool-cordis/src/api-catalog.ts'
...
describe('model-driven cch-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@coco-harness/cch-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('cch-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@coco-harness/cch-tools')
    ...
    const record = ctx.typert.getPackage('@coco-harness/cch-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@coco-harness/cch-tools', 'host')).toBeUndefined()
  })
})
