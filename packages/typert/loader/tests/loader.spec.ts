...
import * as typertLoader from '@coco-harness/cch-typert-loader'
import { validateTypertManifest } from '@coco-harness/cch-typert-loader'
import { z } from 'zod'
...
describe('typert loader', () => {
  it('registers an explicit package without a Loader entry and withdraws it with the loader', LOADER_TEST_TIMEOUT, async () => {
    root = await mkdtemp(join(tmpdir(), 'cch-typert-loader-'))
    await linkZod(root)
