...
} from '@coco-harness/cch-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@coco-harness/cch-typert-protocol'
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@coco-harness/cch-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@coco-harness/cch-tools', 'ToolInput')).toBe('@coco-harness/cch-tools#ToolInput')
    expect(typertPackageKey('@coco-harness/cch-tools', 'host')).toBe('@coco-harness/cch-tools#host')
    expect(ctx.typert.get('@coco-harness/cch-tools#ToolInput')).toMatchObject({
      package: '@coco-harness/cch-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@coco-harness/cch-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@coco-harness/cch-tools', 'host')).toMatchObject({
      key: '@coco-harness/cch-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@coco-harness/cch-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@coco-harness/cch-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@coco-harness/cch-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@coco-harness/cch-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@coco-harness/cch-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@coco-harness/cch-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@coco-harness/cch-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@coco-harness/cch-tools#ToolInput')).not.toBe(projected)
  })
