...
```ts
import { runNativeCommand, type NativeCommandRunner } from '@coco-harness/cch-native-command'
```
...
- **No output bounding** — both streams buffer unbounded in memory; every current caller invokes small native tools whose output is a path or an error line. Adopt `cch-output-retention` bounding before pointing this at commands with meaningful output volume.
