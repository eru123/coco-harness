...
| `web_fetch` | `url` (string) | Retrieves a specific URL. HTML bodies are rendered to markdown (turndown with GFM tables/strikethrough); text bodies pass through. A non-2xx status is reported, not an error. The tool-call timeout is deployment policy (`cch-tool-call-timeout-policy`), not a model argument. |
...
The model sees the generated [`web_search` and `web_fetch` schemas](../../../docs/tool-catalog.md#coco-harnesscch-tool-web). Result-count and timeout budgets are deployment settings, not model arguments.
