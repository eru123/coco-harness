/**
 * Integration: the real fetch backend (`cch-web-fetch-http`) + a real search provider
 * (`cch-web-search-exa`) + the real seam (`cch-web`) + the model tool (`cch-tool-web`) + the
 * tool-call timeout policy (`cch-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@coco-harness/cch-tools'
...
import * as TimeoutPolicy from '@coco-harness/cch-tool-call-timeout-policy'
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by cch-tool-call-timeout-policy,
