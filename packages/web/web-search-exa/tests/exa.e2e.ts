...
import { ExaSearchProvider, EXA_DEFAULT_BASE_URL, EXA_DEFAULT_HIGHLIGHTS_PER_RESULT, EXA_DEFAULT_SEARCH_TYPE } from '@coco-harness/cch-web-search-exa'
...
maybe('ExaSearchProvider real API', () => {
  it('returns sources for a live query', async () => {
    ...
    const result = await provider.search({ query: 'Coco Harness', maxResults: 5 })
