...
export class WebRuntime extends Service {
   ...
   * `$CCH_WEB_SEARCH_PROVIDER` / `$CCH_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.CCH_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.CCH_WEB_FETCH_PROVIDER
  }
