...
`@coco-harness/cch-workflow-worker-thread` is the current engine and `@coco-harness/cch-tool-workflow` is the model-facing consumer. A future process or sandbox engine can replace the implementation without changing the tool.

The package root is the Host face. The browser-safe `@coco-harness/cch-workflow/types` subpath contains run identities, metadata, results, and observe-only lifecycle payloads without importing `Agent`, Cordis services, or Host context declarations; Host-only `WorkflowStartRequest` and `WorkflowRun` live behind the package root.
...
Indirectly, through `cch-tool-workflow` and a workflow engine, which create child-agent requests and return a retained parent tool result.
