...
def _host_platform_tag() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"unsupported coco-harness-runtime-bin build platform: {key}") from exc
...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        if self.target_name == "sdist":
            raise RuntimeError(
                "coco-harness-runtime-bin is wheel-only; build and publish platform wheels only."
            )

        platform_tag = os.environ.get("CCH_RUNTIME_PLATFORM_TAG") or _host_platform_tag()
        ...
        if len(matches) != 1:
            ...
            raise RuntimeError(
                f"unsupported CCH_RUNTIME_PLATFORM_TAG {platform_tag!r}; expected one of {supported}"
            )
        expected_executable = matches[0][1]
        runtime_dir = Path(self.root) / "src" / "coco_harness_runtime" / "runtime"
        runtime_files = sorted(runtime_dir.glob("cch-jsonrpc-agent-pkg-*") if runtime_dir.is_dir() else [])
