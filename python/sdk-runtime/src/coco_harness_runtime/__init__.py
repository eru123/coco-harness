"""Locate the bundled Coco Harness SDK runtime shipped with this package.
...
- **exe (production)**: single-file Node executables named
  ``cch-jsonrpc-agent-pkg-<platform>-<arch>`` (platform in {linux, macos}, arch in
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@coco-harness/cch-sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
the client SDK injects via ``$CCH_CORDIS_CONFIG`` for zero-config runs — the
...
PACKAGE_METADATA_FILENAME = "coco-harness-runtime.json"

RUNTIME_MODE_ENV_VAR = "CCH_RUNTIME_MODE"
...
_EXE_ACQUISITION_HINT = (
    ...
    "coco-harness checkout, or install the matching `coco-harness-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"coco-harness-runtime-bin is missing {metadata}")
    return root
...
def bundled_default_config_path() -> Path:
    ...
    The client SDK injects this path via ``$CCH_CORDIS_CONFIG`` when the caller
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"coco-harness-runtime-bin is missing the default runtime config at {path}"
        )
    return path
...
def bundled_runtime_path() -> Path:
    ...
    path = bundled_package_dir() / "runtime" / f"cch-jsonrpc-agent-pkg-{tag}"
    if not path.is_file():
        raise FileNotFoundError(
            f"coco-harness-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"coco-harness-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    ``CCH_RUNTIME_MODE`` environment variable (``exe`` | ``node``), then
    ...
    raise ValueError(
        f"unsupported Coco Harness runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _current_platform_tag() -> str:
    ...
    if plat is None or arch is None:
        raise FileNotFoundError(
            "no bundled cch-jsonrpc-agent executable exists for this platform "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    if not bin_js.is_file():
        raise FileNotFoundError(
            ...
            f"(no {bin_js}); run `scripts/build-exe-for-python-sdk.ts` in a coco-harness "
