...
class CocoHarness:
    """Reusable synchronous SDK for running Coco Harness agent turns.
    ...
    def __init__(self, config: CocoHarnessConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either CocoHarnessConfig or keyword options, not both")
        self.config = config or CocoHarnessConfig(**kwargs)
        ...
        if self.config.session_root is not None:
            env["CCH_SESSION_ROOT"] = self.config.session_root
        if self.config.cordis is not None:
            env["CCH_CORDIS_CONFIG"] = self.config.cordis
        env["CCH_CWD"] = cwd
    ...
    def __enter__(self) -> "CocoHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: CocoHarness, session_id: str) -> None:
