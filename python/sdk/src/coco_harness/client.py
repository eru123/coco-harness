...
class HarnessConfig:
    """Configuration for launching the local Coco Harness SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Coco Harness SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        try:
            ...
            proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Coco Harness runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Coco Harness runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Coco Harness runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Coco Harness runtime") from exc

    def _start_reader_thread(self) -> None:
        self._reader_thread = threading.Thread(target=self._reader_loop, name="cch-runtime-reader", daemon=True)
        self._reader_thread.start()

    def _start_stderr_thread(self) -> None:
        self._stderr_thread = threading.Thread(target=self._stderr_loop, name="cch-runtime-stderr", daemon=True)
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Coco Harness runtime stdout closed"))
    ...
    def _default_launch_args(self) -> tuple[str, ...]:
        ...
        try:
            from coco_harness_runtime import resolve_bundled_launch_args
        except ImportError as exc:
            raise FileNotFoundError(
                "Unable to locate the bundled Coco Harness SDK runtime. "
                "Install coco-harness-runtime-bin or set HarnessConfig.runtime_bin."
            ) from exc
    ...
    def _inject_bundled_default_config(self, env: dict[str, str]) -> None:
        ...
        if not uses_bundled_runtime or env.get("CCH_CORDIS_CONFIG"):
            return
        # _default_launch_args already imported the package or raised its install error.
        from coco_harness_runtime import bundled_default_config_path

        env["CCH_CORDIS_CONFIG"] = str(bundled_default_config_path())
