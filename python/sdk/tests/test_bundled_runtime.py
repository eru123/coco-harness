...
from coco_harness import CocoHarness, HarnessClient, HarnessConfig
from coco_harness.errors import TransportClosedError
...
def _client(tmp_path: Path, launch_args: tuple[str, ...]) -> HarnessClient:
    return HarnessClient(
        HarnessConfig(
            ...
            cwd=str(tmp_path),
            env={
                "CCH_CORDIS_CONFIG": "./cordis.yml",
                "CCH_SESSION_ROOT": str(tmp_path / "sessions"),
                "CCH_CWD": str(tmp_path),
...
def test_bundled_runtime_boots_a_cordis_config(tmp_path: Path, mode: str) -> None:
    ...
    assert init.serverInfo.name == "coco-harness-sdk-runtime"
...
def test_python_sdk_boots_minimal_jsonrpc_config(tmp_path: Path, mode: str) -> None:
    ...
    harness = CocoHarness(
        ...
        env={
            "CCH_MODEL": model,
            "CCH_CONTEXT_WINDOW": "1000000",
            "CCH_SYSTEM_PROMPT": "You are the Python SDK minimal boot test agent.",
        },
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@coco-harness/cch-does-not-exist'\n"
    )
    ...
    assert "@coco-harness/cch-does-not-exist" in str(excinfo.value)
...
def test_zero_config_run_injects_bundled_default_cordis_config(
    ...
    monkeypatch.setenv("CCH_RUNTIME_MODE", mode)
