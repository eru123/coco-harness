...
from coco_harness import CocoHarness, HarnessClient, HarnessConfig, Notification, SdkProtocolError
...
json.dump({
    ...
    "CCH_CWD": os.environ.get("CCH_CWD"),
    "CCH_SESSION_ROOT": os.environ.get("CCH_SESSION_ROOT"),
    "CCH_CORDIS_CONFIG": os.environ.get("CCH_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    assert dumped_env["CCH_CWD"] == str(tmp_path)
    assert dumped_env["CCH_SESSION_ROOT"] == str(tmp_path / "sessions")
    assert dumped_env["CCH_CORDIS_CONFIG"] == str(tmp_path / "cordis.yml")
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("CCH_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with CocoHarness(launch_args_override=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
for line in sys.stdin:
    ...
    method = msg.get("method")
    if method == "initialize":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-cch"}}}), flush=True)
    elif method == "session/prompt":
        params = msg.get("params") or {}
...
""".strip()
    ...
    with HarnessClient(
        ...
        assert init.serverInfo.name == "fake-cch"
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from coco_harness import CocoHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(CocoHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in CocoHarnessConfig.__dataclass_fields__
    assert "max_tokens" in CocoHarnessConfig.__dataclass_fields__
...
def _install_fake_bundled_runtime(
    ...
    """
    runtime = tmp_path / "cch-jsonrpc-agent"
    runtime.write_text(
        """#!/usr/bin/env python3
import json
import os
...
json.dump({"CCH_CORDIS_CONFIG": os.environ.get("CCH_CORDIS_CONFIG")}, open(os.environ["ENV_DUMP"], "w"))
for line in sys.stdin:
    msg = json.loads(line)
    if msg.get("method") == "initialize":
...
""".strip()
    ...
    module_dir = tmp_path / "coco_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "coco_harness_runtime", raising=False)
...
def test_client_default_launch_uses_bundled_runtime_and_injects_default_config(
    ...
    if ambient_config is None:
        monkeypatch.delenv("CCH_CORDIS_CONFIG", raising=False)
    else:
        monkeypatch.setenv("CCH_CORDIS_CONFIG", ambient_config)
    ...
    assert json.loads(env_dump.read_text())["CCH_CORDIS_CONFIG"] == str(default_config)
...
def test_client_respects_explicit_config_over_bundled_default(
    ...
    with HarnessClient(
        HarnessConfig(env={"ENV_DUMP": str(env_dump), "CCH_CORDIS_CONFIG": "./explicit.yml"})
    ) as client:
        client.initialize(provider="deepseek-official", cwd="/workspace", model="deepseek-v4-pro")

    assert json.loads(env_dump.read_text())["CCH_CORDIS_CONFIG"] == "./explicit.yml"
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install coco-harness-runtime-bin"):
        HarnessClient().start()
