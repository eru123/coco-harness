...
def test_stage_sdk_keeps_distribution_module_and_runtime_pin_distinct(tmp_path: Path) -> None:
    ...
    assert 'name = "coco-harness-sdk"' in pyproject
    assert 'version = "1.2.3"' in pyproject
    assert 'license = "MIT"' in pyproject
    assert '"coco-harness-runtime-bin==1.2.3"' in pyproject
    assert 'license-files = ["LICENSE"]' in pyproject
    assert (destination / "LICENSE").read_bytes() == (ROOT / "LICENSE").read_bytes()
    assert (destination / "src" / "coco_harness" / "__init__.py").is_file()
...
def test_stage_runtime_copies_platform_payload(
    ...
    executable = tmp_path / f"cch-jsonrpc-agent-pkg-{target}"
    ...
    assert {path.name: path.read_bytes() for path in runtime_dir.glob("cch-jsonrpc-agent-pkg-*")} == expected
    pyproject = (destination / "pyproject.toml").read_text()
    assert 'license = "MIT"' in pyproject
