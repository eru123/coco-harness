...
import coco_harness_runtime as runtime
import pytest

from coco_harness_runtime import (
...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@coco-harness/cch-agent-spine-demo" in config
    assert "@coco-harness/cch-session-persistence-jsonl" in config
    assert "@coco-harness/cch-session-checkpoint-policy" in config
...
def test_runtime_requires_spawn_helper_only_on_macos(
    ...
    runtime_dir.mkdir()
    linux = runtime_dir / "cch-jsonrpc-agent-pkg-linux-x64"
    linux.touch()
    (runtime_dir / "cch-jsonrpc-agent-pkg-macos-arm64").touch()
