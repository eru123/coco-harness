...
function clientConfigs(id = '@coco-harness/cch-client-test') {
  return clientBundle(id, ['lib/types/index.js', 'lib/types/invariant.js'])(
    { env: { CCH_BUILD_FACE: 'client' } },
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@coco-harness/cch-client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { CCH_BUILD_FACE: 'client' } })
...
function purityResolveId(): ResolveId {
  ...
  const gate = plugins.find(p => p.name === 'cch-client-bundle-purity')
...
function cssModulePlugin(): CssModulePlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'cch-css-modules-inline')
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@coco-harness/cch-client-ui-slots')).toBeNull()
    expect(resolveId('@coco-harness/cch-client-web-react')).toBeNull()
    expect(resolveId('@coco-harness/cch-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@coco-harness/cch-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@coco-harness/cch-host-apiproxy/api')).toBeNull()
    expect(resolveId('@coco-harness/cch-session/surface')).toBeNull()
    expect(resolveId('@coco-harness/cch-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@coco-harness/cch-goal/remote')).toBeNull()
    expect(() => resolveId('@coco-harness/cch-goal')).toThrow(/purity/)
    expect(() => resolveId('@coco-harness/cch-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@coco-harness/cch-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @coco-harness leak', () => {
    expect(() => resolveId('@coco-harness/cch-agent')).toThrow(/purity/)
    expect(() => resolveId('@coco-harness/cch-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@coco-harness/cch-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@coco-harness/cch-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@coco-harness/cch-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@coco-harness/cch-client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@coco-harness/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@coco-harness/cch-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@coco-harness/cch-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://cch.test/plugins/@coco-harness/cch-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@coco-harness/cch-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@coco-harness/cch-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://cch.test/plugins/@coco-harness/cch-client-connection/client.js.map')
