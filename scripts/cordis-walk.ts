...
const MERGE_HEAD = /declare module ['"](?:@coco-harness\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@coco-harness/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@coco-harness/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@coco-harness/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@coco-harness/cordis'` Context merge
...
/**
 * Every event name a `declare module '@coco-harness/cordis'` Events merge declares in one
