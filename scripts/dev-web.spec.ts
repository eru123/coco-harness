...
it('discovers cch.client packages with sibling roles', async () => {
  const root = await mkdtemp(join(tmpdir(), 'cch-dev-web-discovery-'))
  try {
...
it('rebuilds a client-plugin bundle after its source changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'cch-dev-web-watch-'))
  let bundles: TsdownBundle[] = []
  try {
    await symlink(join(import.meta.dirname, '..', 'node_modules'), join(root, 'node_modules'), 'dir')
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: '@cch-test/dev-web-watch', private: true, type: 'module' }))
