/**
 * Watch-build for client-plugin HMR: runs every `cch.client` plugin package
 ...
 * broadcasts `rebuilt` frames itself (`cch web`), so any process that
 ...
 * whose package.json carries `cch.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      cch?: { client?: { platform?: unknown } }
    }
    if (manifest.cch?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
...
if (isMain) {
  ...
  if (pluginDirs.length === 0) {
    console.error('dev-web: no cch.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  console.log(
    `dev-web: watching ${String(pluginDirs.length)} cch.client plugin packages`
