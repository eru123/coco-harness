...
/** Drop the `@coco-harness/cch-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@coco-harness/cch-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @coco-harness/cch-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
