...
const SESSION_PACKAGE = '@coco-harness/cch-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@coco-harness/cch-session/types'
 ...
 * top-level declaration (in `@coco-harness/cch-session`) and any declaration
 * merge inside a `declare module '@coco-harness/cch-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @coco-harness/cch-session/known-event-types',
    ' */',
    '',
    '/**',
