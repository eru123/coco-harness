/**
 * Generate cch-scope's invariant resolver map from the repository TypeScript
 * Program.
 *
 ...
 * `Events` parameter tuple. Zero matches require `@cchScopeScan unsupported`;
...
class ScopedEventGenerator {
  ...
  render(): string {
    ...
    return [
      '/**',
      ' * Generated scoped-event routing-subject resolvers for cch-scope invariants.',
      ' * Do not edit by hand; run `pnpm run gen-scoped-events`.',
      ' *',
      ' * @module @coco-harness/cch-scope/scoped-events.generated',
      ' */',
      '',
        ...
        if (ts.isInterfaceDeclaration(node) && node.name.text === 'Events' && isCordisModuleInterface(node)) {
          for (const member of node.members) {
            ...
            if (!scopedBase) {
              ...
              if (tag.present) {
                this.violations.push(`${where} has @cchScopeScan metadata but is not a Scoped event`)
              }
              continue
            }
            if (!raw.includes(SCOPE_DOC_MARKER)) {
            ...
            if (candidates.length === 0) {
              if (!tag.unsupported) {
                ...
                this.violations.push(
                  ...
                  + 'add @cchScopeScan unsupported only when the key is intentionally absent from the payload',
                )
              }
            ...
            if (tag.unsupported) {
              this.violations.push(
                `${where} has unnecessary @cchScopeScan unsupported; ${candidates[0]?.path} exposes the routing key`,
              )
              continue
            }
...
/** Return whether an Events interface is inside declare module '@coco-harness/cordis'. */
function isCordisModuleInterface(node: ts.InterfaceDeclaration): boolean {
  ...
  return ts.isModuleBlock(block)
    ...
    && declaration.name.text === '@coco-harness/cordis'
}
...
/** Parse and validate the optional @cchScopeScan unsupported tag. */
function parseScopeTag(raw: string, where: string, violations: string[]): ScopeTag {
  const tags = raw
    ...
    .filter(line => line.startsWith('@cchScopeScan'))
  if (tags.length > 1) violations.push(`${where} has multiple @cchScopeScan tags`)
  if (tags.length === 0) return { present: false, unsupported: false }
  const unsupported = tags[0] === '@cchScopeScan unsupported'
  if (!unsupported) {
    violations.push(
      `${where} has invalid scoped-event scan metadata '${tags[0]}'; expected '@cchScopeScan unsupported'`,
    )
  }
...
/** Generate or freshness-check the fixed cch-scope source file. */
function main(): void {
