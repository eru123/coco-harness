...
describe('tierExternalDeps', () => {
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/test-support/loader-smoke/package.json': { name: '@coco-harness/cch-loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/test-support/client-runtime/package.json': { name: '@coco-harness/cch-client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@coco-harness/cch-mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@coco-harness/cch-cli', dependencies: { 'cli-lib': '^1', '@coco-harness/cch-mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@coco-harness/cch-tui', dependencies: { shared: '^1', '@coco-harness/cch-cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@coco-harness/cch-cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@coco-harness/cch-cli')).toBe(false)
  })
})

describe('virtualManifest', () => {
  it('resolves a manifest from an ordinary prefix-matching store directory', () => {
    const root = mkdtempSync(join(tmpdir(), 'cch-notices-prefix-'))
    try {
  ...
  it('falls back to a content scan when pnpm 11 truncates the store directory name', () => {
    const root = mkdtempSync(join(tmpdir(), 'cch-notices-truncated-'))
    try {
      const name = '@scope/pkg'
      const version = '2.0.0'
      const store = join(root, 'store')
  ...
  it('returns undefined when neither the prefix nor the content scan finds the package', () => {
    const root = mkdtempSync(join(tmpdir(), 'cch-notices-miss-'))
    try {
      const store = join(root, 'store')
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@coco-harness/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
...
describe('collectPythonDependencies', () => {
  it('excludes normalized local project names without exempting a third-party prefix', () => {
    const pyprojects = [
      '[project]\nname = "coco-harness-runtime-bin"\ndependencies = ["pydantic"]\n',
      '[project]\nname = "coco-harness-sdk"\ndependencies = ["Coco.Harness_Runtime-Bin", "deepseek-unrelated"]\n',
    ]
