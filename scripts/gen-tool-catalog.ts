...
import type { ToolSchema } from '@coco-harness/cch-llm'
...
import SqliteSessionQueryEngine from '@coco-harness/cch-session-query-sqlite'
import GoalService from '@coco-harness/cch-goal'
import SystemPrompt from '@coco-harness/cch-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@coco-harness/cch-tools'
import LocalBashExecutor from '@coco-harness/cch-bash-local'
...
import PlanModeController from '@coco-harness/cch-plan-mode'
import WebRuntime from '@coco-harness/cch-web'
import * as WebSearchExa from '@coco-harness/cch-web-search-exa'
import * as WebFetchLocal from '@coco-harness/cch-web-fetch-http'
import SubagentRuntime from '@coco-harness/cch-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@coco-harness/cch-subagent'
import * as ToolSubagentControl from '@coco-harness/cch-tool-subagent-control'
import * as ToolSubagentListAgents from '@coco-harness/cch-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@coco-harness/cch-tool-subagent-report'
import SkillRegistry from '@coco-harness/cch-skill'
import * as SkillFileSystem from '@coco-harness/cch-skill-filesystem'
import LocalJobRegistry from '@coco-harness/cch-jobs-local'
import * as ToolAskUser from '@coco-harness/cch-tool-ask-user'
import * as ToolBash from '@coco-harness/cch-tool-bash'
import * as ToolPwsh from '@coco-harness/cch-tool-pwsh'
import * as ToolBashPersistent from '@coco-harness/cch-tool-bash-persistent'
import CordisHostRunner from '@coco-harness/cch-cordis-host-runner'
import * as ToolCordis from '@coco-harness/cch-tool-cordis'
import * as ToolFs from '@coco-harness/cch-tool-fs'
import * as ToolFsSearch from '@coco-harness/cch-tool-fs-search'
...
import * as ToolSchedule from '@coco-harness/cch-schedule'
import Lsp from '@coco-harness/cch-lsp'
import * as ToolLsp from '@coco-harness/cch-tool-lsp'
import * as ToolSkill from '@coco-harness/cch-tool-skill'
import * as ToolSessionQuery from '@coco-harness/cch-tool-session-query'
import * as ToolTasks from '@coco-harness/cch-tool-jobs'
import * as ToolTodo from '@coco-harness/cch-tool-todo'
import * as ToolSubagent from '@coco-harness/cch-tool-subagent'
import * as ToolWeb from '@coco-harness/cch-tool-web'
import VmWorkflowEngine from '@coco-harness/cch-workflow-worker-thread'
import * as ToolRalph from '@coco-harness/cch-tool-ralph'
import * as ToolWorkflow from '@coco-harness/cch-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@coco-harness/cch-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  {
    pkg: '@coco-harness/cch-plan-mode',
    dir: 'plan-mode',
  ...
  {
    pkg: '@coco-harness/cch-tool-bash',
    dir: 'tool-bash',
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@coco-harness/cch-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@coco-harness/cch-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@coco-harness/cch-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `CCH_*` environment comes from `@coco-harness/cch-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@coco-harness/cch-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@coco-harness/cch-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or CCH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@coco-harness/cch-tool-bash-persistent',
  ...
  {
    pkg: '@coco-harness/cch-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@coco-harness/cch-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@coco-harness/cch-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@coco-harness/cch-tool-terminal',
    dir: 'tool-terminal',
  ...
  {
    pkg: '@coco-harness/cch-tool-goal',
    dir: 'tool-goal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@coco-harness/cch-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@coco-harness/cch-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@coco-harness/cch-tool-skill',
    dir: 'tool-skill',
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        cchHome: resolve(root, '.tmp/tool-catalog/.cch'),
  ...
  {
    pkg: '@coco-harness/cch-tool-session-query',
  ...
  {
    pkg: '@coco-harness/cch-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@coco-harness/cch-tool-subagent-control',
  ...
  {
    pkg: '@coco-harness/cch-tool-subagent-report',
  ...
  {
    pkg: '@coco-harness/cch-tool-jobs',
    dir: 'tool-jobs',
  ...
  {
    pkg: '@coco-harness/cch-tool-todo',
    dir: 'tool-todo',
  ...
  {
    pkg: '@coco-harness/cch-tool-workflow',
    dir: 'tool-workflow',
