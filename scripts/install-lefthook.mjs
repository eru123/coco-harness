...
const HOOKS_DIRECTORY = 'cch-hooks'
const OWNERSHIP_MARKER = '.cch-lefthook-owned'
const OWNERSHIP_MARKER_VERSION = 1
const OWNERSHIP_MARKER_OWNER = 'coco-harness worktree-local lefthook hooks'
const INSTALL_LOCK = 'cch-lefthook-install.lock'
const INSTALL_LOCK_TIMEOUT_MS = 30_000
const INSTALL_LOCK_INITIALIZATION_TIMEOUT_MS = 1_000
const INSTALL_LOCK_POLL_MS = 50
const ALLOW_HOOKS_PATH_OVERRIDE = 'CCH_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE'
const REPOSITORY_EXTENSION_PATTERN = '^extensions\\.'
const PAIRING_MERGE_DRIVER_CONFIG = [
  ['merge.cch-translation-pairing.name', 'Coco Harness bilingual pairing records'],
  [
  ...
  while (true) {
    try {
      ...
      try {
        ...
        const writeDelay = Number(process.env.CCH_TEST_LEFTHOOK_LOCK_WRITE_DELAY_MS ?? 0)
        if (writeDelay > 0) {
