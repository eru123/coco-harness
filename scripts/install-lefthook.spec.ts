...
const forbiddenConfigKey = process.env.CCH_TEST_FORBIDDEN_GIT_CONFIG_KEY
...
const delay = Number(process.env.CCH_TEST_LEFTHOOK_DELAY_MS ?? 0)
if (delay > 0) Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, delay)
const shouldFail = process.env.CCH_TEST_LEFTHOOK_FAIL === '1'
if (!shouldFail) {
...
if (process.env.CCH_TEST_LEFTHOOK_BREAK_WORKTREE_CONFIG === '1') {
...
function createFixture(names: { main?: string; linked?: string } = {}): Fixture {
  const container = mkdtempSync(join(tmpdir(), 'cch-lefthook-'))
...
function hooksPath(fixture: Fixture, root: string): string {
  return join(gitDirectory(fixture, root), 'cch-hooks')
}

function installLockPath(fixture: Fixture): string {
  return join(commonDirectory(fixture), 'cch-lefthook-install.lock')
}
...
describe('worktree-local Lefthook installer', { timeout: 15_000 }, () => {
  for (const [label, extraEnv] of [
    ...
    it(`skips hook installation when ${label} marks an automated job`, async () => {
      ...
      expect(gitResult(fixture, fixture.main, [
        'config', '--get', 'merge.cch-translation-pairing.driver',
      ]).status).toBe(1)
    })
  }
  ...
  it('isolates main and linked worktrees without changing legacy common hooks', async () => {
    ...
    expect(git(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.cch-translation-pairing.driver',
  ...
  it('serializes concurrent installs and keeps repeated output stable', async () => {
    const fixture = createFixture()
    const delayed = { CCH_TEST_LEFTHOOK_DELAY_MS: '150' }
    ...
    expect(existsSync(join(commonDirectory(fixture), 'cch-lefthook-install.lock'))).toBe(false)
  ...
  it('waits for a concurrent installer to finish publishing its lock record', async () => {
    ...
    const publishing = runInstaller(fixture, fixture.main, {
      CCH_TEST_LEFTHOOK_LOCK_WRITE_DELAY_MS: '200',
    })
  ...
  it('repairs its owned absolute hook path after the checkout moves', async () => {
    ...
    expect(readFileSync(join(movedHooks, '.cch-lefthook-owned'), 'utf8')).toContain(
  ...
  it.skipIf(process.platform === 'win32')('refuses a multiply linked ownership marker before relocation rewrites it', async () => {
    ...
    expect(first.status, first.stderr).toBe(0)
    const oldHooks = hooksPath(fixture, oldRoot)
    const markerName = '.cch-lefthook-owned'
  ...
  it('restores the marker-backed stale hook path when relocation reinstall fails', async () => {
    ...
    const failed = await runInstaller(fixture, movedRoot, { CCH_TEST_LEFTHOOK_FAIL: '1' })
  ...
  it('refuses dormant repository extensions before upgrading the repository format', async () => {
    ...
    git(fixture, fixture.main, ['config', 'extensions.cchUnknown', 'true'])
  ...
  it('does not release an installer lock whose ownership changed', async () => {
    ...
    const install = runInstaller(fixture, fixture.main, { CCH_TEST_LEFTHOOK_DELAY_MS: '250' })
  ...
  it('preserves user-owned hook paths unless an inherited value is explicitly overridden', async () => {
    ...
    expect(refused.stderr).toContain('CCH_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE=1')
    ...
    const optedIn = await runInstaller(fixture, fixture.main, {
      CCH_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE: '1',
    })
  ...
  it('does not trust an ownership marker outside a registered worktree hook path', async () => {
    ...
    write(
      join(externalHooks, '.cch-lefthook-owned'),
      `${JSON.stringify({
        version: 1,
        owner: 'coco-harness worktree-local lefthook hooks',
  ...
  it('never replaces a custom worktree pairing merge driver', async () => {
    ...
    git(fixture, fixture.main, ['config', '--file', commonConfig, 'extensions.worktreeConfig', 'true'])
    git(fixture, fixture.main, [
      'config', '--worktree', 'merge.cch-translation-pairing.driver', 'custom-driver %A',
    ])

    const result = await runInstaller(fixture, fixture.main)

    expect(result.status).toBe(1)
    expect(result.stderr).toContain('refusing to replace worktree merge.cch-translation-pairing.driver')
    expect(git(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.cch-translation-pairing.driver',
  ...
  it('never masks an inherited custom pairing merge driver', async () => {
    const fixture = createFixture()
    git(fixture, fixture.main, [
      'config', '--local', 'merge.cch-translation-pairing.driver', 'inherited-driver %A',
    ])

    const result = await runInstaller(fixture, fixture.main)

    expect(result.status).toBe(1)
    expect(result.stderr).toContain('refusing to mask inherited merge.cch-translation-pairing.driver')
    expect(git(fixture, fixture.main, [
      'config', '--local', '--get', 'merge.cch-translation-pairing.driver',
  ...
  it('does not pass unrelated command-scoped Git config to Lefthook', async () => {
    ...
    const result = await runInstaller(fixture, fixture.main, {
      CCH_TEST_FORBIDDEN_GIT_CONFIG_KEY: 'cch.testSentinel',
      GIT_CONFIG_COUNT: '1',
      GIT_CONFIG_KEY_0: 'cch.testSentinel',
  ...
  it('restores the previous hook lookup when Lefthook installation fails', async () => {
    ...
    const result = await runInstaller(fixture, fixture.main, { CCH_TEST_LEFTHOOK_FAIL: '1' })
    expect(result.status).toBe(1)
    ...
    expect(gitResult(fixture, fixture.main, ['config', '--get', 'core.hooksPath']).status).toBe(1)
    expect(gitResult(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.cch-translation-pairing.name',
    ]).status).toBe(1)
    expect(gitResult(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.cch-translation-pairing.driver',
    ]).status).toBe(1)
  ...
  it('reports installation and hook-path rollback failures together', async () => {
    ...
    const result = await runInstaller(fixture, fixture.main, {
      CCH_TEST_LEFTHOOK_BREAK_WORKTREE_CONFIG: '1',
      CCH_TEST_LEFTHOOK_FAIL: '1',
    })
    ...
    expect(result.stderr).toContain('git config --worktree --unset-all merge.cch-translation-pairing.driver failed')
  })
