...
function fixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'cch-package-invariants-'))
  roots.push(root)
  const dir = join(root, 'packages/core/probe')
  mkdirSync(join(dir, 'src'), { recursive: true })
  const packageName = options.packageName ?? '@coco-harness/cch-probe'
  const manifest = {
    name: packageName,
    ...
    peerDependencies: options.invariantDependency === false ? {} : {
      '@coco-harness/cch-invariants': 'workspace:^',
    },
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) => {
  ctx.invariants.register('@coco-harness/cch-foreign', install)
...
`
  ...
  it('rejects generated markers and reporter-free executable installers', () => {
    const generated = fixture({
      source: `/** @generated */\n${handwrittenInvariant('@coco-harness/cch-probe')}`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) =>
  Promise.resolve(ctx.invariants.register('@coco-harness/cch-probe', install))
`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
  ctx.invariants.register('@coco-harness/cch-probe', () => {})
`,
    })
  ...
  it.each([
    ...
    const source = `${handwrittenInvariant('@coco-harness/cch-probe')}\n${defaultExport}\n`
...
export const name = 'probe-invariant'
export const inject = ['invariants']
const PACKAGE_NAME = '@coco-harness/cch-probe'
