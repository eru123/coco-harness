...
const RELEASE_ENTRY_PACKAGE = '@coco-harness/cch'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"cch web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed cch web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed cch web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
class DetachedWorktree {
  ...
  static create(repositoryRoot: string, commit: string, runner: CommandRunner): DetachedWorktree {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'cch-npm-baseline-'))
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@coco-harness/')) {
        throw new Error(`${manifestPath} must name an @coco-harness package`)
      }
      if (name === '@coco-harness/cch-root') {
...
/** Installs one complete bundle outside the workspace and probes the shipped cch entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'cch-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'cch-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
      ...
      const bin = resolve(consumerRoot, 'node_modules/@coco-harness/cch/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed cch bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed cch --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed cch entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed cch Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-cch-web-probe'], result)
    }
  }
}
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@coco-harness/') || name === '@coco-harness/cch-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.CCH_HOME = resolve(consumerRoot, '.cch')
  environment.CCH_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.CCH_TELEMETRY_DISABLED = '1'
