 ...
 *
 * The cch family shares one version across its members and the workspace root:
...
/** Release types the cch family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the cch family's version. */
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:cch <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Plan the cch family's rewrite: one version for every member and the root.
 * @param family - the cch family.
 * @param root - repository root.
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <cch|vendor> [version]')
  ...
  if (family.id === 'cch') {
    ...
    if (request === undefined) throw new Error('usage: release:cch <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:cch takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
