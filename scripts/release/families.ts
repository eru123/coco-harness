 ...
 * owns: `cch` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@coco-harness/cch-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@coco-harness/')) throw new Error(`${normalized} must name an @coco-harness package`)
...
class CchFamily extends ReleaseFamily {
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`cch release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@coco-harness/cch', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@coco-harness/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new CchFamily(), new VendorFamily()]
}
