/**
 * Rescope the vendored Cordis packages into the `@coco-harness` scope, and undo
 ...
 * `@coco-harness/cch-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@coco-harness/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@coco-harness/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@coco-harness/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@coco-harness/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@coco-harness/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@coco-harness/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@coco-harness/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@coco-harness/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@coco-harness/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@coco-harness/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@coco-harness/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@coco-harness\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@coco-harness/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@coco-harness/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@coco-harness/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@coco-harness/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@coco-harness/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@coco-harness/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@coco-harness` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@coco-harness\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@coco-harness/cordis']
    const dev = manifest.devDependencies?.['@coco-harness/cordis']

    if (!peer) errors.push(\`\${label}: @coco-harness/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @coco-harness/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @coco-harness/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    // The rescoped name is already covered by the `@coco-harness/.+` pattern beside it.
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@coco-harness/\')) {',
    replace: `      // Vendored packages are rescoped too (vendor/README.md), so publication
      ...
      if (!name.startsWith('@coco-harness/')) {`,
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@coco-harness/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@coco-harness/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@coco-harness/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@coco-harness/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @coco-harness, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@coco-harness\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@coco-harness/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@coco-harness\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@coco-harness/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @coco-harness, so the consumer installs this repository's
  ...
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'cch-packed-consumer', private: true, type: 'module' }))
...
function main(): void {
  ...
  for (const file of files) {
    ...
    // A tracked file deleted from the working tree (the removed zh corpus)
    // has no bytes to rewrite; skip it rather than fail the whole run.
