...
function readManifest(root: string, file: string): Record<string, unknown> {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(`verify-cch-package-licenses: ${file} must contain a JSON object.`)
  }
...
function workspaceManifestPaths(root: string): string[] {
  ...
  if (!isStringArray(workspaces)) {
    throw new Error('verify-cch-package-licenses: package.json workspaces must be a string array.')
  }
...
export function inspectCchPackageLicenses(root: string): CchPackageLicenseReport {
  ...
  for (const file of workspaceManifestPaths(root)) {
    ...
    if (typeof name !== 'string' || !CCH_PACKAGE_NAME.test(name)) continue
