...
#
# Environment: CCH_WINE_NODE_MAJOR (default $PRIMARY_NODE_VERSION, then 24)
# picks the Windows Node line; CCH_WINE_GATE_CACHE_DIR relocates the cache;
# CCH_WINE_GATE_KEEP=1 preserves the scratch tree for inspection.

set -euo pipefail

repo_root="$(git rev-parse --show-toplevel)"
node_major="${CCH_WINE_NODE_MAJOR:-${PRIMARY_NODE_VERSION:-24}}"
cache_dir="${CCH_WINE_GATE_CACHE_DIR:-$repo_root/.cache/wine-windows}"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/cch-wine-gates.XXXXXX")"
cleanup() {
  wineserver -k > /dev/null 2>&1 || true
  if [ "${CCH_WINE_GATE_KEEP:-0}" = '1' ]; then
...
build_gate() {
  ...
  wine_node "$scratch/logs/host-tsdown.log" "$tsdown_js" --env.CCH_BUILD_FACE host || return $?
  wine_node "$scratch/logs/client-tsc.log" "$tsc_js" -b tsconfig.client.json --pretty false || return $?
  wine_node "$scratch/logs/client-tsdown.log" "$tsdown_js" --env.CCH_BUILD_FACE client
}
site_gate() {
  cd website
