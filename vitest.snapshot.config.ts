...
if (process.env.CCH_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.CCH_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.CCH_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,
